...
bench_aes(void)
{
  ...
  for (len = 1; len <= 8192; len *= 2) {
    ...
    b1 = (char *)tor_malloc_zero(len);
    b2 = (char *)tor_malloc_zero(len);
    start = perftime();
    for (i = 0; i < iters; ++i) {
...
bench_onion_TAP(void)
{
  ...
  char _or[TAP_ONIONSKIN_REPLY_LEN];
  crypto_dh_t *dh_out;
  ...
  for (i = 0; i < iters; ++i) {
    ...
    onion_skin_TAP_server_handshake(os, key, NULL, _or,
  ...
  for (i = 0; i < iters; ++i) {
    char key_out[CPATH_KEY_MATERIAL_LEN];
    onion_skin_TAP_server_handshake(os, key2, key, _or,
                                    key_out, sizeof(key_out));
  }
  end = perftime();
  ...
  for (i = 0; i < iters; ++i) {
    ...
    s = onion_skin_TAP_client_handshake(dh, _or, key_out, sizeof(key_out));
    crypto_dh_free(dh);
    tor_assert(s == 0);
  }
  end = perftime();
...
bench_onion_ntor(void)
{
  ...
  for (i = 0; i < iters; ++i) {
    ...
    onion_skin_ntor_server_handshake(os, keymap, NULL, nodeid, _or,
                                key_out, sizeof(key_out));
  }
  end = perftime();
  ...
  for (i = 0; i < iters; ++i) {
    uint8_t key_out[CPATH_KEY_MATERIAL_LEN];
    int s;
    s = onion_skin_ntor_client_handshake(state, _or, key_out, sizeof(key_out));
    tor_assert(s == 0);
  }
  end = perftime();
  printf("Client-side, part 2: %f usec.\n",
...
bench_cell_aes(void)
{
  ...
  char *b = (char *)tor_malloc(len+max_misalign);
  crypto_cipher_t *c;
  int i, misalign;
...
bench_cell_ops(void)
{
  ...
  or_circuit_t *or_circ = (or_circuit_t *)tor_malloc_zero(sizeof(or_circuit_t));
  cell_t *cell = (cell_t *)tor_malloc(sizeof(cell_t));
  int outbound;
  uint64_t start, end;
 ...
 * some benchmarks. */
int
ecall_test_bench_main(int argc, char **argv)
{
  int i;
  ...
  options = options_new();
  init_logging();
  options->command = or_options_t::CMD_RUN_UNITTESTS;
