    ...
    void (*rs_test)(routerstatus_t *rs, time_t now))
{
  ...
  do {
    ...
    if (vrs) {
      ...
      // Fix me: Why add old router ???
      int ret = router_add_to_routerlist(generate_ri_from_rs(vrs),
                                           &msg,0,0);
      printf("Ret code is: %d", ret);
      test_assert(ret>=0);
      ++idx;
    }
  } while (vrs);
...
struct testcase_t dir_tests[] = {
  // DIR_LEGACY(nicknames), // PASS
  // DIR_LEGACY(formats), // PASS
  // DIR_LEGACY(versions), // PASS
  // DIR_LEGACY(fp_pairs), // PASS
  // DIR(split_fps, 0), // PASS
  // DIR_LEGACY(measured_bw_kb), // PASS
  // DIR_LEGACY(measured_bw_kb_cache), // PASS
  // DIR_LEGACY(param_voting), // PASS
  // DIR_LEGACY(v3_networkstatus),
  // DIR(random_weighted, 0), // PASS
  // DIR(scale_bw, 0), // PASS
  DIR_LEGACY(clip_unmeasured_bw_kb),
  DIR_LEGACY(clip_unmeasured_bw_kb_alt),
  DIR(fmt_control_ns, 0),
  // DIR(http_handling, 0), // PASS
  END_OF_TESTCASES
};
