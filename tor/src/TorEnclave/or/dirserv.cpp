...
{
  authdir_config_t *list = (authdir_config_t *)tor_malloc_zero(sizeof(authdir_config_t));
                       ...
                       authdir_config_t *list)
{
  ...
  status = (router_status_t *)digestmap_get(list->status_by_digest, d);
  if (!status) {
    status = (router_status_t *)tor_malloc_zero(sizeof(router_status_t));
    digestmap_set(list->status_by_digest, d, status);
  }

  if (nickname[0] != '!') {
    char *old_fp = (char *)strmap_get_lc(list->fp_by_name, nickname);
...
dirserv_get_name_status(const char *id_digest, const char *nickname)
{
  ...
  if ((fp_by_name =
       (char *)strmap_get_lc(fingerprint_list->fp_by_name, nickname))) {
                        ...
                        const char **msg, int should_log)
{
  ...
  status_by_digest = (router_status_t *)digestmap_get(fingerprint_list->status_by_digest,
                                   id_digest);
  if (status_by_digest)
...
dirserv_get_nickname_by_digest(const char *digest)
{
  ...
  status = (router_status_t *)digestmap_get(fingerprint_list->status_by_digest, digest);
...
{
  cached_dir_t *d = (cached_dir_t *)tor_malloc_zero(sizeof(cached_dir_t));
  d->refcnt = 1;
  d->dir = s;
  d->dir_len = strlen(s);
                                           ...
                                           time_t published)
{
  ...
  old_networkstatus = (cached_dir_t *)strmap_set(cached_consensuses, flavor_name,
                                 new_networkstatus);
...
dirserv_get_consensus(const char *flavor_name)
{
  if (!cached_consensuses)
    return NULL;
  return (cached_dir_t *)strmap_get(cached_consensuses, flavor_name);
}
                                       ...
                                       digestmap_t *omit_as_sybil)
{
  ...
  uptimes = (uint32_t *)tor_malloc(sizeof(uint32_t)*smartlist_len(rl->routers));
  /* Bandwidth for every active router. */
  bandwidths_kb = (uint32_t *)tor_malloc(sizeof(uint32_t)*smartlist_len(rl->routers));
  /* Bandwidth for every active non-exit router. */
  bandwidths_excluding_exits_kb =
    (uint32_t *)tor_malloc(sizeof(uint32_t)*smartlist_len(rl->routers));
  /* Weighted mean time between failure for each active router. */
  mtbfs = (double *)tor_malloc(sizeof(double)*smartlist_len(rl->routers));
  /* Time-known for each active router. */
  tks = (long *)tor_malloc(sizeof(long)*smartlist_len(rl->routers));
  /* Weighted fractional uptime for each active router. */
  wfus = (double *)tor_malloc(sizeof(double)*smartlist_len(rl->routers));
                          ...
                          time_t as_of)
{
  ...
  e = (mbw_cache_entry_t*)digestmap_get(mbw_cache, parsed_line->node_id);
  ...
  if (e) {
    ...
    e = (mbw_cache_entry_t *)tor_malloc(sizeof(*e));
    e->mbw_kb = parsed_line->bw_kb;
    e->as_of = as_of;
                                   ...
                                   time_t *as_of_out)
{
  ...
  if (mbw_cache && node_id) {
    v = (mbw_cache_entry_t*)digestmap_get(mbw_cache, node_id);
    if (v) {
      /* Found something */
      rv = 1;
                       ...
                       smartlist_t *routerstatuses)
{
  ...
  rs = (vote_routerstatus_t *)smartlist_bsearch(routerstatuses, parsed_line->node_id,
...
{
  char line[512];
  SGX_WRAPPER_FILE fp = tor_fopen_cloexec(from_file, "r");
  int applied_lines = 0;
                                        ...
                                        authority_cert_t *cert)
{
  ...
  SMARTLIST_FOREACH_BEGIN(routers, routerinfo_t *, ri) {
    if (ri->cache_info.published_on >= cutoff) {
      ...
      vrs = (vote_routerstatus_t *)tor_malloc_zero(sizeof(vote_routerstatus_t));
      rs = &vrs->status;
  ...
  v3_out = (networkstatus_t *)tor_malloc_zero(sizeof(networkstatus_t));
...
lookup_cached_dir_by_fp(const char *fp)
{
  ...
  if (tor_digest_is_zero(fp) && cached_consensuses) {
    d = (cached_dir_t *)strmap_get(cached_consensuses, "ns");
  } else if (memchr(fp, '\0', DIGEST_LEN) && cached_consensuses &&
           (d = (cached_dir_t *)strmap_get(cached_consensuses, fp))) {
...
connection_dirserv_add_servers_to_outbuf(dir_connection_t *conn)
{
  ...
  while (smartlist_len(conn->fingerprint_stack) &&
    ...
    const char *body;
    char *fp = (char *)smartlist_pop_last(conn->fingerprint_stack);
...
connection_dirserv_add_microdescs_to_outbuf(dir_connection_t *conn)
{
  ...
  while (smartlist_len(conn->fingerprint_stack) &&
         connection_get_outbuf_len(TO_CONN(conn)) < DIRSERV_BUFFER_MIN) {
    char *fp256 = (char *)smartlist_pop_last(conn->fingerprint_stack);
