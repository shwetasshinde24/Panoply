...
state_transport_line_is_valid(const char *line)
{
  ...
  addrport = (char *)smartlist_get(items, 1);
                     ...
                     int from_setconf, char **msg)
{
  ...
  return or_state_validate((or_state_t *)state, msg);
}
...
or_state_new(void)
{
  or_state_t *new_state = (or_state_t *)tor_malloc_zero(sizeof(or_state_t));
...
or_state_load(void)
{
  ...
  printf("State fname is: %s \n", fname);
  switch (file_status(fname)) {
    case FN_FILE:
      if (!(contents = read_file_to_str(fname, 0, NULL))) {
         printf("Unable to read state file");
    ...
    case FN_DIR:
    default:
      printf("State file \"%s\" is not a file? Failing.", fname);
...
get_transport_in_state_by_name(const char *transport)
{
  ...
  for (line = or_state->TransportProxies ; line ; line = line->next) {
    ...
    if (!strcmp((const char *)smartlist_get(items, 0), transport)) {
      ret = line;
      goto done;
    }
