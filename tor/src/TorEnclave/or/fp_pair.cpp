...
struct fp_pair_map_impl { 
  struct fp_pair_map_entry_s **hth_table; 
...
fp_pair_map_new(void)
{
  ...
  result = (fp_pair_map_t *)tor_malloc(sizeof(fp_pair_map_t));
...
fp_pair_map_set(fp_pair_map_t *map, const fp_pair_t *key, void *val)
{
  ...
  if (resolve) {
    ...
    resolve = (fp_pair_map_entry_t *)tor_malloc_zero(sizeof(fp_pair_map_entry_t));
...
{
  fp_pair_map_entry_t **ent, **next, *_this;
  ...
  if (map) {
    for (ent = HT_START(fp_pair_map_impl, &(map->head));
      ...
      if (free_val) free_val(_this->val);
      tor_free(_this);
    }
