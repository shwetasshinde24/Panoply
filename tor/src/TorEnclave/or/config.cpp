...
#define SHARE_DATADIR "/home/kekkaishi/datadir"
#define LOCALSTATEDIR "/home/kekkaishi/localstatedir"
#define BINDDIR "/home/kekkaishi/binddir"

static inline void set_option_command(or_options_t *opt, int command)
{
    switch(command)  {
      case 0:
        opt->command = or_options_t::CMD_RUN_TOR;
        break;
      case 1:
        opt->command = or_options_t::CMD_LIST_FINGERPRINT;
        break;
      case 2:
        opt->command = or_options_t::CMD_HASH_PASSWORD;
        break;
      case 3:
        opt->command = or_options_t::CMD_VERIFY_CONFIG;
        break;
      case 4:
        opt->command = or_options_t::CMD_RUN_UNITTESTS;
        break;
      case 5:
        opt->command = or_options_t::CMD_DUMP_CONFIG;
        break;
      default:
        fprintf(stderr, "set_option_command: command: %d is invalid command \n", command);
    }
}
...
static int options_transition_allowed(const or_options_t *old, const or_options_t *_new, char **msg);
...
// Fix me: just hardcode now
// extern const char tor_git_revision[]; /* from tor_main.c */
const char* tor_git_revision = "tor-0.2.5.11";
...
{
  tor_assert(address);
  if (get_options()->SafeLogging_ == or_options_t::SAFELOG_SCRUB_ALL)
    return "[scrubbed]";
  else
    return address;
}
...
{
  tor_assert(address);
  if (get_options()->SafeLogging_ != or_options_t::SAFELOG_SCRUB_NONE)
    return "[scrubbed]";
  else
    return address;
}
                            ...
                            const or_options_t *old_options)
{
  ...
  if (!options->DirAuthorities) {
    ...
    if (!options->AlternateBridgeAuthority)
      type = (dirinfo_type_t)( type | BRIDGE_DIRINFO );
    if (!options->AlternateDirAuthority)
      type = (dirinfo_type_t)( type | V3_DIRINFO | EXTRAINFO_DIRINFO | MICRODESC_DIRINFO );
...
options_act_reversible(const or_options_t *old_options, char **msg)
{
  ...
  int running_tor = options->command == or_options_t::CMD_RUN_TOR;
  ...
  if (running_tor && options->RunAsDaemon) {
    ...
    // Fix me: start_daemon is not handled
    fprintf(stderr, "Fix me: call unimplemented start_daemon in options_act_reversible in or/config.cpp \n");
    // start_daemon();
    return -1;
  }
 ...
 commit:
  ...
  if (logs_marked) {
    log_severity_list_t *severity =
      (log_severity_list_t *)tor_malloc_zero(sizeof(log_severity_list_t));
    close_temp_logs();
...

  {
    ...
    if (options->SafeLogging_ != or_options_t::SAFELOG_SCRUB_ALL) {
      bad_safelog = 1;
...
options_act(const or_options_t *old_options)
{
  ...
  if (options->BridgePassword) {
    ...
    options->BridgePassword_AuthDigest_ = (char *)tor_malloc(DIGEST256_LEN);
                         ...
                         config_line_t **cmdline_result)
{
  ...
  while (i < argc) {
    ...
    param = (config_line_t *)tor_malloc_zero(sizeof(config_line_t));
...
{
  int r;
  or_options_t *trial_options = (or_options_t *)config_dup(&options_format, get_options());

  if ((r=config_assign(&options_format, trial_options,
                       list, use_defaults, clear_first, msg)) < 0) {
    config_free(&options_format, trial_options);
    return (setopt_err_t)r;
  }
  ...
  const or_options_t *old_options = get_options();
  if (options_transition_allowed(old_options, trial_options, msg) < 0) {
    config_free(&options_format, trial_options);
...
options_new(void)
{
  return (or_options_t *)config_new(&options_format);
}
  ...
  *auth = NO_DIRINFO;
  ...
  SMARTLIST_FOREACH_BEGIN(list, const char *, string) {
    if (!strcasecmp(string, "v1"))
      ...
      if (options->BridgeRelay)
        *auth = (dirinfo_type_t)(*auth | BRIDGE_DIRINFO);
      else
        *auth = (dirinfo_type_t)(*auth | V3_DIRINFO);
...
options_validate_cb(void *old_options, void *options, void *default_options,
                    int from_setconf, char **msg)
{
  return options_validate((or_options_t *)old_options, (or_options_t *)options, (or_options_t *)default_options,
                          from_setconf, msg);
}
...
#define COMPLAIN(arg) STMT_BEGIN log_warn(LD_CONFIG, arg); STMT_END
  ...
  *msg = NULL;
  ...
  options->TransProxyType_parsed = or_options_t::TPT_DEFAULT;
...
#else
      options->TransProxyType_parsed = or_options_t::TPT_PF_DIVERT;
#endif
    } else if (!strcasecmp(options->TransProxyType, "tproxy")) {
#ifndef __linux__
      REJECT("TPROXY is a Linux-specific feature.");
#else
      options->TransProxyType_parsed = or_options_t::TPT_TPROXY;
#endif
    } else if (!strcasecmp(options->TransProxyType, "ipfw")) {
#ifndef __FreeBSD__
      REJECT("ipfw is a FreeBSD-specific feature.");
#else
      options->TransProxyType_parsed = or_options_t::TPT_IPFW;
#endif
    } else {
  ...
  if (options->FascistFirewall && !options->ReachableAddresses) {
    if (options->FirewallPorts && smartlist_len(options->FirewallPorts)) {
      ...
      config_line_t *new_line = (config_line_t *)tor_malloc_zero(sizeof(config_line_t));
  ...
  for (i=0; i<3; i++) {
    ...
    for (;;) {
      ...
      linep = &((*linep)->next);
      if (!*linep) {
        *linep = (config_line_t *)tor_malloc_zero(sizeof(config_line_t));
  ...
  options->AllowInvalid_ = (invalid_router_usage_t)0;

  if (options->AllowInvalidNodes) {
    SMARTLIST_FOREACH_BEGIN(options->AllowInvalidNodes, const char *, cp) {
        if (!strcasecmp(cp, "entry"))
          options->AllowInvalid_ = (invalid_router_usage_t)(options->AllowInvalid_ | ALLOW_INVALID_ENTRY); 
        else if (!strcasecmp(cp, "exit"))
          options->AllowInvalid_ = (invalid_router_usage_t)(options->AllowInvalid_ | ALLOW_INVALID_EXIT); 
        else if (!strcasecmp(cp, "middle"))
          options->AllowInvalid_ = (invalid_router_usage_t)(options->AllowInvalid_ | ALLOW_INVALID_MIDDLE); 
        else if (!strcasecmp(cp, "introduction"))
          options->AllowInvalid_ = (invalid_router_usage_t)(options->AllowInvalid_ | ALLOW_INVALID_INTRODUCTION); 
        else if (!strcasecmp(cp, "rendezvous"))
          options->AllowInvalid_ = (invalid_router_usage_t)(options->AllowInvalid_ | ALLOW_INVALID_RENDEZVOUS);
        else {
          tor_asprintf(msg,
  ...
  if (!options->SafeLogging ||
    ...
    options->SafeLogging_ = or_options_t::SAFELOG_SCRUB_NONE;
  } else if (!strcasecmp(options->SafeLogging, "relay")) {
    options->SafeLogging_ = or_options_t::SAFELOG_SCRUB_RELAY;
  } else if (!strcasecmp(options->SafeLogging, "1")) {
    options->SafeLogging_ = or_options_t::SAFELOG_SCRUB_ALL;
  } else {
    tor_asprintf(msg,
...
#if SIZEOF_VOID_P >= 8
#define MAX_DEFAULT_MAXMEM (50*ONE_MEGABYTE)
#else
#define MAX_DEFAULT_MAXMEM (50*ONE_MEGABYTE)
#endif
...
options_init_from_torrc(int argc, char **argv)
{
  ...
  command = or_options_t::CMD_RUN_TOR;
  for (p_index = cmdline_only_options; p_index; p_index = p_index->next) {
    if (!strcmp(p_index->key,"--list-fingerprint")) {
      command = or_options_t::CMD_LIST_FINGERPRINT;
    } else if (!strcmp(p_index->key, "--hash-password")) {
      command = or_options_t::CMD_HASH_PASSWORD;
      command_arg = p_index->value;
    } else if (!strcmp(p_index->key, "--dump-config")) {
      command = or_options_t::CMD_DUMP_CONFIG;
      command_arg = p_index->value;
    } else if (!strcmp(p_index->key, "--verify-config")) {
      command = or_options_t::CMD_VERIFY_CONFIG;
    }
  }

  if (command == or_options_t::CMD_HASH_PASSWORD) {
                         ...
                         char **msg)
{
  ...
  newoptions = (or_options_t *)tor_malloc_zero(sizeof(or_options_t));
  newoptions->magic_ = OR_OPTIONS_MAGIC;
  options_init(newoptions);
  // set_option_command(newoptions, command);
  ...
  for (i = 0; i < 2; ++i) {
    ...
    if (retval < 0) {
      printf("Fail in config_get_lines 4286\n");
    ...
    if (retval < 0) {
      printf("Fail in config_assign 4295\n");
      err = SETOPT_ERR_PARSE;
      goto err;
    }
    if (i==0)
      newdefaultoptions = (or_options_t *)config_dup(&options_format, newoptions);
  }

  if (newdefaultoptions == NULL) {
    newdefaultoptions = (or_options_t *)config_dup(&options_format, global_default_options);
  }
  ...
  if (retval < 0) {
    printf("Fail in config_assign 4308\n");
    err = SETOPT_ERR_PARSE;
    goto err;
  }
  ...
  if (newoptions->TestingTorNetwork) {
    ...
    newoptions->magic_ = OR_OPTIONS_MAGIC;
    options_init(newoptions);
    // Fix me: need to cast int to the nested enum
    // set_option_command(newoptions, command);
    newoptions->command = command;
    newoptions->command_arg = command_arg ? tor_strdup(command_arg) : NULL;
    ...
    for (i = 0; i < 2; ++i) {
      ...
      retval = config_get_lines(body, &cl, 1);
      if (retval < 0) {
        printf("Fail in config_get_lines 4356\n");
        err = SETOPT_ERR_PARSE;
        goto err;
      }
      retval = config_assign(&options_format, newoptions, cl, 0, 0, msg);
      config_free_lines(cl);
      if (retval < 0) {
        printf("Fail in config_assign 4362\n");
        err = SETOPT_ERR_PARSE;
        goto err;
      }
      if (i==0)
    ...
    retval = config_assign(&options_format, newoptions,
                           global_cmdline_options, 0, 0, msg);
    if (retval < 0) {
      printf("Fail in config_assign 4373\n");
      err = SETOPT_ERR_PARSE;
      goto err;
    }
  }
...
config_register_addressmaps(const or_options_t *options)
{
  ...
  for (opt = options->AddressMap; opt; opt = opt->next) {
    ...
    from = (char *)smartlist_get(elts,0);
    to = (char *)smartlist_get(elts,1);
...
options_init_logs(or_options_t *options, int validate_only)
{
  ...
  for (opt = options->Logs; opt; opt = opt->next) {
    ...
    severity = (log_severity_list_t *)tor_malloc_zero(sizeof(log_severity_list_t));
    ...
    if (smartlist_len(elts) == 1 &&
        (!strcasecmp((const char *)smartlist_get(elts,0), "stdout") ||
         !strcasecmp((const char *)smartlist_get(elts,0), "stderr"))) {
    ...
    if (smartlist_len(elts) == 1 &&
        !strcasecmp((const char *)smartlist_get(elts,0), "syslog")) {
#ifdef HAVE_SYSLOG_H
      if (!validate_only) {
    ...
    if (smartlist_len(elts) == 2 &&
        !strcasecmp((const char *)smartlist_get(elts,0), "file")) {
      if (!validate_only) {
        char *fname = expand_filename((const char *)smartlist_get(elts, 1));
...
parse_bridge_line(const char *line)
{
  ...
  char *field=NULL;
  bridge_line_t *bridge_line = (bridge_line_t *)tor_malloc_zero(sizeof(bridge_line_t));
  ...
  field = (char *)smartlist_get(items, 0);
  ...
  if (string_is_C_identifier(field)) {
    ...
    addrport = (char *)smartlist_get(items, 0); /* Next field is addrport then. */
    smartlist_del_keeporder(items, 0);
  } else {
    addrport = field;
  }
                            ...
                            const char *line, int validate_only)
{
  ...
  /* Get the first line element, split it to commas into
     ...
     the transport names. */
  transports = (const char *)smartlist_get(items, 0);
  ...
  field2 = (char *)smartlist_get(items, 1);
  ...
  if (is_managed) { /* managed */
    ...
    if (!validate_only && !is_useless_proxy) {
      ...
      proxy_argv = (char **)tor_malloc_zero(sizeof(char*)*(proxy_argc+1));
      tmp = proxy_argv;
      for (i=0;i<proxy_argc;i++) { /* store arguments */
        *tmp++ = (char *)smartlist_get(items, 2);
    ...
    addrport = (char *)smartlist_get(items, 2);
    ...
    if (!validate_only) {
      transport_add_from_config(&addr, port, (char *)smartlist_get(transport_list, 0),
                                socks_ver);
...
get_bindaddr_from_transport_listen_line(const char *line,const char *transport)
{
  ...
  parsed_transport = (char *)smartlist_get(items, 0);
  addrport = tor_strdup((char *)smartlist_get(items, 1));
                            ...
                            const char *line, int validate_only)
{
  ...
  /* Get the first line element, split it to commas into
     transport_list (in case it's multiple transports) and validate
     the transport names. */
  transports = (char *)smartlist_get(items, 0);
  transport_list = smartlist_new();
  smartlist_split_string(transport_list, transports, ",",
                         SPLIT_SKIP_SPACE|SPLIT_IGNORE_BLANK, 0);
  SMARTLIST_FOREACH_BEGIN(transport_list, const char *, transport_name) {
  ...
  type = (char *)smartlist_get(items, 1);
                         ...
                         int validate_only)
{
  ...
  dirinfo_type_t type = (dirinfo_type_t)0;
  double weight = 1.0;
  ...
  if (is_legal_nickname((char *)smartlist_get(items, 0))) {
    nickname = (char *)smartlist_get(items, 0);
    smartlist_del_keeporder(items, 0);
  }

  while (smartlist_len(items)) {
    char *flag = (char *)smartlist_get(items, 0);
  ...
  addrport = (char *)smartlist_get(items, 0);
  smartlist_del_keeporder(items, 0);
  ...
  if (!validate_only && (!required_type || required_type & type)) {
    dir_server_t *ds;
    if (required_type)
      type = (dirinfo_type_t)(type & required_type); /* pare down what we think of them as an
                              * authority for. */
                        ...
                        int validate_only)
{
  ...
  if (tor_addr_port_split(LOG_INFO, (const char *)smartlist_get(positional, 0),
                          &address, &dirport) < 0 ||
...
port_cfg_new(void)
{
  port_cfg_t *cfg = (port_cfg_t *)tor_malloc_zero(sizeof(port_cfg_t));
  cfg->ipv4_traffic = 1;
                  ...
                  unsigned flags)
{
  ...
  for (; ports; ports = ports->next) {
    ...
    addrport = (char *)smartlist_get(elts, 0);
                         ...
                         int listener_type)
{
  ...
  for ( ; cfg; cfg = cfg->next) {
    ...
    port_cfg_t *port = (port_cfg_t *)tor_malloc_zero(sizeof(port_cfg_t) + len + 1);
    port->is_unix_addr = 1;
                           ...
                           uint8_t **cookie_out, int *cookie_is_set_out)
{
  ...
  char *cookie_file_str = (char *)tor_malloc(cookie_file_str_len);
  int retval = -1;
  ...
  /* Generate the cookie */
  *cookie_out = (uint8_t *)tor_malloc(cookie_len);
