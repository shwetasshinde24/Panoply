...
connection_edge_finished_connecting(edge_connection_t *edge_conn)
{
  ...
  IF_HAS_NO_BUFFEREVENT(conn)
    connection_watch_events(conn, (watchable_events_t)READ_EVENT); /* stop writing, keep reading */
...
#define WARN_INTRVL_PRIV 300
    ...
    ENTRY_TO_EDGE_CONN(conn)->rend_data = rend_data =
      (rend_data_t *)tor_malloc_zero(sizeof(rend_data_t));
                        ...
                        int use_begindir, int want_onehop)
{
  ...
  if (use_begindir) {
    conn->chosen_exit_name = (char *)tor_malloc(HEX_DIGEST_LEN+2);
                 ...
                 uint8_t *end_reason_out)
{
  ...
  *end_reason_out = END_STREAM_REASON_MISC;
  ...
  nul = (const uint8_t *)memchr(body, 0, rh.length);
  if (! nul) {
...
connection_exit_connect(edge_connection_t *edge_conn)
{
  ...
  switch (connection_connect(conn, conn->address, addr, port, &socket_error)) {
    ...
    case 0:
      ...
      connection_watch_events(conn, (watchable_events_t)(READ_EVENT | WRITE_EVENT));
  ...
  if (connection_get_outbuf_len(conn)) {
    ...
    IF_HAS_NO_BUFFEREVENT(conn)
      connection_watch_events(conn, (watchable_events_t)READ_EVENT);
  }
                                         ...
                                         int dry_run)
{
  ...
  if (!circ->isolation_values_set) {
    ...
    circ->socks_username = (char *) (sr->username ?
      tor_memdup(sr->username, sr->usernamelen) : NULL);
    circ->socks_password = (char *) (sr->password ?
