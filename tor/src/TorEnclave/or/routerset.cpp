...
routerset_new(void)
{
  routerset_t *result = (routerset_t *)tor_malloc_zero(sizeof(routerset_t));
 ...
 * equal routersets. */
int
routerset_equal(const routerset_t *old, const routerset_t *_new)
{
  if (routerset_is_empty(old) && routerset_is_empty(_new)) {
    /* Two empty sets are equal */
    return 1;
  } else if (routerset_is_empty(old) || routerset_is_empty(_new)) {
    /* An empty set is equal to nothing else. */
    return 0;
  }
  tor_assert(old != NULL);
  tor_assert(_new != NULL);

  if (smartlist_len(old->list) != smartlist_len(_new->list))
    return 0;

  SMARTLIST_FOREACH(old->list, const char *, cp1, {
    const char *cp2 = (const char *)smartlist_get(_new->list, cp1_sl_idx);
    if (strcmp(cp1, cp2))
      return 0;
  });
