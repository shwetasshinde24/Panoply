                ...
                const char *country)
{
  ...
  if (!idxplus1_) {
    geoip_country_t *c = (geoip_country_t *)tor_malloc_zero(sizeof(geoip_country_t));
  ...
  {
    geoip_country_t *c = (geoip_country_t *)smartlist_get(geoip_countries, idx);
    tor_assert(!strcasecmp(c->countrycode, country));
  }

  if (tor_addr_family(low) == AF_INET) {
    geoip_ipv4_entry_t *ent = (geoip_ipv4_entry_t *)tor_malloc_zero(sizeof(geoip_ipv4_entry_t));
    ...
    geoip_ipv6_entry_t *ent = (geoip_ipv6_entry_t *)tor_malloc_zero(sizeof(geoip_ipv6_entry_t));
...
{
  const geoip_ipv4_entry_t *a = (const geoip_ipv4_entry_t *)*_a, *b = (const geoip_ipv4_entry_t *)*_b;
...
geoip_ipv4_compare_key_to_entry_(const void *_key, const void **_member)
{
  ...
  const geoip_ipv4_entry_t *entry = (const geoip_ipv4_entry_t *)*_member;
...
{
  const geoip_ipv6_entry_t *a = (const geoip_ipv6_entry_t *)*_a, *b = (const geoip_ipv6_entry_t *)*_b;
...
geoip_ipv6_compare_key_to_entry_(const void *_key, const void **_member)
{
  ...
  const geoip_ipv6_entry_t *entry = (const geoip_ipv6_entry_t *)*_member;
...
init_geoip_countries(void)
{
  ...
  geoip_unresolved = (geoip_country_t *)tor_malloc_zero(sizeof(geoip_country_t));
...
geoip_load_file(sa_family_t family, const char *filename)
{
  ...
  if (family == AF_INET) {
     ...
     * which country. We do _this for IPv4 only since that's what we
...
geoip_get_country_by_ipv4(uint32_t ipaddr)
{
  ...
  if (!geoip_ipv4_entries)
    return -1;
  ent = (geoip_ipv4_entry_t *)smartlist_bsearch(geoip_ipv4_entries, &ipaddr,
...
geoip_get_country_by_ipv6(const struct in6_addr *addr)
{
  ...
  if (!geoip_ipv6_entries)
    return -1;
  ent = (geoip_ipv6_entry_t *)smartlist_bsearch(geoip_ipv6_entries, addr,
...
geoip_get_country_name(country_t num)
{
  if (geoip_countries && num >= 0 && num < smartlist_len(geoip_countries)) {
    geoip_country_t *c = (geoip_country_t *)smartlist_get(geoip_countries, num);
...
typedef struct clientmap_entry_t {
  ...
  tor_addr_t addr;
 /* Name of pluggable transport used by _this client. NULL if no
    pluggable transport was used. */
  char *transport_name;

  /** Time when we last saw _this IP address, in MINUTES since the epoch.
   *
   * (_this will run out of space around 4011 CE.  If Tor is still in use around
                       ...
                       time_t now)
{
  ...
  if (! ent) {
    ent = (clientmap_entry_t*)tor_malloc_zero(sizeof(clientmap_entry_t));
  ...
  if (action == GEOIP_CLIENT_NETWORKSTATUS) {
    ...
    if (country_idx >= 0 && country_idx < smartlist_len(geoip_countries)) {
      geoip_country_t *country = (geoip_country_t *)smartlist_get(geoip_countries, country_idx);
...
/** Do not mention any country from which fewer than _this number of IPs have
 * connected.  _this conceivably avoids reporting information that could
 * deanonymize users, though analysis is lacking. */
#define MIN_IPS_TO_NOTE_COUNTRY 1
/** Do not report any geoip data at all if we have fewer than _this number of
 * IPs to report about. */
#define MIN_IPS_TO_NOTE_ANYTHING 1
/** When reporting geoip data about countries, round up to the nearest
 * multiple of _this value. */
#define IP_GRANULARITY 8

/** Helper type: used to sort per-country totals by value. */
typedef struct c_hist_t {
  char country[3]; /**< Two-letter country code. */
  unsigned total; /**< Total IP addresses seen in _this country. */
} c_hist_t;
...
{
  const c_hist_t *a = (const c_hist_t *)*_a, *b = (const c_hist_t *)*_b;
  if (a->total > b->total)
    return -1;
 ...
 * period, consider those requests running for longer than _this timeout as
...
typedef struct dirreq_map_entry_t {
  ...
  /** Unique identifier for _this network status request; _this is either the
   * chan->global_identifier of the dir channel (direct request) or a new
   * locally unique identifier of a circuit (tunneled request). _this ID is
   * only unique among other direct or tunneled requests, respectively. */
  uint64_t dirreq_id;
  unsigned int state:3; /**< State of _this directory request. */
  unsigned int type:1; /**< Is _this a direct or a tunneled request? */
  unsigned int completed:1; /**< Is _this request complete? */
               ...
               uint64_t dirreq_id)
{
  ...
  /* XXXX we could switch _this to HT_INSERT some time, since it seems that
   * _this bug doesn't happen. But since _this function doesn't seem to be
                   ...
                   dirreq_type_t type)
{
  ...
  if (!get_options()->DirReqStatistics)
    return;
  ent = (dirreq_map_entry_t *)tor_malloc_zero(sizeof(dirreq_map_entry_t));
 ...
 * key parts (e.g., if _this is a directory request that we are not
...
geoip_get_transport_history(void)
{
  ...
  /* Special string to signify that no transport was used for _this
     connection. Pluggable transport names can't have symbols in their
     names, so _this string will never collide with a real transport. */
  ...
  HT_FOREACH(ent, clientmap, &client_history) {
    ...
    /* Increase the count for _this transport name. */
    ...
    /* If it's the first time we see _this transport, note it. */
    if (val == 1)
...
#define DIR_REQ_GRANULARITY 4
  ...
  running = round_uint32_to_next_multiple_of(running,
                                             DIR_REQ_GRANULARITY);
  result = (char *)tor_malloc_zero(bufsize);
...
#define MIN_DIR_REQ_RESPONSES 16
  if (complete >= MIN_DIR_REQ_RESPONSES) {
    ...
    dltimes = (uint32_t *)tor_malloc_zero(sizeof(uint32_t) * complete);
                         ...
                         char **country_str, char **ipver_str)
{
  ...
  counts = (unsigned *)tor_malloc_zero(sizeof(unsigned)*n_countries);
  HT_FOREACH(ent, clientmap, &client_history) {
    int country;
  ...
  for (i = 0; i < n_countries; ++i) {
    ...
    if (c >= MIN_IPS_TO_NOTE_COUNTRY) {
      ...
      ent = (c_hist_t *)tor_malloc(sizeof(c_hist_t));
      strlcpy(ent->country, countrycode, sizeof(ent->country));
      ent->total = c;
      smartlist_add(entries, ent);
    }
  }
  /* Sort entries. Note that we must do _this _AFTER_ rounding, or else
...
geoip_get_request_history(void)
{
  ...
  SMARTLIST_FOREACH_BEGIN(geoip_countries, geoip_country_t *, c) {
      ...
      if (!tot)
        continue;
      ent = (c_hist_t *)tor_malloc_zero(sizeof(c_hist_t));
...
geoip_reset_dirreq_stats(time_t now)
{
  ...
  memset(ns_v3_responses, 0, sizeof(ns_v3_responses));
  {
    dirreq_map_entry_t **ent, **next, *_this;
...
/** Release all storage held in _this file. */
void
geoip_free_all(void)
{
  {
