                             ...
                             const char *resource, int pds_flags)
{
  ...
  switch (dir_purpose) {
    case DIR_PURPOSE_FETCH_EXTRAINFO:
      type = (dirinfo_type_t)(EXTRAINFO_DIRINFO |
    ...
    case DIR_PURPOSE_FETCH_CONSENSUS:
      ...
      if (resource && !strcmp(resource,"microdesc"))
        type = (dirinfo_type_t)(type | MICRODESC_DIRINFO);
      break;
    case DIR_PURPOSE_FETCH_MICRODESC:
  ...
  if (dir_purpose == DIR_PURPOSE_FETCH_CONSENSUS) {
    ...
    if (flav != -1) {
      ...
      v = networkstatus_get_latest_consensus_by_flavor((consensus_flavor_t)flav);
      if (v)
  ...
  if (!anonymized_connection && !use_begindir) {
    ...
    IF_HAS_BUFFEREVENT(ENTRY_TO_CONN(linked_conn), {
      connection_watch_events(ENTRY_TO_CONN(linked_conn),
                              (watchable_events_t)(READ_EVENT|WRITE_EVENT));
...
directory_get_consensus_url(const char *resource)
{
...

  {
    ...
    SMARTLIST_FOREACH_BEGIN(router_get_trusted_dir_servers(),
        ...
        hex = (char *)tor_malloc(2*CONDITIONAL_CONSENSUS_FPR_LEN+1);
...
note_request(const char *key, size_t bytes)
{
  ...
  if (!r) {
    r = (request_t *)tor_malloc_zero(sizeof(request_t));
                             ...
                             const char *req_body, size_t req_body_len)
{
  ...
  if (!strcmpstart(url, "/tor/status-vote/current/consensus")) {
    ...
    if (1) {
      ...
      {
        char *fp = (char *)tor_malloc_zero(DIGEST_LEN);
        if (flavor)
...
{
  const fp_pair_t *fp1 = (const fp_pair_t *)*a, *fp2 = (const fp_pair_t *)*b;
  int r;
                                          ...
                                          smartlist_t *pairs_out)
{
  ...
  if (smartlist_len(pairs_tmp)) {
    char *last = (char *)smartlist_get(pairs_tmp,smartlist_len(pairs_tmp)-1);
                                     ...
                                     int flags)
{
  ...
  if (smartlist_len(fp_tmp)) {
    char *last = (char *)smartlist_get(fp_tmp,smartlist_len(fp_tmp)-1);
    size_t last_len = strlen(last);
    if (last_len > 2 && !strcmp(last+last_len-2, ".z")) {
      last[last_len-2] = '\0';
      if (compressed_out)
  ...
  if (decode_hex || decode_base64) {
    ...
    for (i = 0; i < smartlist_len(fp_tmp); ++i) {
      cp = (char *)smartlist_get(fp_tmp, i);
      ...
      d = (char *)tor_malloc_zero(digest_len);
      if (decode_hex ?
