                             ...
                             time_t now, int set)
{
  ...
  if (set) {
    time_t *oldptr;
    last_request_ptr = (time_t *)tor_malloc_zero(sizeof(time_t));
    *last_request_ptr = now;
    oldptr = (time_t *)strmap_set(last_hid_serv_requests, hsdir_desc_comb_id,
                        last_request_ptr);
    tor_free(oldptr);
  } else
    last_request_ptr = (time_t *)strmap_get_lc(last_hid_serv_requests,
                                     hsdir_desc_comb_id);
...
directory_get_from_hs_dir(const char *desc_id, const rend_data_t *rend_query)
{
  ...
  hs_dir = (routerstatus_t *)smartlist_choose(usable_responsible_dirs);
  if (! hs_dir && ! options->StrictNodes)
    hs_dir = (routerstatus_t *)smartlist_choose(responsible_dirs);
                                       ...
                                       unsigned int failure_type)
{
  ...
  for (i = 0; i < smartlist_len(ent->parsed->intro_nodes); i++) {
    rend_intro_point_t *intro = (rend_intro_point_t *)smartlist_get(ent->parsed->intro_nodes, i);
                                  ...
                                  const int warnings)
{
 ...
 again:
  ...
  intro = (rend_intro_point_t *)smartlist_get(usable_nodes, i);
...
rend_client_lookup_service_authorization(const char *onion_address)
{
  ...
  return (rend_service_authorization_t*)strmap_get(auth_hid_servs, onion_address);
}
...
{
  rend_service_authorization_free((rend_service_authorization_t *)service_auth);
}
                                 ...
                                 int validate_only)
{
  ...
  for (line = options->HidServAuth; line; line = line->next) {
    ...
    auth = (rend_service_authorization_t *)tor_malloc_zero(sizeof(rend_service_authorization_t));
    /* Parse onion address. */
    onion_address = (char *)smartlist_get(sl, 0);
    ...
    descriptor_cookie = (char *)smartlist_get(sl, 1);
