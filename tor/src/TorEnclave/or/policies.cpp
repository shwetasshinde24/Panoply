...
cmp_addr_policies(smartlist_t *a, smartlist_t *b)
{
  ...
  for (i = 0; i < len_a && i < len_b; ++i) {
    if ((r = cmp_single_addr_policy((addr_policy_t *)smartlist_get(a, i), (addr_policy_t *)smartlist_get(b, i))))
      return r;
  }
  if (i == len_a && i == len_b)
    return 0;
...
addr_policy_get_canonical_entry(addr_policy_t *e)
{
  ...
  if (!found) {
    found = (policy_map_ent_t *)tor_malloc_zero(sizeof(policy_map_ent_t));
    found->policy = (addr_policy_t *)tor_memdup(e, sizeof(addr_policy_t));
...
exit_policy_remove_redundancies(smartlist_t *dest)
{
   ...
   */
  {
    ...
    for (i = 0; i < smartlist_len(dest); ++i) {
      sa_family_t family;
      ap = (addr_policy_t *)smartlist_get(dest, i);
  ...
  for (i = 0; i < smartlist_len(dest)-1; ++i) {
    ...
    for (j = i+1; j < smartlist_len(dest); ++j) {
      tmp = (addr_policy_t *)smartlist_get(dest, j);
      tor_assert(j > i);
...
policy_summary_create(void)
{
  ...
  item = (policy_summary_item_t*)tor_malloc_zero(sizeof(policy_summary_item_t));
  item->prt_min = 1;
  item->prt_max = 65535;
  item->reject_count = 0;
  item->accepted = 0;
...
{
  policy_summary_item_t* _new;

  _new = (policy_summary_item_t*)tor_malloc_zero(sizeof(policy_summary_item_t));
  _new->prt_min = new_starts;
  _new->prt_max = old->prt_max;
  _new->reject_count = old->reject_count;
  _new->accepted = old->accepted;

  old->prt_max = new_starts-1;

  tor_assert(old->prt_min <= old->prt_max);
  tor_assert(_new->prt_min <= _new->prt_max);
  return _new;
}
...
parse_short_policy(const char *summary)
{
...

  {
    ...
    result = (short_policy_t *)tor_malloc_zero(size);
