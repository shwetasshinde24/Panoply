                   ...
                   int for_discovery, int for_directory)
{
  ...
  if (chosen) {
    ...
    rs = router_pick_directory_server((dirinfo_type_t)(MICRODESC_DIRINFO|V3_DIRINFO),
                                      PDS_FOR_GUARD);
    if (!rs)
      return NULL;
  ...
  entry = (entry_guard_t *)tor_malloc_zero(sizeof(entry_guard_t));
...
remove_obsolete_entry_guards(time_t now)
{
  ...
  for (i = 0; i < smartlist_len(entry_guards); ++i) {
    entry_guard_t *entry = (entry_guard_t *)smartlist_get(entry_guards, i);
...
remove_all_entry_guards(void)
{
  ...
  while (smartlist_len(entry_guards)) {
    entry_guard_t *entry = (entry_guard_t *)smartlist_get(entry_guards, 0);
    base16_encode(dbuf, sizeof(dbuf), entry->identity, DIGEST_LEN);
...
entry_guards_compute_status(const or_options_t *options, time_t now)
{
  ...
  if (changed) {
    SMARTLIST_FOREACH_BEGIN(entry_guards, entry_guard_t *, entry) {
      const char *reason = (const char *)digestmap_get(reasons, entry->identity);
      const char *live_msg = "";
...
{
  return choose_random_entry_impl(state, 0, (dirinfo_type_t)0, NULL);
}
                         ...
                         dirinfo_type_t dirinfo_type, int *n_options_out)
{
 ...
 choose_and_finish:
  if (entry_list_is_constrained(options)) {
    ...
    node = (const node_t*)smartlist_choose(live_entry_guards);
  }
  if (n_options_out)
  ...
  *msg = NULL;
  for (line = state->EntryGuards; line; line = line->next) {
    if (!strcasecmp(line->key, "EntryGuard")) {
      ...
      node = (entry_guard_t *)tor_malloc_zero(sizeof(entry_guard_t));
      ...
      } else if (!is_legal_nickname((const char *)smartlist_get(args,0))) {
        *msg = tor_strdup("Unable to parse entry nodes: "
                          "Bad nickname for EntryGuard");
      } else {
        strlcpy(node->nickname, (const char *)smartlist_get(args,0), MAX_NICKNAME_LEN+1);
        if (base16_decode(node->identity, DIGEST_LEN, (const char *)smartlist_get(args,1),
                          strlen((const char *)smartlist_get(args,1)))<0) {
          *msg = tor_strdup("Unable to parse entry nodes: "
                            "Bad hex digest for EntryGuard");
        }
      }
      if (smartlist_len(args) >= 3) {
        const char *is_cache = (const char *)smartlist_get(args, 2);
  ...
  SMARTLIST_FOREACH_BEGIN(new_entry_guards, entry_guard_t *, e) {
     char *sp;
     char *val = (char *)digestmap_get(added_by, e->identity);
...
entry_guards_update_state(or_state_t *state)
{
  ...
  *next = NULL;
  ...
  SMARTLIST_FOREACH_BEGIN(entry_guards, entry_guard_t *, e) {
      ...
      *next = line = (config_line_t *)tor_malloc_zero(sizeof(config_line_t));
      ...
      if (e->unreachable_since) {
        *next = line = (config_line_t *)tor_malloc_zero(sizeof(config_line_t));
        ...
        line->value = (char *)tor_malloc(ISO_TIME_LEN+1+ISO_TIME_LEN+1);
      ...
      if (e->bad_since) {
        *next = line = (config_line_t *)tor_malloc_zero(sizeof(config_line_t));
        ...
        line->value =(char *) tor_malloc(ISO_TIME_LEN+1);
...
bridge_add_from_config(bridge_line_t *bridge_line)
{
  ...
  b = (bridge_info_t *)tor_malloc_zero(sizeof(bridge_info_t));
...
num_bridges_usable(void)
{
  int n_options = 0;
  tor_assert(get_options()->UseBridges);
  (void) choose_random_entry_impl(NULL, 0, (dirinfo_type_t)0, &n_options);
  return n_options;
}
