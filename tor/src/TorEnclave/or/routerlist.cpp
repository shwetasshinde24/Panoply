...
download_status_reset_by_sk_in_cl(cert_list_t *cl, const char *digest)
{
  ...
  if (!dlstatus) {
    ...
    dlstatus = (download_status_t*)tor_malloc_zero(sizeof(*dlstatus));
...
get_cert_list(const char *id_digest)
{
  ...
  cl = (cert_list_t *)digestmap_get(trusted_dir_certs, id_digest);
  if (!cl) {
    cl = (cert_list_t*)tor_malloc_zero(sizeof(cert_list_t));
    ...
    SMARTLIST_FOREACH_BEGIN(status->voters, networkstatus_voter_info_t *,
      ...
      SMARTLIST_FOREACH_BEGIN(voter->sigs, document_signature_t *, sig) {
        ...
        if (download_status_is_ready_by_sk_in_cl(
          ...
          fp_tmp = (fp_pair_t*)tor_malloc(sizeof(*fp_tmp));
...
{
  const signed_descriptor_t *r1 = (const signed_descriptor_t *)*_a, *r2 = (const signed_descriptor_t *)*_b;
...
router_rebuild_store(int flags, desc_store_t *store)
{
  ...
  SMARTLIST_FOREACH_BEGIN(signed_descriptors, signed_descriptor_t *, sd) {
      ...
      c = (sized_chunk_t*)tor_malloc(sizeof(sized_chunk_t));
      c->bytes = body;
...
router_pick_directory_server_impl(dirinfo_type_t type, int flags)
{
 ...
 retry_without_exclude:
  ...
  if (smartlist_len(tunnel)) {
    ...
    result = (const node_t *)smartlist_choose(trusted_tunnel);
    ...
    result = (const node_t *)smartlist_choose(trusted_direct);
  }
...
dirserver_choose_by_weight(const smartlist_t *servers, double authority_weight)
{
  ...
  weights = (u64_dbl_t*)tor_malloc(sizeof(u64_dbl_t) * n);
  for (i = 0; i < n; ++i) {
    ds = (const dir_server_t *)smartlist_get(servers, i);
  ...
  tor_free(weights);
  return (i < 0) ? NULL : (const dir_server_t *)smartlist_get(servers, i);
}
                                           ...
                                           bandwidth_weight_rule_t rule)
{
...

  {
    int idx = choose_array_element_by_weight(bandwidths,
                                             smartlist_len(sl));
    tor_free(bandwidths);
    return idx < 0 ? NULL : (const node_t *)smartlist_get(sl, idx);
  }
}
                            ...
                            u64_dbl_t **bandwidths_out)
{
  ...
  bandwidths = (u64_dbl_t*)tor_malloc_zero(sizeof(u64_dbl_t)*smartlist_len(sl));
...
#define EPSILON .1
  ...
  if (total_exit_bw+total_nonexit_bw < EPSILON) {
    ...
    tor_free(exit_bits);
    tor_free(guard_bits);
    return (const node_t *)smartlist_choose(sl);
  }
...
{ /* XXXX MOVE */
  ...
  if (!choice && (need_uptime || need_capacity || need_guard)) {
    ...
    log_info(LD_CIRC,
             ...
             need_uptime?", stable":"",
             need_guard?", guard":"");
    flags = (router_crn_flags_t)(flags & (~ (CRN_NEED_UPTIME|CRN_NEED_CAPACITY|CRN_NEED_GUARD)));
...
router_get_routerlist(void)
{
  if (PREDICT_UNLIKELY(!routerlist)) {
    routerlist = (routerlist_t*)tor_malloc_zero(sizeof(routerlist_t));
...
signed_descriptor_from_routerinfo(routerinfo_t *ri)
{
  ...
  sd = (signed_descriptor_t*)tor_malloc_zero(sizeof(signed_descriptor_t));
...
routerlist_insert(routerlist_t *rl, routerinfo_t *ri)
{
  ...
  if (sd_old) {
    ...
    if (idx < smartlist_len(rl->old_routers)) {
       signed_descriptor_t *d = (signed_descriptor_t *)smartlist_get(rl->old_routers, idx);
...
routerlist_remove(routerlist_t *rl, routerinfo_t *ri, int make_old, time_t now)
{
  ...
  if (idx < smartlist_len(rl->routers)) {
    routerinfo_t *r = (routerinfo_t *)smartlist_get(rl->routers, idx);
...
{
//  printf("\n router_add_to_routerlist %p %p %d %d \n ", router, msg, from_cache, from_fetch);
...
{
  const struct duration_idx_t *d1 = (const struct duration_idx_t *)_d1;
  const struct duration_idx_t *d2 = (const struct duration_idx_t *)_d2;
                                             ...
                                             digestset_t *retain)
{
  ...
  for (i = lo+1; i <= hi; ++i) {
    signed_descriptor_t *r = (signed_descriptor_t *)smartlist_get(lst, i);
  ...
  lifespans = (struct duration_idx_t*)tor_malloc_zero(sizeof(struct duration_idx_t)*n);
  rmv = (uint8_t*)tor_malloc_zero(sizeof(uint8_t)*n);
  must_keep = (uint8_t*)tor_malloc_zero(sizeof(uint8_t)*n);
  ...
  for (i = lo; i <= hi; ++i) {
    ...
    if (i < hi) {
      r_next = (signed_descriptor_t *)smartlist_get(lst, i+1);
  ...
  do {
    if (rmv[i-lo])
      routerlist_remove_old(routerlist, (signed_descriptor_t *)smartlist_get(lst, i), i);
  } while (--i >= lo);
  tor_free(must_keep);
  tor_free(rmv);
  tor_free(lifespans);
  ...
  */
  ...
  if (consensus) {
    ...
    for (i = 0; i < smartlist_len(routerlist->routers); ++i) {
      router = (routerinfo_t *)smartlist_get(routerlist->routers, i);
  ...
  for (i = 0; i < smartlist_len(routerlist->old_routers); ++i) {
    sd = (signed_descriptor_t *)smartlist_get(routerlist->old_routers, i);
  ...
  /* Fix indices. */
  for (i = 0; i < smartlist_len(routerlist->old_routers); ++i) {
    signed_descriptor_t *r = (signed_descriptor_t *)smartlist_get(routerlist->old_routers, i);
               ...
               double weight)
{
  ...
  ent = (dir_server_t*)tor_malloc_zero(sizeof(dir_server_t));
                              ...
                              int lo, int hi, int pds_flags)
{
  ...
  cp = resource = (char*)tor_malloc(r_len);
  memcpy(cp, "d/", 2);
  cp += 2;
  for (i = lo; i < hi; ++i) {
    if (b64_256) {
      digest256_to_base64(cp, (const char *)smartlist_get(digests, i));
    } else {
      base16_encode(cp, r_len-(cp-resource),
                    (const char *)smartlist_get(digests,i), digest_len);
    }
    cp += enc_digest_len;
    *cp++ = sep;
  }
                                             ...
                                             networkstatus_t *consensus)
{
  ...
  if (is_vote) {
    ...
    dir_server_t *ds;
    networkstatus_voter_info_t *voter = (networkstatus_voter_info_t *)smartlist_get(consensus->voters, 0);
    tor_assert(voter);
  ...
  SMARTLIST_FOREACH_BEGIN(consensus->routerstatus_list, void *, rsp) {
      routerstatus_t *rs =
        is_vote ? &(((vote_routerstatus_t *)rsp)->status) : (routerstatus_t *)rsp;
      signed_descriptor_t *sd;
...
update_extrainfo_downloads(time_t now)
{
  ...
  for (old_routers = 0; old_routers < 2; ++old_routers) {
    ...
    for (i = 0; i < smartlist_len(lst); ++i) {
      ...
      char *d;
      if (old_routers)
        sd = (signed_descriptor_t *)smartlist_get(lst, i);
      else
...
router_differences_are_cosmetic(const routerinfo_t *r1, const routerinfo_t *r2)
{
  ...
  if (r1->declared_family && r2->declared_family) {
    ...
    for (i=0; i < n; ++i) {
      if (strcasecmp((const char *)smartlist_get(r1->declared_family, i),
                     (const char *)smartlist_get(r2->declared_family, i)))
        return 0;
    }
  }
                                     ...
                                     const char *id)
{
  ...
  i = start;
  do {
    routerstatus_t *r = (routerstatus_t *)smartlist_get(c->routerstatus_list, i);
    if (r->is_hs_dir) {
...
hid_serv_responsible_for_desc_id(const char *query)
{
  ...
  last_rs = (routerstatus_t *)smartlist_get(responsible, smartlist_len(responsible)-1);
