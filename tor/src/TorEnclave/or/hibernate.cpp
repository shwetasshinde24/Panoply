...
#include "router.h"
#include "statefile.h"
#include <sgx_stdlib_util.h>  // include dummy exit
...
accounting_parse_options(const or_options_t *options, int validate_only)
{
  ...
  s = (const char *)smartlist_get(items,0);
  ...
  switch (unit) {
  case UNIT_WEEK:
    d = tor_parse_long((const char *)smartlist_get(items,1), 10, 1, 7, &ok, NULL);
    if (!ok) {
    ...
    break;
  case UNIT_MONTH:
    d = tor_parse_long((const char *)smartlist_get(items,1), 10, 1, 28, &ok, NULL);
    if (!ok) {
  ...
  s = (const char *)smartlist_get(items, idx);
                          ...
                          const char **errmsg)
{
  ...
  if (!strcmp(question, "accounting/enabled")) {
    ...
    *answer = (char *)tor_malloc(ISO_TIME_LEN+1);
