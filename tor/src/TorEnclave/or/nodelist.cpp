...
typedef struct nodelist_t {
  ...
  struct nodelist_map nodes_by_id;
...
init_nodelist(void)
{
  if (PREDICT_UNLIKELY(the_nodelist == NULL)) {
    the_nodelist = (nodelist_t *)tor_malloc_zero(sizeof(nodelist_t));
...
node_get_or_create(const char *identity_digest)
{
  ...
  node = (node_t *)tor_malloc_zero(sizeof(node_t));
...
nodelist_drop_node(node_t *node, int remove_from_ht)
{
  ...
  tor_assert(node == (node_t *)smartlist_get(the_nodelist->nodes, idx));
  smartlist_del(the_nodelist->nodes, idx);
  if (idx < smartlist_len(the_nodelist->nodes)) {
    tmp = (node_t *)smartlist_get(the_nodelist->nodes, idx);
...
node_get_by_nickname(const char *nickname, int warn_if_unnamed)
{
  ...
  /* Okay, so the name is not canonical for anybody. */
  {
    ...
    if (smartlist_len(matches)>1 && warn_if_unnamed) {
      ...
      node_t *node = (node_t *)smartlist_get(matches, 0);
      if (node->name_lookup_warned) {
    ...
    if (smartlist_len(matches))
      choice = (const node_t *)smartlist_get(matches, 0);
...
node_get_all_orports(const node_t *node)
{
  ...
  if (node->ri != NULL) {
    if (node->ri->addr != 0) {
      tor_addr_port_t *ap = (tor_addr_port_t *)tor_malloc(sizeof(tor_addr_port_t));
