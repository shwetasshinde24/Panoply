...
ewma_alloc_cmux_data(circuitmux_t *cmux)
{
  ...
  pol = (ewma_policy_data_t *)tor_malloc_zero(sizeof(*pol));
                     ...
                     unsigned int cell_count)
{
  ...
  cdata = (ewma_policy_circ_data_t *)tor_malloc_zero(sizeof(*cdata));
                         ...
                         circuitmux_policy_data_t *pol_data)
{
  ...
  if (smartlist_len(pol->active_circuit_pqueue) > 0) {
    ...
    cell_ewma = (cell_ewma_t *)smartlist_get(pol->active_circuit_pqueue, 0);
...
{
  const cell_ewma_t *e1 = (const cell_ewma_t *)p1, *e2 = (const cell_ewma_t *)p2;
...
cell_ewma_to_circuit(cell_ewma_t *ewma)
{
  ...
  tor_assert(ewma);
  cdata = (ewma_policy_circ_data_t *)SUBTYPE_P(ewma, ewma_policy_circ_data_t, cell_ewma);
  tor_assert(cdata);
...
pop_first_cell_ewma(ewma_policy_data_t *pol)
{
  ...
  return (cell_ewma_t *)smartlist_pqueue_pop(pol->active_circuit_pqueue,
                              compare_cell_ewma_counts,
                              STRUCT_OFFSET(cell_ewma_t, heap_index));
}
