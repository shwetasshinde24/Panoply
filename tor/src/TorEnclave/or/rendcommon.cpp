...
rend_encode_v2_intro_points(char **encoded, rend_service_descriptor_t *desc)
{
  ...
  unenc = (char *)tor_malloc_zero(unenc_len);
  for (i = 0; i < smartlist_len(desc->intro_nodes); i++) {
    ...
    int res;
    rend_intro_point_t *intro = (rend_intro_point_t *)smartlist_get(desc->intro_nodes, i);
                                   ...
                                   smartlist_t *client_cookies)
{
  ...
  SMARTLIST_FOREACH_BEGIN(client_cookies, const char *, cookie) {
    client_part = (char *)tor_malloc_zero(REND_BASIC_AUTH_CLIENT_ENTRY_LEN);
    /* Encrypt session key. */
                                     ...
                                     const char *descriptor_cookie)
{
  ...
  enc = (char *)tor_malloc_zero(1 + CIPHER_IV_LEN + strlen(encoded));
                           ...
                           smartlist_t *client_cookies)
{
  ...
  if (auth_type == REND_STEALTH_AUTH) {
    descriptor_cookie = (char *)smartlist_get(client_cookies, 0);
    tor_assert(descriptor_cookie);
  }
  ...
  if (smartlist_len(desc->intro_nodes) > 0) {
    ...
    ipos_base64 = (char *)tor_malloc_zero(ipos_len * 2);
  ...
  for (k = 0; k < REND_NUMBER_OF_NON_CONSECUTIVE_REPLICAS; k++) {
    ...
    rend_encoded_v2_service_descriptor_t *enc =
      (rend_encoded_v2_service_descriptor_t *)tor_malloc_zero(sizeof(rend_encoded_v2_service_descriptor_t));
    ...
    desc_len = 2000 + smartlist_len(desc->intro_nodes) * 1000; /* far too long,
                                                                  but okay.*/
    enc->desc_str = desc_str = (char *)tor_malloc_zero(desc_len);
...
{
  rend_cache_entry_free((rend_cache_entry_t *)p);
}
...
rend_cache_store_v2_desc_as_dir(const char *desc)
{
  ...
  while (rend_parse_v2_service_descriptor(&parsed, desc_id, &intro_content,
    ...
    if (!e) {
      e = (rend_cache_entry_t*)tor_malloc_zero(sizeof(rend_cache_entry_t));
                                   ...
                                   const rend_data_t *rend_query)
{
  ...
  e->received = time(NULL);
  e->parsed = parsed;
  e->desc = (char *)tor_malloc_zero(encoded_size + 1);
...
{
  tor_assert(data);
  return (rend_data_t *)tor_memdup(data, sizeof(rend_data_t));
}
