...
#define VER_PRE tor_version_t::VER_PRE
#define VER_RC tor_version_t::VER_RC
#define VER_RELEASE tor_version_t::VER_RELEASE
...
dump_desc(const char *desc, const char *type)
{
  ...
  if (!last_desc_dumped || last_desc_dumped + 60 < now) {
    ...
    char *content = (char *)tor_malloc_zero(filelen);
...
#define BEGIN_END_OVERHEAD_LEN 64
  ...
  signature = (char *)tor_malloc(keysize);
                      ...
                      const char *doctype)
{
  ...
  signed_digest = (char *)tor_malloc(keysize);
                              ...
                              const char *prepend_annotations)
{
  ...
  while (1) {
    ...
    end = (const char *)tor_memstr(*s, eos-*s, "\nrouter-signature");
    if (end)
      end = (const char *)tor_memstr(end, eos-end, "\n-----END SIGNATURE-----\n");
    if (end)
      end += strlen("\n-----END SIGNATURE-----\n");
                               ...
                               const char *prepend_annotations)
{
  ...
  cp = (const char *)tor_memstr(s, end-s, "\nrouter ");
  if (!cp) {
  ...
  router = (routerinfo_t *)tor_malloc_zero(sizeof(routerinfo_t));
  ...
  if (cache_copy) {
    ...
    char *cp =
      router->cache_info.signed_descriptor_body = (char *)tor_malloc(len+1);
    if (prepend_annotations) {
  ...
  if ((tok = find_opt_by_keyword(tokens, K_ONION_KEY_NTOR))) {
    ...
    router->onion_curve25519_pkey =
      (curve25519_public_key_t *)tor_memdup(&k, sizeof(curve25519_public_key_t));
  }
                           ...
                           int cache_copy, struct digest_ri_map_t *routermap)
{
  ...
  tok = (directory_token_t *)smartlist_get(tokens,0);
  if (tok->tp != K_EXTRA_INFO) {
    log_warn(LD_DIR,"Entry does not start with \"extra-info\"");
    goto err;
  }

  extrainfo = (extrainfo_t *)tor_malloc_zero(sizeof(extrainfo_t));
  extrainfo->cache_info.is_extrainfo = 1;
  if (cache_copy)
    extrainfo->cache_info.signed_descriptor_body = (char *)tor_memdup_nulterm(s,end-s);
  ...
  if (routermap &&
      (router = (routerinfo_t *)digestmap_get((digestmap_t*)routermap,
  ...
  if (key) {
    ...
    extrainfo->pending_sig = (char *)tor_memdup(tok->object_body,
                                        tok->object_size);
...
#define MAX_CERT_SIZE (128*1024)
  ...
  cert = (authority_cert_t *)tor_malloc_zero(sizeof(authority_cert_t));
  ...
  tok = (directory_token_t *)smartlist_get(tokens, smartlist_len(tokens)-1);
  ...
  cert->cache_info.signed_descriptor_body = (char *)tor_malloc(len+1);
...
find_start_of_next_routerstatus(const char *s)
{
  ...
  footer = (const char *)tor_memstr(s, eos-s, "\ndirectory-footer");
  sig = (const char *)tor_memstr(s, eos-s, "\ndirectory-signature");
                                     ...
                                     consensus_flavor_t flav)
{
  ...
  if (vote_rs) {
    ...
    rs = (routerstatus_t *)tor_malloc_zero(sizeof(routerstatus_t));
  }
    ...
    SMARTLIST_FOREACH_BEGIN(tokens, directory_token_t *, t) {
      if (t->tp == K_M && t->n_args) {
        vote_microdesc_hash_t *line =
          (vote_microdesc_hash_t *) tor_malloc(sizeof(vote_microdesc_hash_t));
...
{
  const vote_routerstatus_t *a = (const vote_routerstatus_t *)*_a, *b = (const vote_routerstatus_t *)*_b;
                                     ...
                                     networkstatus_type_t ns_type)
{
  ...
  ns = (networkstatus_t *)tor_malloc_zero(sizeof(networkstatus_t));
  ...
  if (tok->n_args > 1) {
    ...
    ns->flavor = flav = (consensus_flavor_t)flavor;
  }
  ...
  SMARTLIST_FOREACH_BEGIN(tokens, directory_token_t *, _tok) {
    ...
    if (tok->tp == K_DIR_SOURCE) {
      ...
      voter = (networkstatus_voter_info_t *)tor_malloc_zero(sizeof(networkstatus_voter_info_t));
  ...
  if (ns->type != NS_TYPE_CONSENSUS &&
    ...
    int bad = 1;
    if (strlen(tok->args[0]) == HEX_DIGEST_LEN) {
      networkstatus_voter_info_t *voter = (networkstatus_voter_info_t *)smartlist_get(ns->voters, 0);
  ...
  while (!strcmpstart(s, "r ")) {
    if (ns->type != NS_TYPE_CONSENSUS) {
      vote_routerstatus_t *rs = (vote_routerstatus_t *)tor_malloc_zero(sizeof(vote_routerstatus_t));
      if (routerstatus_parse_entry_from_string(rs_area, &s, rs_tokens, ns,
                                               rs, 0, (consensus_flavor_t)0))
  ...
  for (i = 1; i < smartlist_len(ns->routerstatus_list); ++i) {
    ...
    if (ns->type != NS_TYPE_CONSENSUS) {
      vote_routerstatus_t *a = (vote_routerstatus_t *)smartlist_get(ns->routerstatus_list, i-1);
      vote_routerstatus_t *b = (vote_routerstatus_t *)smartlist_get(ns->routerstatus_list, i);
      rs1 = &a->status; rs2 = &b->status;
    } else {
      rs1 = (routerstatus_t *)smartlist_get(ns->routerstatus_list, i-1);
      rs2 = (routerstatus_t *)smartlist_get(ns->routerstatus_list, i);
    }
  ...
  if ((tok = find_opt_by_keyword(footer_tokens, K_DIRECTORY_FOOTER))) {
    if (tok != (directory_token_t *)smartlist_get(footer_tokens, 0)) {
  ...
  SMARTLIST_FOREACH_BEGIN(footer_tokens, directory_token_t *, _tok) {
    ...
    sig = (document_signature_t *)tor_malloc_zero(sizeof(document_signature_t));
    ...
    if (ns->type != NS_TYPE_CONSENSUS) {
      ...
      sig->signature = (char *)tor_memdup(tok->object_body, tok->object_size);
...
{
  digests_t *d = (digests_t *)strmap_get(sigs->digests, flavor_name);
  if (!d) {
    d = (digests_t *)tor_malloc_zero(sizeof(digests_t));
...
{
  smartlist_t *sl = (smartlist_t *)strmap_get(sigs->signatures, flavor_name);
  if (!sl) {
    sl = smartlist_new();
...
networkstatus_parse_detached_signatures(const char *s, const char *eos)
{
  ...
  ns_detached_signatures_t *sigs =
    (ns_detached_signatures_t *)tor_malloc_zero(sizeof(ns_detached_signatures_t));
...
#define RET_ERR(msg)                                               \
  STMT_BEGIN                                                       \
    ...
    tok = (directory_token_t *)ALLOC_ZERO(sizeof(directory_token_t));                   \
    tok->tp = ERR_;                                                \
...
#define MAX_ARGS 512
  ...
  tok->n_args = j;
  tok->args = (char **)memarea_memdup(area, args, j*sizeof(char*));
  return j;
#undef MAX_ARGS
}
...
#define MAX_LINE_LENGTH (128*1024)
    ...
    if (!strcmp_len(*s, table[i].t, next-*s)) {
      ...
      *s = eat_whitespace_eos_no_nl(next, eol);
      ...
      if (table[i].concat_args) {
        ...
        tok->args = (char **)ALLOC(sizeof(char*));
  ...
  *s = eol+1;    /* Set *s to possible start of object data (could be eos) */
  ...
  next = (const char *)tor_memstr(*s, eos-*s, "-----END ");
  if (!next) {
    RET_ERR("Malformed object: missing object end line");
  }
  tor_assert(eos >= next);
  eol = (const char*)memchr(next, '\n', eos-next);
  ...
  if (!strcmp(tok->object_type, "RSA PUBLIC KEY")) { /* If it's a public key */
    ...
    int r;
    tok->object_body = (char *)ALLOC(next-*s); /* really, this is too much RAM. */
                ...
                token_rule_t *table, int flags)
{
  ...
  if ((flags & TS_ANNOTATIONS_OK)) {
    ...
    for (i = 0; i < smartlist_len(out); ++i) {
      tok = (directory_token_t *)smartlist_get(out, i);
  ...
  for (i = 0; table[i].t; ++i) {
    ...
    if (table[i].pos & AT_START) {
      if (smartlist_len(out) < 1 ||
          (tok = (directory_token_t *)smartlist_get(out, first_nonannotation))->tp != table[i].v) {
    ...
    if (table[i].pos & AT_END) {
      if (smartlist_len(out) < 1 ||
          (tok = (directory_token_t *)smartlist_get(out, smartlist_len(out)-1))->tp != table[i].v) {
                            ...
                            const char **start_out, const char **end_out)
{
  ...
  start = (const char *)tor_memstr(s, s_len, start_str);
  if (!start) {
  ...
  end = (const char *)tor_memstr(start+strlen(start_str),
                   s_len - (start-s) - strlen(start_str), end_str);
  if (!end) {
    log_warn(LD_DIR,"couldn't find end of hashed material \"%s\"",end_str);
    return -1;
  }
  end = (const char*)memchr(end+strlen(end_str), end_c, s_len - (end-s) - strlen(end_str));
  if (!end) {
                             ...
                             saved_location_t where)
{
  ...
  while (s < eos) {
    ...
    md = (microdesc_t *)tor_malloc_zero(sizeof(microdesc_t));
    {
      const char *cp = (const char *)tor_memstr(s, start_of_next_microdesc-s,
                                  "onion-key");
      tor_assert(cp);
      ...
      if (copy_body)
        md->body = (char *)tor_memdup_nulterm(cp, md->bodylen);
      else
        md->body = (char*)cp;
      md->off = cp - start;
    }
                                 ...
                                 const char **next_out, const char *desc)
{
  rend_service_descriptor_t *result =
                            (rend_service_descriptor_t *)tor_malloc_zero(sizeof(rend_service_descriptor_t));
  ...
  *encoded_size_out = eos - desc;
  ...
  tor_assert(tok == (directory_token_t *)smartlist_get(tokens, 0));
  tor_assert(tok->n_args == 1);
  ...
  for (i = 0; i < smartlist_len(versions); i++) {
    version = (int) tor_parse_long((const char *)smartlist_get(versions, i),
  ...
  if (tok) {
    ...
    *intro_points_encrypted_out = (char *)tor_memdup(tok->object_body,
                                             tok->object_size);
                                 ...
                                 size_t ipos_encrypted_size)
{
  ...
  if (ipos_encrypted[0] == (int)REND_BASIC_AUTH) {
    ...
    dec = (char *)tor_malloc_zero(ipos_encrypted_size - CIPHER_IV_LEN - 1);
                               ...
                               size_t intro_points_encoded_size)
{
  ...
  while (!fast_memcmpstart(current_ipo, end_of_intro_points-current_ipo,
                      "introduction-point ")) {
    /* Determine end of string. */
    const char *eos = (const char *)tor_memstr(current_ipo, end_of_intro_points-current_ipo,
                                 "\nintroduction-point ");
    if (!eos)
    ...
    intro = (rend_intro_point_t *)tor_malloc_zero(sizeof(rend_intro_point_t));
    info = intro->extend_info = (extend_info_t *)tor_malloc_zero(sizeof(extend_info_t));
    /* Parse identifier. */
...
rend_parse_client_keys(strmap_t *parsed_clients, const char *ckstr)
{
  ...
  while (!strcmpstart(current_entry, "client-name ")) {
    ...
    parsed_entry = (rend_authorized_client_t *)tor_malloc_zero(sizeof(rend_authorized_client_t));
