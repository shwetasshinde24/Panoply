...
{
  rend_authorized_client_free((rend_authorized_client_t *)authorized_client);
}
...
rend_add_service(rend_service_t *service)
{
  ...
  if (!smartlist_len(service->ports)) {
    ...
    for (i = 0; i < smartlist_len(service->ports); ++i) {
      p = (rend_service_port_config_t *)smartlist_get(service->ports, i);
...
parse_port_config(const char *string)
{
  ...
  virtport = (int)tor_parse_long((const char *)smartlist_get(sl,0), 10, 1, 65535, NULL,NULL);
  if (!virtport) {
    log_warn(LD_CONFIG, "Missing or invalid port %s in hidden service port "
             "configuration", escaped((const char *)smartlist_get(sl,0)));
    goto err;
  }
  ...
  if (smartlist_len(sl) == 1) {
    ...
    addrport = (const char *)smartlist_get(sl,1);
  ...
  result = (rend_service_port_config_t *)tor_malloc(sizeof(rend_service_port_config_t));
...
rend_config_services(const or_options_t *options, int validate_only)
{
  ...
  for (line = options->RendConfigLines; line; line = line->next) {
    if (!strcasecmp(line->key, "HiddenServiceDir")) {
      ...
      service = (rend_service_t *)tor_malloc_zero(sizeof(rend_service_t));
    ...
    if (!strcasecmp(line->key, "HiddenServicePort")) {
      ...
      authname = (char *)smartlist_get(type_names_split, 0);
      ...
      smartlist_split_string(clients,(const char*)smartlist_get(type_names_split, 1),
                             ",", SPLIT_SKIP_SPACE, 0);
      SMARTLIST_FOREACH(type_names_split, char *, cp, tor_free(cp));
      smartlist_free(type_names_split);
      ...
      SMARTLIST_FOREACH_BEGIN(clients, const char *, client_name)
      {
        ...
        client = (rend_authorized_client_t *)tor_malloc_zero(sizeof(rend_authorized_client_t));
  ...
  if (old_service_list && !validate_only) {
     ...
     * probably ok? */
    SMARTLIST_FOREACH_BEGIN(rend_service_list, rend_service_t *, _new) {
      SMARTLIST_FOREACH_BEGIN(old_service_list, rend_service_t *, old) {
        if (!strcmp(old->directory, _new->directory)) {
          smartlist_add_all(_new->intro_nodes, old->intro_nodes);
          smartlist_clear(old->intro_nodes);
          smartlist_add(surviving_services, old);
          break;
        }
      } SMARTLIST_FOREACH_END(old);
    } SMARTLIST_FOREACH_END(_new);
...
rend_service_update_descriptor(rend_service_t *service)
{
  ...
  d = service->desc = (rend_service_descriptor_t *)tor_malloc_zero(sizeof(rend_service_descriptor_t));
  ...
  for (i = 0; i < smartlist_len(service->intro_nodes); ++i) {
    rend_intro_point_t *intro_svc = (rend_intro_point_t *)smartlist_get(service->intro_nodes, i);
     ...
     * our descriptor. */
    intro_desc = (rend_intro_point_t *)tor_malloc_zero(sizeof(rend_intro_point_t));
...
rend_service_load_auth_keys(rend_service_t *s, const char *hfname)
{
  ...
  SMARTLIST_FOREACH_BEGIN(s->clients, rend_authorized_client_t *, client) {
    rend_authorized_client_t *parsed = (rend_authorized_client_t *)
                       ...
                       size_t request_len)
{
  ...
  launched->rend_data = (rend_data_t *)tor_malloc_zero(sizeof(rend_data_t));
  ...
  launched->build_state->service_pending_final_cpath_ref =
    (crypt_path_reference_t *)tor_malloc_zero(sizeof(crypt_path_reference_t));
  ...
  launched->build_state->service_pending_final_cpath_ref->cpath = cpath =
    (crypt_path_t *)tor_malloc_zero(sizeof(crypt_path_t));
                               ...
                               char **err_msg_out)
{
  ...
  rv = (rend_intro_cell_t *)tor_malloc_zero(sizeof(*rv));
  ...
  rv->ciphertext = (uint8_t*)tor_malloc(request_len - DIGEST_LEN);
    ...
    char **err_msg_out)
{
  ...
  endptr = (const char*)memchr(rp_nickname, 0, nickname_field_len);
    ...
    char **err_msg_out)
{
  ...
  extend_info = (extend_info_t *)tor_malloc_zero(sizeof(extend_info_t));
    ...
    char **err_msg_out)
{
  ...
  if (intro->u.v3.auth_type != REND_NO_AUTH &&
    ...
    intro->u.v3.auth_data = (uint8_t*)tor_malloc(intro->u.v3.auth_len);
    /*
    ...
    char **err_msg_out)
{
  ...
  intro->plaintext = (uint8_t*)tor_malloc(intro->plaintext_len);
                         ...
                         int seconds_valid)
{
  ...
  for (i = 0; i < smartlist_len(descs); i++) {
    rend_encoded_v2_service_descriptor_t *desc = (rend_encoded_v2_service_descriptor_t *)smartlist_get(descs, i);
    ...
    for (j = 0; j < smartlist_len(responsible_dirs); j++) {
      ...
      char *hs_dir_ip;
      const node_t *node;
      hs_dir = (routerstatus_t *)smartlist_get(responsible_dirs, j);
  ...
  if (!failed_upload) {
    ...
    SMARTLIST_FOREACH(successful_uploads, const char *, c, {
      if (!smartlist_contains_digest(renddesc->successful_uploads, c)) {
        char *hsdir_id = (char *)tor_memdup(c, DIGEST_LEN);
      ...
      for (j = 0; j < num_descs; j++) {
        ...
        switch (service->auth_type) {
          ...
          case REND_STEALTH_AUTH:
            client = (rend_authorized_client_t *)smartlist_get(service->clients, j);
        ...
        for (i = 0; i < smartlist_len(descs); i++)
          rend_encoded_v2_service_descriptor_free((rend_encoded_v2_service_descriptor_t *)smartlist_get(descs, i));
...
rend_services_introduce(void)
{
  ...
  for (i=0; i < smartlist_len(rend_service_list); ++i) {
    ...
    service = (rend_service_t *)smartlist_get(rend_service_list, i);
    ...
    for (j = (int)n_intro_points_unexpired;
         ...
         ++j) { /* XXXX remove casts */
      router_crn_flags_t flags = (router_crn_flags_t)(CRN_NEED_UPTIME|CRN_NEED_DESC);
      if (get_options()->AllowInvalid_ & ALLOW_INVALID_INTRODUCTION)
        flags = (router_crn_flags_t)(flags | CRN_ALLOW_INVALID);
    ...
    for (j=prev_intro_nodes; j < smartlist_len(service->intro_nodes); ++j) {
      intro = (rend_intro_point_t *)smartlist_get(service->intro_nodes, j);
                                      ...
                                      origin_circuit_t *circ)
{
  ...
  chosen_port = (rend_service_port_config_t *)smartlist_choose(matching_ports);
