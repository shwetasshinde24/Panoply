...
connection_cpu_process_inbuf(connection_t *conn)
{
  ...
  if (conn->state == CPUWORKER_STATE_BUSY_ONION) {
    ...
    connection_fetch_from_buf((char *)&rpl,sizeof(cpuworker_reply_t),conn);
...
cpuworker_main(void *data)
{
  ...
  tor_socket_t *fdarray = (tor_socket_t *)data;
  tor_socket_t fd;
  ...
  for (;;) {
    if (read_all(fd, (char *)&req, sizeof(req), 1) != sizeof(req)) {
    ...
    if (req.task == CPUWORKER_TASK_ONION) {
      ...
      if (write_all(fd, (const char*)&rpl, sizeof(rpl), 1) != sizeof(rpl)) {
...
spawn_cpuworker(void)
{
  ...
  fdarray = (tor_socket_t *)tor_malloc(sizeof(tor_socket_t)*2);
...
#define CULL_CPUWORKERS_INTERVAL 60
  ...
  if (1) {
    ...
    connection_write_to_buf((const char*)&req, sizeof(req), cpuworker);
    memwipe(&req, 0, sizeof(req));
  }
  return 0;
}
