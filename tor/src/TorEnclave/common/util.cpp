...
tor_strndup_(const char *s, size_t n DMALLOC_PARAMS)
{
  ...
  dup = (char *)tor_malloc_((n+1) DMALLOC_FN_ARGS);
...
tor_memdup_(const void *mem, size_t len DMALLOC_PARAMS)
{
  ...
  tor_assert(mem);
  dup = (char *)tor_malloc_(len DMALLOC_FN_ARGS);
  memcpy(dup, mem, len);
  return dup;
}
...
tor_memdup_nulterm_(const void *mem, size_t len DMALLOC_PARAMS)
{
  ...
  tor_assert(mem);
  dup = (char *)tor_malloc_(len+1 DMALLOC_FN_ARGS);
  memcpy(dup, mem, len);
  dup[len] = '\0';
  return dup;
}
...
esc_for_log(const char *s)
{
  ...
  result = outp = (char *)tor_malloc(len);
  *outp++ = '\"';
  for (cp = s; *cp; ++cp) {
    switch (*cp) {
      case '\\':
...
tor_escape_str_for_pt_args(const char *string, const char *chars_to_escape)
{
  ...
  new_string = new_cp = (char *)tor_malloc(new_length);
...
write_all(tor_socket_t fd, const char *buf, size_t count, int isSocket)
{
  ...
  while (written != count) {
    if (isSocket)
      ...
      {
        const char *bufdup = (const char *)malloc(count);
        memcpy(bufdup, buf, count);
        result = write((int)fd, bufdup+written, count-written);
      } else {
...
file_status(const char *fname)
{
  ...
  tor_free(f);
  if (r) {
    // Fix me: errno is not accessible, always return ENOENT
    // if (errno == ENOENT) {
    //   return FN_NOENT;
    // }
    // return FN_ERROR;
    log_debug(LD_FS, "Fix me: always return FN_NOENT if cannot stat", f);
    return FN_NOENT;
  }
                  ...
                  const char *effective_user)
{
  ...
  if (r) {
    // if (errno != ENOENT) {
    //   printf("Directory %s cannot be read: %s", dirname,
    //            strerror(errno));
    ...
    if (check & CPD_CREATE) {
      ...
      printf("%d \n", sgx_is_within_enclave(dirname, strlen(dirname)));
  ...
  if (st.st_mode & mask) {
    ...
    // Fix me: memory error - dirname is not inside enclave ???
    const char* dirnamedup = strdup(dirname);
    if (chmod(dirnamedup, new_mode)) {
      log_warn(LD_FS, "Could not chmod directory %s: %s", dirnamedup,
          strerror(errno));
      return -1;
    } else {
      return 0;
...
struct open_file_t {
  ...
  SGX_WRAPPER_FILE stdio_file; /**< stdio wrapper for <b>fd</b>. */
};
...
{
  open_file_t *new_file = (open_file_t *)tor_malloc_zero(sizeof(open_file_t));
  ...
  *data_out = NULL;
  ...
  // Fix me: Memory error in free dynamic memory
...
write_chunks_to_file_impl(const char *fname, const smartlist_t *chunks,
                          int open_flags)
{
//  printf("Call write_chunks_to_file_impl \n");
...
//    printf("After assert ");
 ...
 err:
//  printf("err write_chunks_to_file_impl");
...
write_bytes_to_file_impl(const char *fname, const char *str, size_t len,
                         int flags)
{
//  printf("Call write_bytes_to_file_impl \n");
  int r;
...
{
  printf("read_file_to_str_until_eof \n");
  ssize_t r;
  size_t pos = 0;
  char *string = NULL;
  size_t string_max = 0;
  ...
  do {
    ...
    string = (char *)tor_realloc(string, string_max);
...
read_file_to_str(const char *filename, int flags, struct stat *stat_out)
{
  ...
  if (fd<0) {
    ...
    // Fix me: hardcode errno to ENOENT
    errno = ENOENT;
...
#define FIFO_READ_MAX (1024*1024)
  ...
  string = (char *)tor_malloc((size_t)(statbuf.st_size+1));
...
unescape_string(const char *s, char **result, size_t *size_out)
{
 ...
 end_of_loop:
  out = *result = (char *)tor_malloc(cp-s + 1);
  cp = s+1;
  while (1) {
    switch (*cp)
      {
...
format_win_cmdline_argument(const char *arg)
{
  ...
  formatted_arg = (char *)tor_malloc(sizeof(char) *
...
{
  process_handle_t *out = (process_handle_t *)tor_malloc_zero(sizeof(process_handle_t));
...
{
  process_handle_t *process_handle = (process_handle_t *)arg;
...
//     retval = dup2(stdout_pipe[1], fileno(stdout));
//     if (-1 == retval)
//         goto error;
...
//     retval = dup2(stderr_pipe[1], fileno(stderr));
//     if (-1 == retval)
//         goto error;
...
//      * this needless. */
//     for (fd = fileno(stderr) + 1; fd < max_fd; fd++) {
//       close(fd);
//     }
...
//         nbytes = write(fileno(stdout), error_message, error_message_length);
//         nbytes = write(fileno(stdout), hex_errno, n);
//       }
//     }
                            ...
                            int also_terminate_process))
{
  ...
  if (also_terminate_process) {
    // Fix me:
    fprintf(stderr, "Fix me: Call dummy tor_terminate_process in tor_process_handle_destroy in util.cpp \n");
...
{
  process_environment_t *env = (process_environment_t *)tor_malloc_zero(sizeof(process_environment_t));
  ...
  env->unixoid_environment_block = (char **)tor_calloc(n_env_vars + 1, sizeof(char *));
  ...
  for (i = 0; i < n_env_vars; ++i) {
    const char *s = (const char *)smartlist_get(env_vars, i);
  ...
  env->windows_environment_block = (char *)tor_malloc_zero(total_env_length);
  ...
  /* Now copy the strings into the environment blocks. */
  {
    ...
    for (i = 0; i < n_env_vars; ++i) {
      const char *s = (const char *)smartlist_get(env_vars_sorted, i);
      size_t slen = strlen(s);
...
ssize_t
tor_read_all_handle(SGX_WRAPPER_FILE h, char *buf, size_t count,
                    const process_handle_t *process,
                    int *eof)
{
  size_t numread = 0;
 ...
 *  <b>stream_status_out</b> appropriately. */
MOCK_IMPL(smartlist_t *,
tor_get_lines_from_handle, (SGX_WRAPPER_FILE handle,
                            enum stream_status *stream_status_out))
{
 ...
 */
static int
log_from_pipe(SGX_WRAPPER_FILE stream, int severity, const char *executable,
              int *child_status)
{
  char buf[256];
 ...
 *  in <b>buf_out</b>. */
enum stream_status
get_string_from_pipe(SGX_WRAPPER_FILE stream, char *buf_out, size_t count)
{
  char *retval;
  size_t len;
...
handle_fw_helper_line(const char *executable, const char *line)
{
  ...
  if (strcmp((const char *)smartlist_get(tokens, 0), "tor-fw-helper") ||
      strcmp((const char *)smartlist_get(tokens, 1), "tcp-forward"))
    goto err;

  external_port = (const char *)smartlist_get(tokens, 2);
  internal_port = (const char *)smartlist_get(tokens, 3);
  result = (const char *)smartlist_get(tokens, 4);
...
#define TIME_TO_EXEC_FWHELPER_FAIL 60
  ...
  if ((!child_handle || child_handle->status != PROCESS_STATUS_RUNNING) &&
    ...
    argv = (const char **)tor_malloc_zero(sizeof(char*)*args_n);
    ...
    fprintf(stderr, "Fix me: Call not support tor_spawn_background in tor_check_port_forwarding in util.cpp \n");
