...
#include <sgx/sys/param.h> /* FreeBSD needs this to know what version it is */
#endif
// #include <stdio.h>
...
{
  // printf("Call abc_open_cloexec %s \n", path);
  int fd = -1;

#ifdef O_CLOEXEC
  path = sandbox_intern_string(path);
  const char* pathdup = strdup(path);
  fd = open(pathdup, flags|O_CLOEXEC, mode);
  if (fd >= 0)
    return fd;
...
{
  SGX_WRAPPER_FILE result = fopen(path, mode);
#ifdef FD_CLOEXEC
  if (result != NULL) {
...
{
  // Fix me: rename is not around
  // log_debug(LD_FS, "Renaming %s to %s", path_old, path_new);
  const char* path_old_dup = strdup(sandbox_intern_string(path_old));
  const char* path_new_dup = strdup(sandbox_intern_string(path_new));
  // fprintf(stderr, "Fix me: Call dummy xl_tor_rename %s, - , %s \n", path_old_dup, path_new_dup);
  int retval = -1;
  int status = ocall_rename(&retval, path_old_dup, path_new_dup);  
  // printf("Done !!! %x \n", status);
  return retval;
}
...
tor_mmap_file(const char *filename)
{
  ...
  string = (char *)mmap(0, size, PROT_READ, MAP_PRIVATE, fd, 0);
  close(fd);
  ...
  res = (tor_mmap_t*)tor_malloc_zero(sizeof(tor_mmap_t));
  res->data = string;
  res->size = filesize;
...
tor_mmap_file(const char *filename)
{
  ...
  tor_mmap_t *res = (tor_mmap_t*)tor_malloc_zero(sizeof(tor_mmap_t));
  int empty = 0;
...
tor_mmap_file(const char *filename)
{
  ...
  if (! res)
    return NULL;
  handle = (tor_mmap_t*)tor_malloc_zero(sizeof(tor_mmap_t));
  handle->data = res;
           ...
           const void *_needle, size_t nlen)
{
  ...
  while ((p = (const char*)memchr(p, first, last_possible_start + 1 - p))) {
...
{
  printf("Fix me: dummy touch file is called \n");
...
tor_lockfile_lock(const char *filename, int blocking, int *locked_out)
{
  ...
  *locked_out = 0;
  ...
  result = (tor_lockfile_t *)tor_malloc(sizeof(tor_lockfile_t));
...
// Fix me: Change from nonblocking to all blockcing socket
tor_socket_t
...
tor_accept_socket_nonblocking(tor_socket_t sockfd, struct sockaddr *addr,
                              socklen_t *len)
{
  // Fix me: Change accept socket to blocking
  return tor_accept_socket_with_extensions(sockfd, addr, len, 1, 0);
}
...
set_max_file_descriptors(rlim_t limit, int *max_out)
{
  ...
  const unsigned long MAX_CONNECTIONS = 100;
#elif defined(_WIN32)
...
{
	// Fix me: log_credential_status is disabled for compiling
	fprintf(stderr, "Call dummy log_credential_status \n");
...
{
  struct passwd *new_pw = (struct passwd *)tor_malloc_zero(sizeof(struct passwd));
  if (pw->pw_name)
...
get_environment(void)
{
  ...
  // return environ;
#endif
   // Fix me: get environment is unclear
   fprintf(stderr, "Fix me: Call dummy get_environment \n");
   return NULL;
}
...
{
  if (af == AF_INET) {
    if (tor_inet_ntoa((const struct in_addr*)src, dst, len) < 0)
      return NULL;
    else
      return dst;
  } else if (af == AF_INET6) {
...
{
  if (af == AF_INET) {
    return tor_inet_aton(src, (struct in_addr *)dst);
  } else if (af == AF_INET6) {
...
tor_pthread_helper_fn(void *_data)
{
  ...
  sigfillset(&sigs);
  // Fix me: pthread_sigmask is not handled yet
 ...
 */
static SGX_WRAPPER_PTHREAD_ATTRIBUTE attr_detached;
...
spawn_func(void (*func)(void *), void *data)
{
  ...
  if (PREDICT_UNLIKELY(!threads_initialized))
    tor_threads_init();
  d = (tor_pthread_data_t *)tor_malloc(sizeof(tor_pthread_data_t));
  d->data = data;
  d->func = func;
  ...
  fprintf(stderr, "Fork() is strictly forbiden in SGX \n");
  abort();
...
tor_mutex_init(tor_mutex_t *mutex)
{
  ...
  // Manually set mutex lock to recursive type
  mutex->mutex.m_control = SGX_THREAD_MUTEX_RECURSIVE;
...
tor_mutex_new(void)
{
  tor_mutex_t *m = (tor_mutex_t *)tor_malloc_zero(sizeof(tor_mutex_t));
  tor_mutex_init(m);
  return m;
}
...
tor_threads_init(void)
{
  if (!threads_initialized) {
  	// Mutex attribute is not supported in SGX
...
tor_set_max_memlock(void)
{
  ...
  //   return -1;
  // }
  fprintf(stderr, "Fix me: Call dummy tor_set_max_memlock \n");
  return -1;
}
#endif
...
get_total_system_memory_impl(void)
{
  ...
  // Fix me: System sscanf is not supported
  // if (sscanf(cp, "MemTotal: %llu kB\n", &result) != 1)
  //   goto err;
  if (tor_sscanf(cp, "MemTotal: %llu kB\n", &result) != 1)
    goto err;  
