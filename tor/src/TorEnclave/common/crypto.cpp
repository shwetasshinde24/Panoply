...
crypto_new_pk_from_rsa_(RSA *rsa)
{
  crypto_pk_t *env;
  tor_assert(rsa);
  env = (crypto_pk_t *)tor_malloc(sizeof(crypto_pk_t));
  env->refs = 1;
  env->key = rsa;
  return env;
}
...
EVP_PKEY *
crypto_pk_get_evp_pkey_(crypto_pk_t *env, int isprivate)
{
  RSA *key = NULL;
  EVP_PKEY *pkey = NULL;
  tor_assert(env->key);
...
crypto_cipher_new_with_iv(const char *key, const char *iv)
{
  ...
  env = (crypto_cipher_t *)tor_malloc_zero(sizeof(crypto_cipher_t));
                                   ...
                                   size_t *len, int is_public)
{
  ...
  *dest = (char *)tor_malloc(buf->length+1);
...
crypto_pk_asn1_decode(const char *str, size_t len)
{
  ...
  cp = buf = (unsigned char *)tor_malloc(len);
  memcpy(buf,str,len);
...
crypto_digest_all(digests_t *ds_out, const char *m, size_t len)
{
  ...
  for (i = DIGEST_SHA256; i < N_DIGEST_ALGORITHMS; ++i) {
    if (crypto_digest256(ds_out->d[i], m, len, (digest_algorithm_t)i) < 0)
      return -1;
  }
  return 0;
}
...
{
  crypto_digest_t *r;
  r = (crypto_digest_t *)tor_malloc(sizeof(crypto_digest_t));
  SHA1_Init(&r->d.sha1);
...
crypto_store_dynamic_dh_modulus(const char *fname)
{
  ...
  base64_encoded_dh = (char*)tor_malloc_zero(len * 2); /* should be enough */
...
crypto_get_stored_dynamic_dh_modulus(const char *fname)
{
  ...
  cp = base64_decoded_dh = (unsigned char*)tor_malloc_zero(strlen(contents_tmp) + 1);
...
{
  crypto_dh_t *res = (crypto_dh_t *)tor_malloc_zero(sizeof(crypto_dh_t));
...
{
  crypto_dh_t *dh_new = (crypto_dh_t *)tor_malloc_zero(sizeof(crypto_dh_t));
  dh_new->dh = dh->dh;
  DH_up_ref(dh->dh);
  return dh_new;
}
                         ...
                         char *secret_out, size_t secret_bytes_out)
{
  ...
  secret_tmp = (char*)tor_malloc(secret_tmp_len);
...
{
  int i;
  uint8_t *cp, *tmp = (uint8_t *)tor_malloc(key_in_len+1);
...
crypto_seed_rng(int startup)
{
  ...
  load_entropy_ok = 1;//!crypto_strongest_rand(buf, sizeof(buf));
  if (load_entropy_ok) {
    sgx_read_rand((unsigned char *)buf, sizeof(buf));
                       ...
                       const char *suffix)
{
  ...
  rand_bytes = (char *)tor_malloc(rand_bytes_len);
  crypto_rand(rand_bytes, rand_bytes_len);

  result = (char *)tor_malloc(resultlen);
...
base32_decode(char *dest, size_t destlen, const char *src, size_t srclen)
{
  ...
  tmp = (char *)tor_malloc_zero(srclen);
  for (j = 0; j < srclen; ++j) {
...
openssl_dynlock_create_cb_(const char *file, int line)
{
  ...
  (void)file;
  (void)line;
  v = (struct CRYPTO_dynlock_value *)tor_malloc(sizeof(struct CRYPTO_dynlock_value));
...
setup_openssl_threading(void)
{
  ...
  openssl_mutexes_ = (tor_mutex_t **)tor_malloc(n*sizeof(tor_mutex_t *));
  for (i=0; i < n; ++i)
...
crypto_global_cleanup(void)
{
  ...
  // Fix me: method is disable in OPENSSL Lib 
  fprintf(stderr, "Fix me: Call unimplemented OpenSSL CONF_modules_unload in crypto.cpp \n"); 
