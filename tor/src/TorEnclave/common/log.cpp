...
#warn "MAXLINE is a very low number; it might not be from syslog.h after all"
      ...
      if (domain & LD_NOCB) {
        if (!callbacks_deferred && pending_cb_messages) {
          pending_cb_message_t *msg = (pending_cb_message_t *)tor_malloc(sizeof(pending_cb_message_t));
...
static int sigsafe_log_fds[MAX_SIGSAFE_FDS] = { SGX_STDERR };
...
tor_log_update_sigsafe_err_fds(void)
{
  ...
  if (!found_real_stderr &&
      int_array_contains(sigsafe_log_fds, n_sigsafe_log_fds, SGX_STDOUT)) {
...
{
  logfile_t *lf;
  lf = (logfile_t *)tor_malloc_zero(sizeof(logfile_t));
  lf->fd = fd;
  lf->filename = tor_strdup(name);
  lf->severities = (log_severity_list_t *)tor_memdup(severity, sizeof(log_severity_list_t));
  lf->next = logfiles;
...
{
  log_severity_list_t *s = (log_severity_list_t *)tor_malloc_zero(sizeof(log_severity_list_t));
  set_log_severity_config(min_severity, LOG_ERR, s);
  LOCK_LOGS();
  add_stream_log_impl(s, "<temp>", SGX_STDOUT);
  tor_free(s);
