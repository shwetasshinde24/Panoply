...
target_s_includes := $(addprefix -I,$(target_c_includes))
%.S : %.pl
	perl $< elf $(target_s_includes) $(openssl_cflags) $(target_c_flags) > $@

jni/topenssl/crypto/sha/asm/sha256-x86_64.S:
	perl jni/topenssl/crypto/sha/asm/sha512-x86_64.pl elf $(target_s_includes) $(openssl_cflags) $(target_c_flags) > $@

target_edl_dir    := $(LOCAL_PATH)/crypto
$(target_edl_dir)/openssltime_t.h :  $(target_edl_dir)/openssltime.edl
	cd $(target_edl_dir) && $(SGX_EDGER8R_TOOL) --header-only openssltime.edl

$(LOCAL_PATH)/crypto/bn/bn_rand.c : $(LOCAL_PATH)/crypto/openssltime_t.h
