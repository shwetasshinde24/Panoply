...
extern int sgx_wrapper_fsync(int fd);
extern int sgx_wrapper_ftruncate(int fd, off_t len);
extern unsigned int sgx_wrapper_alarm(unsigned int seconds);
extern int copy_arg_to_enclave(char* buff, int buff_size, const char **argv, int index);
extern int sgx_wrapper_mknod(const char *pathname, mode_t mode, dev_t dev);
...
#define fsync(A) sgx_wrapper_fsync(A)
