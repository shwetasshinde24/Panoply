...
void
latselect_doit(int n, fd_set *set)
{
	fd_set	nosave = *set;
...
latselect_main(int ac, char **av)
{
	...
	if (streq(av[1], "tcp")) {
		...
		snprintf(fname, L_tmpnam, "lat_selectXXXXXX");
		mkstemp(fname);
		char* my_fname = (char*)strdup(fname);
		fd = open(fname, O_RDWR|O_APPEND|O_CREAT, 0666);
		unlink(fname);
		if (fd <= 0) {
			int buflen = L_tmpnam+128;
			char buf[buflen];
			snprintf(buf, buflen,
	...
	BENCH(latselect_doit(nfds, &set), 0);
	snprintf(buf, L_tmpnam+256, report, n);
	micro(buf, get_n());
