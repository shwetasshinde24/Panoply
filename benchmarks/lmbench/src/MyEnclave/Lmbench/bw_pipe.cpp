...
void	bwpipe_reader(int controlfd, int pipefd, size_t bytes);
void	bwpipe_writer(int controlfd, int pipefd);
...
int
bwpipe_main(int argc, char** argv)
{
	int	pipes[2];
	int	control[2];
	...
	switch (pid = fork()) {
		...
		touch(buf, XFERSIZE);
		bwpipe_writer(control[0], pipes[1]);
		return(0);
		/*NOTREACHED*/
	...
	BENCH(bwpipe_reader(control[1], pipes[0], XFER), MEDIUM);
...
void
bwpipe_writer(int controlfd, int pipefd)
{
	size_t	todo;
...
void
bwpipe_reader(int controlfd, int pipefd, size_t bytes)
{
	int	done = 0;
	size_t	todo = bytes;
	size_t	bufsize = XFERSIZE;
