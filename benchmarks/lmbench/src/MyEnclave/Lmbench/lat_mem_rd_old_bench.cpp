...
#define	MEMTRIES	4
#define	LOWER	512
void	lat_mem_rd_old_loads(char *addr, size_t range, size_t stride);
size_t	lat_mem_rd_old_step(size_t k);

int
lat_mem_rd_old_main(int ac, char **av)
{
	size_t	len;
	size_t	range;
	size_t	stride;
	...
	if (av[2] == 0) {
		...
		for (range = LOWER; range <= len; range = lat_mem_rd_old_step(range)) {
			lat_mem_rd_old_loads(addr, range, STRIDE);
		}
	} else {
		for (i = 2; i < ac; ++i) {
			stride = bytes(av[i]);
...
void
lat_mem_rd_old_loads(char *addr, size_t range, size_t stride)
{
