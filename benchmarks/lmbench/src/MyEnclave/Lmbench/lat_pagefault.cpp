 ...
 * (1) the latpagefault_benchmark is unmodified, and
...
void	latpagefault_timeit(char *file, char *where, int size);

int
latpagefault_main(int ac, char **av)
{
#ifdef	MS_INVALIDATE
	int fd;
	char *where;
	...
	latpagefault_timeit(fname, where, sbuf.st_size);
 ...
 */
void
latpagefault_timeit(char *file, char *where, int size)
{
