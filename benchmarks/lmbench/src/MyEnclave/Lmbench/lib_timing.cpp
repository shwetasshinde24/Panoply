...
p64(uint64 big)
{
...
#ifdef  linux
	{
        ...
        if (a[1]) {
                snprintf(s, 20, "0x%x%08x", a[1], a[0]);
        } else {
                snprintf(s, 20, "0x%x", a[0]);
        }
	}
#endif
#ifdef	__sgi
        snprintf(s, 20, "0x%llx", big);
#endif
	return (s);
}
...
p64sz(uint64 big)
{
	...
	if (d < 100) {
		snprintf(s, 20, "%.4f%c", d, tags[t]);
	} else {
		snprintf(s, 20, "%.2f%c", d, tags[t]);
	}
	return (s);
}
