...
int sgx_wrapper_printf(const char *fmt, ...)
{
    ...
    char id[100];
    int result;
    // snprintf(id, WRAPBUFSIZ, "From Enclave %d --- ", enclave_self_id);
    va_list ap;
    va_start(ap, fmt);
    vsnprintf(buf, WRAPBUFSIZ, fmt, ap);
    va_end(ap);
    // ocall_print_string(&result, id);
...
int sgx_wrapper_pclose(SGX_WRAPPER_FILE file) 
{
  int ret = 0;
  sgx_status_t status = ocall_pclose(&ret, file);
  CHECK_STATUS(status);
  return ret;
}
...
char *sgx_wrapper_tempnam(const char *dir, const char *pfx)
{
  char * retval;
  sgx_status_t status = ocall_tempnam(&retval, dir, pfx);
  CHECK_STATUS(status);
  int len = strlen(retval)+1;
  char* t_dir = (char*)malloc(len);
  strncpy(t_dir, retval, len);
  return t_dir;
}
...
SGX_WRAPPER_FILE sgx_wrapper_popen(const char* command, const char* type)
{
  SGX_WRAPPER_FILE f = 0;
...
{
  // sgx_wrapper_printf("Warning: Use tor_sscanf which only handle limited case (Check code) \n");
  int r;
  va_list ap;
  va_start(ap, pattern);
