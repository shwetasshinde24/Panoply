				      ...
				      unsigned long arg)
{
	...
	// Fix me: Force vm_mmap to return the same virtual addr for every secs->base
	// Hardcode this stupided thing only for fork()
	vm_munmap(0x7fffb0000000, secs->size);
	secs->base = vm_mmap(filep, 0x7fffb0000000, secs->size,
