...
{
  tor_tls_t *result = (tor_tls_t *)SSL_get_ex_data(ssl, tor_tls_object_ex_data_index);
  if (result)
...
tor_cert_new(X509 *x509_cert)
{
  ...
  cert = (tor_cert_t *)tor_malloc_zero(sizeof(tor_cert_t));
  ...
  cert->encoded = (uint8_t*)tor_malloc(length);
                    ...
                    unsigned flags, int is_client)
{
  ...
  result = (tor_tls_context_t *)tor_malloc_zero(sizeof(tor_tls_context_t));
  result->refcnt = 1;
  if (!is_client) {
...
rectify_client_ciphers(STACK_OF(SSL_CIPHER) **ciphers)
{
  ...
  if (PREDICT_UNLIKELY(!CLIENT_CIPHER_STACK)) {
    ...
    CLIENT_CIPHER_DUMMIES =
      (SSL_CIPHER*)tor_malloc_zero(sizeof(SSL_CIPHER)*N_CLIENT_CIPHERS);
...
{
  BIO *bio = NULL;
  tor_tls_t *result = (tor_tls_t *)tor_malloc_zero(sizeof(tor_tls_t));
