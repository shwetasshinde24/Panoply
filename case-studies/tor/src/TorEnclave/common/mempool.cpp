...
mp_chunk_new(mp_pool_t *pool)
{
  ...
  mp_chunk_t *chunk = (mp_chunk_t *)ALLOC(CHUNK_OVERHEAD + sz);
...
mp_pool_get(mp_pool_t *pool)
{
  ...
  if (chunk->first_free) {
     ...
     * that. */
    allocated = (mp_allocated_t*)chunk->next_mem;
...
{
  mp_allocated_t *allocated = (mp_allocated_t*) M2A(item);
...
mp_pool_new(size_t item_size, size_t chunk_capacity)
{
  ...
  pool = (mp_pool_t *)ALLOC(sizeof(mp_pool_t));
  CHECK_ALLOC(pool);
...
mp_pool_sort_used_chunks(mp_pool_t *pool)
{
  ...
  if (!inverted)
    return;
  //printf("Sort %d/%d\n",inverted,n);
  chunks = (mp_chunk_t **)ALLOC(sizeof(mp_chunk_t *)*n);
#ifdef ALLOC_CAN_RETURN_NULL
