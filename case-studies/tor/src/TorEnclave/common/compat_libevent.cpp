...
libevent_logging_callback(int severity, const char *msg)
{
  ...
  switch (severity) {
    ...
    case _EVENT_LOG_WARN:
      // Fix me: EPOLL DELL always failed ???
...
{
  struct event *e = (struct event *)tor_malloc_zero(sizeof(struct event));
                   ...
                   void *data)
{
  ...
  tor_assert(tv);
  tor_assert(cb);
  timer = (periodic_timer_t *)tor_malloc_zero(sizeof(periodic_timer_t));
