...
void outside_enclave_free(void* p);
#ifdef USE_DMALLOC
extern int dmalloc_free(const char *file, const int line, void *pnt,
                        const int func_id);
#define tor_free(p) STMT_BEGIN \
    if (PREDICT_LIKELY((p)!=NULL)) {                \
      printf("USE_DMALLOC free call \n");           \
...
enum stream_status get_string_from_pipe(SGX_WRAPPER_FILE stream, char *buf, size_t count);
...
SGX_WRAPPER_FILE start_writing_to_stdio_file(const char *fname, int open_flags, int mode,
                                  open_file_t **data_out);
SGX_WRAPPER_FILE fdopen_file(open_file_t *file_data);
...
struct process_handle_t {
  ...
  int stdout_pipe;
  int stderr_pipe;
  SGX_WRAPPER_FILE stdout_handle;
  SGX_WRAPPER_FILE stderr_handle;
  pid_t pid;
...
#else
ssize_t tor_read_all_handle(SGX_WRAPPER_FILE h, char *buf, size_t count,
                            const process_handle_t *process,
                            int *eof);
#endif
...
#else
SGX_WRAPPER_FILE tor_process_get_stdout_pipe(process_handle_t *process_handle);
#endif
