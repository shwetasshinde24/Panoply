...
alloc_chunk(size_t sz, int freelist_ok)
{
  ...
  if (freelist && freelist_ok) {
    ...
    res = (memarea_chunk_t *)tor_malloc(chunk_size);
    res->next_chunk = NULL;
...
memarea_new(void)
{
  memarea_t *head = (memarea_t *)tor_malloc(sizeof(memarea_t));
...
memarea_alloc(memarea_t *area, size_t sz)
{
  ...
  */
  chunk->next_mem = (char*)realign_pointer(chunk->next_mem);
  return result;
}
...
{
  char *result = (char *)memarea_alloc(area, n);
  memcpy(result, s, n);
  return result;
}
...
{
  return (char *)memarea_memdup(area, s, strlen(s)+1);
}
...
memarea_strndup(memarea_t *area, const char *s, size_t n)
{
  ...
  for (ln = 0; ln < n && s[ln]; ++ln)
    ;
  result = (char *)memarea_alloc(area, ln+1);
  memcpy(result, s, ln);
  result[ln]='\0';
  return result;
}
