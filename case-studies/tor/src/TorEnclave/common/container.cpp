...
#define MAX_CAPACITY (int)((SIZE_MAX / (sizeof(void*))))
  ...
  if (size > sl->capacity) {
    ...
    sl->capacity = higher;
    sl->list = (void **)tor_realloc(sl->list, sizeof(void*)*((size_t)sl->capacity));
  }
}
...
smartlist_ints_eq(const smartlist_t *sl1, const smartlist_t *sl2)
{
  ...
  SMARTLIST_FOREACH(sl1, int *, cp1, {
      int *cp2 = (int *)smartlist_get(sl2, cp1_sl_idx);
      if (*cp1 != *cp2)
        return 0;
    });
  return 1;
...
{
  return (char *)smartlist_get_most_frequent(sl, compare_digests256_);
}
...
typedef struct digestmap_entry_t { 
  struct { 
...
// FIx me: Bug in the MACRO, rectify by preprocess the MACRO and bring back the genuinely source
HT_PROTOTYPE(strmap_impl, strmap_entry_t, node, strmap_entry_hash,
             strmap_entries_eq)
// int strmap_impl_HT_GROW(struct strmap_impl *ht, unsigned min_capacity); void strmap_impl_HT_CLEAR(struct strmap_impl *ht); int strmap_impl_HT_REP_IS_BAD_(const struct strmap_impl *ht); static inline void strmap_impl_HT_INIT(struct strmap_impl *head) { head->hth_table_length = 0; head->hth_table = 0; head->hth_n_entries = 0; head->hth_load_limit = 0; head->hth_prime_idx = -1; } static inline struct strmap_entry_t ** strmap_impl_HT_FIND_P_(struct strmap_impl *head, struct strmap_entry_t *elm) { struct strmap_entry_t **p; if (!head->hth_table) return 0; p = &((head)->hth_table[((elm)->node.hte_hash) % head->hth_table_length]); while (*p) { if (strmap_entries_eq(*p, elm)) return p; p = &(*p)->node.hte_next; } return p; } __attribute__ ((unused)) static inline struct strmap_entry_t * strmap_impl_HT_FIND(const struct strmap_impl *head, struct strmap_entry_t *elm) { struct strmap_entry_t **p; struct strmap_impl *h = head; do { (elm)->node.hte_hash = strmap_entry_hash(elm); } while (0); p = strmap_impl_HT_FIND_P_(h, elm); return p ? *p : 0; } __attribute__ ((unused)) static inline void strmap_impl_HT_INSERT(struct strmap_impl *head, struct strmap_entry_t *elm) { struct strmap_entry_t **p; if (!head->hth_table || head->hth_n_entries >= head->hth_load_limit) strmap_impl_HT_GROW(head, head->hth_n_entries+1); ++head->hth_n_entries; do { (elm)->node.hte_hash = strmap_entry_hash(elm); } while (0); p = &((head)->hth_table[((elm)->node.hte_hash) % head->hth_table_length]); elm->node.hte_next = *p; *p = elm; } __attribute__ ((unused)) static inline struct strmap_entry_t * strmap_impl_HT_REPLACE(struct strmap_impl *head, struct strmap_entry_t *elm) { struct strmap_entry_t **p, *r; if (!head->hth_table || head->hth_n_entries >= head->hth_load_limit) strmap_impl_HT_GROW(head, head->hth_n_entries+1); do { (elm)->node.hte_hash = strmap_entry_hash(elm); } while (0); p = strmap_impl_HT_FIND_P_(head, elm); r = *p; *p = elm; if (r && (r!=elm)) { elm->node.hte_next = r->node.hte_next; r->node.hte_next = 0; return r; } else { ++head->hth_n_entries; return 0; } } __attribute__ ((unused)) static inline struct strmap_entry_t * strmap_impl_HT_REMOVE(struct strmap_impl *head, struct strmap_entry_t *elm) { struct strmap_entry_t **p, *r; do { (elm)->node.hte_hash = strmap_entry_hash(elm); } while (0); p = strmap_impl_HT_FIND_P_(head,elm); if (!p || !*p) return 0; r = *p; *p = r->node.hte_next; r->node.hte_next = 0; --head->hth_n_entries; return r; } __attribute__ ((unused)) static inline void strmap_impl_HT_FOREACH_FN(struct strmap_impl *head, int (*fn)(struct strmap_entry_t *, void *), void *data) { unsigned idx; struct strmap_entry_t **p, **nextp, *next; if (!head->hth_table) return; for (idx=0; idx < head->hth_table_length; ++idx) { p = &head->hth_table[idx]; while (*p) { nextp = &(*p)->node.hte_next; next = *nextp; if (fn(*p, data)) { --head->hth_n_entries; *p = next; } else { p = nextp; } } } } __attribute__ ((unused)) static inline struct strmap_entry_t ** strmap_impl_HT_START(struct strmap_impl *head) { unsigned b = 0; while (b < head->hth_table_length) { if (head->hth_table[b]) return &head->hth_table[b]; ++b; } return 0; } __attribute__ ((unused)) static inline struct strmap_entry_t ** strmap_impl_HT_NEXT(struct strmap_impl *head, struct strmap_entry_t **elm) { if ((*elm)->node.hte_next) { return &(*elm)->node.hte_next; } else { unsigned b = (((*elm)->node.hte_hash) % head->hth_table_length)+1; while (b < head->hth_table_length) { if (head->hth_table[b]) return &head->hth_table[b]; ++b; } return 0; } } __attribute__ ((unused)) static inline struct strmap_entry_t ** strmap_impl_HT_NEXT_RMV(struct strmap_impl *head, struct strmap_entry_t **elm) { unsigned h = ((*elm)->node.hte_hash); *elm = (*elm)->node.hte_next; --head->hth_n_entries; if (*elm) { return elm; } else { unsigned b = (h % head->hth_table_length)+1; while (b < head->hth_table_length) { if (head->hth_table[b]) return &head->hth_table[b]; ++b; } return 0; } }

HT_GENERATE(strmap_impl, strmap_entry_t, node, strmap_entry_hash,
            strmap_entries_eq, 0.6, malloc, realloc, free)
// static unsigned strmap_impl_PRIMES[] = { 53, 97, 193, 389, 769, 1543, 3079, 6151, 12289, 24593, 49157, 98317, 196613, 393241, 786433, 1572869, 3145739, 6291469, 12582917, 25165843, 50331653, 100663319, 201326611, 402653189 }; static unsigned strmap_impl_N_PRIMES = (unsigned)(sizeof(strmap_impl_PRIMES)/sizeof(strmap_impl_PRIMES[0])); int strmap_impl_HT_GROW(struct strmap_impl *head, unsigned size) { unsigned new_len, new_load_limit; int prime_idx; struct strmap_entry_t **new_table; if (head->hth_prime_idx == (int)strmap_impl_N_PRIMES - 1) return 0; if (head->hth_load_limit > size) return 0; prime_idx = head->hth_prime_idx; do { new_len = strmap_impl_PRIMES[++prime_idx]; new_load_limit = (unsigned)(0.6*new_len); } while (new_load_limit <= size && prime_idx < (int)strmap_impl_N_PRIMES); if ((new_table = malloc(new_len*sizeof(struct strmap_entry_t*)))) { unsigned b; memset(new_table, 0, new_len*sizeof(struct strmap_entry_t*)); for (b = 0; b < head->hth_table_length; ++b) { struct strmap_entry_t *elm, *next; unsigned b2; elm = head->hth_table[b]; while (elm) { next = elm->node.hte_next; b2 = ((elm)->node.hte_hash) % new_len; elm->node.hte_next = new_table[b2]; new_table[b2] = elm; elm = next; } } if (head->hth_table) free(head->hth_table); head->hth_table = new_table; } else { unsigned b, b2; new_table = realloc(head->hth_table, new_len*sizeof(struct strmap_entry_t*)); if (!new_table) return -1; memset(new_table + head->hth_table_length, 0, (new_len - head->hth_table_length)*sizeof(struct strmap_entry_t*)); for (b=0; b < head->hth_table_length; ++b) { struct strmap_entry_t *e, **pE; for (pE = &new_table[b], e = *pE; e != 0; e = *pE) { b2 = ((e)->node.hte_hash) % new_len; if (b2 == b) { pE = &e->node.hte_next; } else { *pE = e->node.hte_next; e->node.hte_next = new_table[b2]; new_table[b2] = e; } } } head->hth_table = new_table; } head->hth_table_length = new_len; head->hth_prime_idx = prime_idx; head->hth_load_limit = new_load_limit; return 0; } void strmap_impl_HT_CLEAR(struct strmap_impl *head) { if (head->hth_table) free(head->hth_table); head->hth_table_length = 0; strmap_impl_HT_INIT(head); } int strmap_impl_HT_REP_IS_BAD_(const struct strmap_impl *head) { unsigned n, i; struct strmap_entry_t *elm; if (!head->hth_table_length) { if (!head->hth_table && !head->hth_n_entries && !head->hth_load_limit && head->hth_prime_idx == -1) return 0; else return 1; } if (!head->hth_table || head->hth_prime_idx < 0 || !head->hth_load_limit) return 2; if (head->hth_n_entries > head->hth_load_limit) return 3; if (head->hth_table_length != strmap_impl_PRIMES[head->hth_prime_idx]) return 4; if (head->hth_load_limit != (unsigned)(0.6*head->hth_table_length)) return 5; for (n = i = 0; i < head->hth_table_length; ++i) { for (elm = head->hth_table[i]; elm; elm = elm->node.hte_next) { if (((elm)->node.hte_hash) != strmap_entry_hash(elm)) return 1000 + i; if ((((elm)->node.hte_hash) % head->hth_table_length) != i) return 10000 + i; ++n; } } if (n != head->hth_n_entries) return 6; return 0; }

HT_PROTOTYPE(digestmap_impl, digestmap_entry_t, node, digestmap_entry_hash,
             digestmap_entries_eq)
// int digestmap_impl_HT_GROW(struct digestmap_impl *ht, unsigned min_capacity); void digestmap_impl_HT_CLEAR(struct digestmap_impl *ht); int digestmap_impl_HT_REP_IS_BAD_(const struct digestmap_impl *ht); static inline void digestmap_impl_HT_INIT(struct digestmap_impl *head) { head->hth_table_length = 0; head->hth_table = 0; head->hth_n_entries = 0; head->hth_load_limit = 0; head->hth_prime_idx = -1; } static inline struct digestmap_entry_t ** digestmap_impl_HT_FIND_P_(struct digestmap_impl *head, struct digestmap_entry_t *elm) { struct digestmap_entry_t **p; if (!head->hth_table) return 0; p = &((head)->hth_table[((elm)->node.hte_hash) % head->hth_table_length]); while (*p) { if (digestmap_entries_eq(*p, elm)) return p; p = &(*p)->node.hte_next; } return p; } __attribute__ ((unused)) static inline struct digestmap_entry_t * digestmap_impl_HT_FIND(const struct digestmap_impl *head, struct digestmap_entry_t *elm) { struct digestmap_entry_t **p; struct digestmap_impl *h = head; do { (elm)->node.hte_hash = digestmap_entry_hash(elm); } while (0); p = digestmap_impl_HT_FIND_P_(h, elm); return p ? *p : 0; } __attribute__ ((unused)) static inline void digestmap_impl_HT_INSERT(struct digestmap_impl *head, struct digestmap_entry_t *elm) { struct digestmap_entry_t **p; if (!head->hth_table || head->hth_n_entries >= head->hth_load_limit) digestmap_impl_HT_GROW(head, head->hth_n_entries+1); ++head->hth_n_entries; do { (elm)->node.hte_hash = digestmap_entry_hash(elm); } while (0); p = &((head)->hth_table[((elm)->node.hte_hash) % head->hth_table_length]); elm->node.hte_next = *p; *p = elm; } __attribute__ ((unused)) static inline struct digestmap_entry_t * digestmap_impl_HT_REPLACE(struct digestmap_impl *head, struct digestmap_entry_t *elm) { struct digestmap_entry_t **p, *r; if (!head->hth_table || head->hth_n_entries >= head->hth_load_limit) digestmap_impl_HT_GROW(head, head->hth_n_entries+1); do { (elm)->node.hte_hash = digestmap_entry_hash(elm); } while (0); p = digestmap_impl_HT_FIND_P_(head, elm); r = *p; *p = elm; if (r && (r!=elm)) { elm->node.hte_next = r->node.hte_next; r->node.hte_next = 0; return r; } else { ++head->hth_n_entries; return 0; } } __attribute__ ((unused)) static inline struct digestmap_entry_t * digestmap_impl_HT_REMOVE(struct digestmap_impl *head, struct digestmap_entry_t *elm) { struct digestmap_entry_t **p, *r; do { (elm)->node.hte_hash = digestmap_entry_hash(elm); } while (0); p = digestmap_impl_HT_FIND_P_(head,elm); if (!p || !*p) return 0; r = *p; *p = r->node.hte_next; r->node.hte_next = 0; --head->hth_n_entries; return r; } __attribute__ ((unused)) static inline void digestmap_impl_HT_FOREACH_FN(struct digestmap_impl *head, int (*fn)(struct digestmap_entry_t *, void *), void *data) { unsigned idx; struct digestmap_entry_t **p, **nextp, *next; if (!head->hth_table) return; for (idx=0; idx < head->hth_table_length; ++idx) { p = &head->hth_table[idx]; while (*p) { nextp = &(*p)->node.hte_next; next = *nextp; if (fn(*p, data)) { --head->hth_n_entries; *p = next; } else { p = nextp; } } } } __attribute__ ((unused)) static inline struct digestmap_entry_t ** digestmap_impl_HT_START(struct digestmap_impl *head) { unsigned b = 0; while (b < head->hth_table_length) { if (head->hth_table[b]) return &head->hth_table[b]; ++b; } return 0; } __attribute__ ((unused)) static inline struct digestmap_entry_t ** digestmap_impl_HT_NEXT(struct digestmap_impl *head, struct digestmap_entry_t **elm) { if ((*elm)->node.hte_next) { return &(*elm)->node.hte_next; } else { unsigned b = (((*elm)->node.hte_hash) % head->hth_table_length)+1; while (b < head->hth_table_length) { if (head->hth_table[b]) return &head->hth_table[b]; ++b; } return 0; } } __attribute__ ((unused)) static inline struct digestmap_entry_t ** digestmap_impl_HT_NEXT_RMV(struct digestmap_impl *head, struct digestmap_entry_t **elm) { unsigned h = ((*elm)->node.hte_hash); *elm = (*elm)->node.hte_next; --head->hth_n_entries; if (*elm) { return elm; } else { unsigned b = (h % head->hth_table_length)+1; while (b < head->hth_table_length) { if (head->hth_table[b]) return &head->hth_table[b]; ++b; } return 0; } }

HT_GENERATE(digestmap_impl, digestmap_entry_t, node, digestmap_entry_hash,
            digestmap_entries_eq, 0.6, malloc, realloc, free)
// static unsigned digestmap_impl_PRIMES[] = { 53, 97, 193, 389, 769, 1543, 3079, 6151, 12289, 24593, 49157, 98317, 196613, 393241, 786433, 1572869, 3145739, 6291469, 12582917, 25165843, 50331653, 100663319, 201326611, 402653189 }; static unsigned digestmap_impl_N_PRIMES = (unsigned)(sizeof(digestmap_impl_PRIMES)/sizeof(digestmap_impl_PRIMES[0])); int digestmap_impl_HT_GROW(struct digestmap_impl *head, unsigned size) { unsigned new_len, new_load_limit; int prime_idx; struct digestmap_entry_t **new_table; if (head->hth_prime_idx == (int)digestmap_impl_N_PRIMES - 1) return 0; if (head->hth_load_limit > size) return 0; prime_idx = head->hth_prime_idx; do { new_len = digestmap_impl_PRIMES[++prime_idx]; new_load_limit = (unsigned)(0.6*new_len); } while (new_load_limit <= size && prime_idx < (int)digestmap_impl_N_PRIMES); if ((new_table = malloc(new_len*sizeof(struct digestmap_entry_t*)))) { unsigned b; memset(new_table, 0, new_len*sizeof(struct digestmap_entry_t*)); for (b = 0; b < head->hth_table_length; ++b) { struct digestmap_entry_t *elm, *next; unsigned b2; elm = head->hth_table[b]; while (elm) { next = elm->node.hte_next; b2 = ((elm)->node.hte_hash) % new_len; elm->node.hte_next = new_table[b2]; new_table[b2] = elm; elm = next; } } if (head->hth_table) free(head->hth_table); head->hth_table = new_table; } else { unsigned b, b2; new_table = realloc(head->hth_table, new_len*sizeof(struct digestmap_entry_t*)); if (!new_table) return -1; memset(new_table + head->hth_table_length, 0, (new_len - head->hth_table_length)*sizeof(struct digestmap_entry_t*)); for (b=0; b < head->hth_table_length; ++b) { struct digestmap_entry_t *e, **pE; for (pE = &new_table[b], e = *pE; e != 0; e = *pE) { b2 = ((e)->node.hte_hash) % new_len; if (b2 == b) { pE = &e->node.hte_next; } else { *pE = e->node.hte_next; e->node.hte_next = new_table[b2]; new_table[b2] = e; } } } head->hth_table = new_table; } head->hth_table_length = new_len; head->hth_prime_idx = prime_idx; head->hth_load_limit = new_load_limit; return 0; } void digestmap_impl_HT_CLEAR(struct digestmap_impl *head) { if (head->hth_table) free(head->hth_table); head->hth_table_length = 0; digestmap_impl_HT_INIT(head); } int digestmap_impl_HT_REP_IS_BAD_(const struct digestmap_impl *head) { unsigned n, i; struct digestmap_entry_t *elm; if (!head->hth_table_length) { if (!head->hth_table && !head->hth_n_entries && !head->hth_load_limit && head->hth_prime_idx == -1) return 0; else return 1; } if (!head->hth_table || head->hth_prime_idx < 0 || !head->hth_load_limit) return 2; if (head->hth_n_entries > head->hth_load_limit) return 3; if (head->hth_table_length != digestmap_impl_PRIMES[head->hth_prime_idx]) return 4; if (head->hth_load_limit != (unsigned)(0.6*head->hth_table_length)) return 5; for (n = i = 0; i < head->hth_table_length; ++i) { for (elm = head->hth_table[i]; elm; elm = elm->node.hte_next) { if (((elm)->node.hte_hash) != digestmap_entry_hash(elm)) return 1000 + i; if ((((elm)->node.hte_hash) % head->hth_table_length) != i) return 10000 + i; ++n; } } if (n != head->hth_n_entries) return 6; return 0; }
...
{
  strmap_t *result;
  result = (strmap_t *)tor_malloc(sizeof(strmap_t));
...
{
  digestmap_t *result;
  result = (digestmap_t *)tor_malloc(sizeof(digestmap_t));
...
strmap_set(strmap_t *map, const char *key, void *val)
{
  ...
  if (resolve) {
    ...
    resolve = (strmap_entry_t *)tor_malloc_zero(sizeof(strmap_entry_t));
...
digestmap_set(digestmap_t *map, const char *key, void *val)
{
  ...
  if (resolve) {
    ...
    resolve = (digestmap_entry_t *)tor_malloc_zero(sizeof(digestmap_entry_t));
  ...
  HT_FIND_OR_INSERT_(digestmap_impl, node, digestmap_entry_hash, &(map->head),
         ...
         },
         {
           ...
           digestmap_entry_t *newent = (digestmap_entry_t *)
...
{
  strmap_entry_t **ent, **next, *the_entry;
  if (!map)
    return;
  ...
  for (ent = HT_START(strmap_impl, &map->head); ent != NULL; ent = next) {
    ...
    tor_free(the_entry->key);
    if (free_val)
      free_val(the_entry->val);
    tor_free(the_entry);
  }
...
{
  digestmap_entry_t **ent, **next, *the_entry;
  if (!map)
    return;
...
#define IMPLEMENT_ORDER_FUNC(funcname, elt_t)                   \
  ...
  {                                                             \
    const elt_t *a = (const elt_t *)_a, *b = (const elt_t *)_b;                               \
    if (*a<*b)                                                  \
      return -1;                                                \
    else if (*a>*b)                                             \
      return 1;                                                 \
...
digestset_new(int max_elements)
{
  ...
  digestset_t *r = (digestset_t *)tor_malloc(sizeof(digestset_t));
  r->mask = n_bits - 1;
