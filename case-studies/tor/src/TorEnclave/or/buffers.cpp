...
chunk_grow(chunk_t *chunk, size_t sz)
{
  ...
  chunk = (chunk_t *)tor_realloc(chunk, CHUNK_ALLOC_SIZE(sz));
  chunk->memlen = sz;
...
buf_new(void)
{
  buf_t *buf = (buf_t *)tor_malloc_zero(sizeof(buf_t));
...
{
  chunk_t *newch = (chunk_t *)tor_memdup(in_chunk, CHUNK_ALLOC_SIZE(in_chunk->memlen));
...
buf_find_pos_of_char(char ch, buf_pos_t *out)
{
  ...
  for (chunk = out->chunk; chunk; chunk = chunk->next) {
    char *cp = (char *)memchr(chunk->data+pos, ch, chunk->datalen - pos);
    if (cp) {
      out->chunk = chunk;
...
#define CONTENT_LENGTH "\r\nContent-Length: "
  ...
  if (headers_out) {
    *headers_out = (char *)tor_malloc(headerlen+1);
  ...
  if (body_out) {
    ...
    *body_used = bodylen;
    *body_out = (char *)tor_malloc(bodylen+1);
...
{
  return (socks_request_t *)tor_malloc_zero(sizeof(socks_request_t));
}
            ...
            size_t *want_length_out)
{
  ...
  if (req->socks_version == 5 && !req->got_auth) {
    ...
    if (*data == 1) { /* username/pass version 1 */
      ...
      if (usernamelen) {
        req->username = (char *)tor_memdup(data+2u, usernamelen);
        req->usernamelen = usernamelen;
      }
      if (passlen) {
        req->password = (char *)tor_memdup(data+3u+usernamelen, passlen);
  ...
  switch (socksver) { /* which version of socks? */
    ...
    case 4: { /* socks4 */
      ...
      if (socks4_prot == socks4a) {
        ...
        startaddr = next+1;
        next = (char *)memchr(startaddr, 0, data + datalen - startaddr);
        if (!next) {
          if (datalen >= 1024) {
      ...
      if (authend != authstart) {
        ...
        req->username = (char *)tor_memdup(authstart, authend - authstart);
      }
...
buf_find_offset_of_char(buf_t *buf, char ch)
{
  ...
  off_t offset = 0;
  for (chunk = buf->head; chunk; chunk = chunk->next) {
    char *cp = (char *)memchr(chunk->data, ch, chunk->datalen);
    if (cp)
