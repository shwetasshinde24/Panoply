...
connection_or_remove_from_identity_map(or_connection_t *conn)
{
  ...
  if (!orconn_identity_map)
    return;
  tmp = (or_connection_t *)digestmap_get(orconn_identity_map, conn->identity_digest);
  if (!tmp) {
...
connection_or_set_identity_digest(or_connection_t *conn, const char *digest)
{
  ...
  tmp = (or_connection_t *)digestmap_set(orconn_identity_map, digest, conn);
...
connection_or_remove_from_ext_or_id_map(or_connection_t *conn)
{
  ...
  tmp = (or_connection_t *)digestmap_remove(orconn_ext_or_id_map, conn->ext_or_conn_id);
...
connection_or_get_by_ext_or_id(const char *id)
{
  if (!orconn_ext_or_id_map)
    return NULL;
  return (or_connection_t *)digestmap_get(orconn_ext_or_id_map, id);
}
...
connection_or_set_ext_or_identifier(or_connection_t *conn)
{
  ...
  if (!conn->ext_or_conn_id)
    conn->ext_or_conn_id = (char *)tor_malloc_zero(EXT_OR_CONN_ID_LEN);

  memcpy(conn->ext_or_conn_id, random_id, EXT_OR_CONN_ID_LEN);

  tmp = (or_connection_t *)digestmap_set(orconn_ext_or_id_map, random_id, conn);
  tor_assert(!tmp);
}
...
{
  const broken_state_count_t *a = (const broken_state_count_t *)*a_ptr, *b = (const broken_state_count_t *)*b_ptr;
  if (b->count < a->count)
    return -1;
...
connection_or_report_broken_states(int severity, int domain)
{
  ...
  STRMAP_FOREACH(broken_connection_counts, state, void *, countptr) {
    broken_state_count_t *c = (broken_state_count_t *)tor_malloc(sizeof(broken_state_count_t));
...
var_cell_new(uint16_t payload_len)
{
  ...
  var_cell_t *cell = (var_cell_t *)tor_malloc_zero(size);
                                     ...
                                     int started_here)
{
  ...
  if (r) {
    ...
    if (n) {
      ...
      conn->nickname = (char *)tor_malloc(HEX_DIGEST_LEN+2);
      conn->nickname[0] = '$';
                      ...
                      channel_tls_t *chan)
{
  ...
  switch (connection_connect(TO_CONN(conn), conn->base_.address,
    ...
    case -1:
      ...
      return NULL;
    case 0:
      connection_watch_events(TO_CONN(conn), (watchable_events_t)(READ_EVENT|WRITE_EVENT));
...
{
  or_connection_t *conn = (or_connection_t *)_conn;
  (void)tls;
...
connection_init_or_handshake_state(or_connection_t *conn, int started_here)
{
  ...
  s = conn->handshake_state = (or_handshake_state_t *)tor_malloc_zero(sizeof(or_handshake_state_t));
