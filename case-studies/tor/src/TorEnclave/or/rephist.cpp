...
get_or_history(const char* id)
{
  ...
  hist = (or_history_t *)digestmap_get(history_map, id);
  if (!hist) {
    hist = (or_history_t *)tor_malloc_zero(sizeof(or_history_t));
...
get_link_history(const char *from_id, const char *to_id)
{
  ...
  if (!lhist) {
    lhist = (link_history_t *)tor_malloc_zero(sizeof(link_history_t));
...
{
  or_history_t *hist = (or_history_t *)_hist;
...
find_next_with(smartlist_t *sl, int i, const char *prefix)
{
  for ( ; i < smartlist_len(sl); ++i) {
    const char *line = (const char *)smartlist_get(sl, i);
...
rep_hist_load_mtbf_data(time_t now)
{
...

  {
    ...
    if (smartlist_len(lines)>4) {
      firstline = (const char *)smartlist_get(lines, 0);
  ...
  for (i = 1; i < smartlist_len(lines); ++i) {
    line = (const char *)smartlist_get(lines, i);
    if (!strcmp(line, "data"))
      break;
  ...
  for (; i < smartlist_len(lines); ++i) {
    ...
    if (format == 1) {
      ...
      if (mtbf_idx >= 0) {
        const char *mtbfline = (const char *)smartlist_get(lines, mtbf_idx);
      ...
      if (wfu_idx >= 0) {
        const char *wfuline = (const char *)smartlist_get(lines, wfu_idx);
...
bw_array_new(void)
{
  bw_array_t *b;
  time_t start;
  b = (bw_array_t *)tor_malloc_zero(sizeof(bw_array_t));
                                   ...
                                   const int s_interval)
{
  ...
  if (s_values && s_begins >= now - NUM_SECS_BW_SUM_INTERVAL*NUM_TOTALS) {
    ...
    SMARTLIST_FOREACH_BEGIN(s_values, const char *, cp) {
        ...
        if (have_maxima) {
          maxstr = (const char *)smartlist_get(s_maxima, cp_sl_idx);
...
{
  predicted_port_t *pp = (predicted_port_t *)tor_malloc(sizeof(predicted_port_t));
  pp->port = port;
  pp->time = now;
...
rep_hist_exit_stats_init(time_t now)
{
  ...
  exit_bytes_read = (uint64_t *)tor_malloc_zero(EXIT_STATS_NUM_PORTS *
                                    sizeof(uint64_t));
  exit_bytes_written = (uint64_t *)tor_malloc_zero(EXIT_STATS_NUM_PORTS *
                                       sizeof(uint64_t));
  exit_streams = (uint32_t *)tor_malloc_zero(EXIT_STATS_NUM_PORTS *
                                 sizeof(uint32_t));
}
    ...
    double mean_time_cells_in_queue, uint32_t processed_cells)
{
  ...
  if (!start_of_buffer_stats_interval)
    return; /* Not initialized. */
  stat = (circ_buffer_stats_t *)tor_malloc_zero(sizeof(circ_buffer_stats_t));
...
{
  const circ_buffer_stats_t *a = (const circ_buffer_stats_t *)*_a, *b = (const circ_buffer_stats_t *)*_b;
...
rep_hist_format_desc_stats(time_t now)
{
  ...
  if (size > 0) {
    vals = (int *)tor_malloc(size * sizeof(int));
                            ...
                            size_t num_written, time_t when)
{
  ...
  if (num_read > 0 || num_written > 0) {
    ...
    if (entry) {
      ...
      entry = (bidi_map_entry_t *)tor_malloc_zero(sizeof(bidi_map_entry_t));
