...
router_reload_consensus_networkstatus(void)
{
  ...
  for (flav = 0; flav < N_CONSENSUS_FLAVORS; ++flav) {
    char buf[128];
    const char *flavor = networkstatus_get_flavor_name((consensus_flavor_t)flav);
    if (flav == FLAV_NS) {
...
{
  document_signature_t *r = (document_signature_t *)tor_memdup(sig, sizeof(document_signature_t));
  if (r->signature)
    r->signature = (char *)tor_memdup(sig->signature, sig->signature_len);
  return r;
}
                                       ...
                                       const authority_cert_t *cert)
{
  ...
  signed_digest = (char *)tor_malloc(signed_digest_len);
...
compare_digest_to_routerstatus_entry(const void *_key, const void **_member)
{
  ...
  const routerstatus_t *rs = (const routerstatus_t *)*_member;
...
{
  const char *key = (const char *)_key;
  const vote_routerstatus_t *vrs = (const vote_routerstatus_t *)*_member;
...
{
  return (routerstatus_t*)smartlist_bsearch(ns->routerstatus_list, digest,
                                                 ...
                                                 const char *digest)
{
  ...
  return (routerstatus_t*)digestmap_get(consensus->desc_digest_map, digest);
}
...
router_get_mutable_consensus_status_by_id(const char *digest)
{
  if (!current_consensus)
    return NULL;
  return (routerstatus_t *)smartlist_bsearch(current_consensus->routerstatus_list, digest,
                           compare_digest_to_routerstatus_entry);
}
...
networkstatus_get_router_digest_by_nickname(const char *nickname)
{
  if (!named_server_map)
    return NULL;
  return (const char *)strmap_get_lc(named_server_map, nickname);
}
...
update_consensus_networkstatus_downloads(time_t now)
{
  ...
  for (i=0; i < N_CONSENSUS_FLAVORS; ++i) {
    ...
    c = networkstatus_get_latest_consensus_by_flavor((consensus_flavor_t)i);
    ...
    resource = networkstatus_get_flavor_name((consensus_flavor_t)i);
    ...
    log_info(LD_DIR, "Launching %s networkstatus consensus download.",
             networkstatus_get_flavor_name((consensus_flavor_t)i));
...
{
  const or_options_t *options = get_options();
  networkstatus_t *c = networkstatus_get_latest_consensus_by_flavor((consensus_flavor_t)flav);
  const char *flavor = networkstatus_get_flavor_name((consensus_flavor_t)flav);
...
#define REASONABLY_LIVE_TIME (24*60*60)
  networkstatus_t *consensus =
    networkstatus_get_latest_consensus_by_flavor((consensus_flavor_t)flavor);
  if (consensus &&
                                    ...
                                    unsigned flags)
{
  ...
  if ((int)c->flavor != flav) {
    ...
    flav = c->flavor;
    flavor = networkstatus_get_flavor_name((consensus_flavor_t)flav);
  }
...
signed_descs_update_status_from_consensus_networkstatus(smartlist_t *descs)
{
  ...
  {
    const routerstatus_t *rs = (const routerstatus_t*)digestmap_get(ns->desc_digest_map,
