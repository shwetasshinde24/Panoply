...
{
  const cached_resolve_t *a = (const cached_resolve_t *)_a, *b = (const cached_resolve_t *)_b;
...
purge_expired_resolves(time_t now)
{
  ...
  while (smartlist_len(cached_resolve_pqueue)) {
    resolve = (cached_resolve_t *) smartlist_get(cached_resolve_pqueue, 0);
                 ...
                 cached_resolve_t **resolve_out)
{
  ...
  *made_connection_pending_out = 0;
  ...
  if (resolve && resolve->expire > now) { /* already there */
    switch (resolve->state) {
      case CACHE_STATE_PENDING:
        ...
        pending_connection = (pending_connection_t *)tor_malloc_zero(
  ...
  resolve = (cached_resolve_t*)tor_malloc_zero(sizeof(cached_resolve_t));
  ...
  pending_connection = (pending_connection_t *)tor_malloc_zero(sizeof(pending_connection_t));
  pending_connection->conn = exitconn;
  resolve->pending_connections = pending_connection;
  *made_connection_pending_out = 1;
...
make_pending_resolve_cached(cached_resolve_t *resolve)
{
  ...
  * way.  XXXXX we could do better!*/
  ...
  {
    cached_resolve_t *new_resolve = (cached_resolve_t *)tor_memdup(resolve,
...
configure_nameservers(int force)
{
  ...
  if (conf_fname) {
    ...
    const char* dup_fname = "/etc/resolv.conf";
               ...
               void *arg)
{
  ...
  if (result == DNS_ERR_NONE) {
    if (type == DNS_IPv4_A && count) {
      ...
      uint32_t *addrs = (uint32_t *)addresses;
      ...
      char *escaped_address;
      struct in6_addr *addrs = (struct in6_addr *)addresses;
                   ...
                   const tor_addr_t *ptr_address)
{
  ...
  char *addr = (char *)tor_malloc(addr_len + 2);
...
wildcard_increment_answer(const char *id)
{
  ...
  ip = (int *)strmap_get(dns_wildcard_response_count, id); // may be null (0)
  if (!ip) {
    ip = (int *)tor_malloc_zero(sizeof(int));
                              ...
                              void *addresses, void *arg)
{
  ...
  if (result == DNS_ERR_NONE && count) {
    char *string_address = (char *)arg;
    int i;
    if (type == DNS_IPv4_A) {
      const uint32_t *addrs = (const uint32_t *)addresses;
      for (i = 0; i < count; ++i) {
        char answer_buf[INET_NTOA_BUF_LEN+1];
        struct in_addr in;
        in.s_addr = addrs[i];
      ...
      const struct in6_addr *addrs = (const struct in6_addr *)addresses;
      for (i = 0; i < count; ++i) {
