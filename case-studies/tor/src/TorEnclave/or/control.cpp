...
write_escaped_data(const char *data, size_t len, char **out)
{
  ...
  *out = outp = (char *)tor_malloc(sz_out+1);
  end = data+len;
  start_of_line = 1;
  while (data < end) {
    if (*data == '\n') {
...
read_escaped_data(const char *data, size_t len, char **out)
{
  ...
  *out = outp = (char *)tor_malloc(len+1);
                   ...
                   char **out, size_t *out_len)
{
  ...
  tor_assert(*end == '\"');
  outp = *out = (char *)tor_malloc(len+1);
  *out_len = n_chars;
                       ...
                       const char *body)
{
  ...
  if ((len = smartlist_len(unrecognized))) {
    ...
    char *tmp = (char *)smartlist_get(answers, len-1);
                         ...
                         const char *body)
{
  ...
  retval = options_init_from_string(NULL, body, or_options_t::CMD_RUN_TOR, NULL, &errstring);
                            ...
                            const char *body)
{
  ...
  if (!len) {
    ...
    tor_assert(i>0);
    password_len = i/2;
    password = (char *)tor_malloc(password_len + 1);
                          ...
                          const char *body)
{
  ...
  SMARTLIST_FOREACH_BEGIN(lines, char *, line) {
    ...
    if (smartlist_len(elts) == 2) {
      const char *from = (const char *)smartlist_get(elts,0);
      const char *to = (const char *)smartlist_get(elts,1);
                    ...
                    char **answer, const char **errmsg)
{
  ...
  if (!strcmp(question, "version")) {
    ...
    *answer = (char *)tor_malloc(HEX_DIGEST_LEN+1);
                                ...
                                const signed_descriptor_t *ei)
{
  ...
  outp = out = (char *)tor_malloc(ri_len+ei_len+1);
  if (!(router_sig = (const char *)tor_memstr(ri_body, ri_len, "\nrouter-signature")))
    goto bail;
  ++router_sig;
  ...
  for (i=0; i < 2; ++i) {
    const char *kwd = i ? "\nwrite-history " : "\nread-history ";
    const char *cp, *eol;
    if (!(cp = (const char *)tor_memstr(ei_body, ei_len, kwd)))
      continue;
    ++cp;
    if (!(eol = (const char *)memchr(cp, '\n', ei_len - (cp-ei_body))))
      continue;
                   ...
                   const char **errmsg)
{
  ...
  if (!strcmpstart(question, "desc/id/")) {
    ...
    *answer = smartlist_join_strings(sl, "", 0, NULL);
    ...
    cp = *answer = (char *)tor_malloc(answer_len+1);
    SMARTLIST_FOREACH(descs, signed_descriptor_t *, sd,
                      {
                       ...
                       const char *body)
{
  ...
  for (i = 0; i < smartlist_len(answers); i += 2) {
    char *k = (char *)smartlist_get(answers, i);
    char *v = (char *)smartlist_get(answers, i+1);
    if (!strchr(v, '\n') && !strchr(v, '\r')) {
...
find_element_starting_with(smartlist_t *sl, int start_at, const char *prefix)
{
  ...
  for (i = start_at; i < smartlist_len(sl); ++i) {
    const char *elt = (const char *)smartlist_get(sl, i);
                             ...
                             const char *body)
{
  ...
  if (zero_circ) {
    ...
    if ((smartlist_len(args) == 1) ||
        (smartlist_len(args) >= 2 && is_keyval_pair((const char *)smartlist_get(args, 1)))) {
  ...
  if (!zero_circ && !(circ = get_circ((const char*)smartlist_get(args,0)))) {
  ...
  smartlist_split_string(router_nicknames, (const char *)smartlist_get(args,1), ",", 0, 0);
                                 ...
                                 uint32_t len, const char *body)
{
  ...
  if (!(circ = get_circ((const char*)smartlist_get(args,0)))) {
    connection_printf_to_buf(conn, "552 Unknown circuit \"%s\"\r\n",
                             (char*)smartlist_get(args, 0));
    goto done;
  }
                            ...
                            const char *body)
{
  ...
  if (!(ap_conn = get_stream((const char *)smartlist_get(args, 0)))) {
    connection_printf_to_buf(conn, "552 Unknown stream \"%s\"\r\n",
                             (char*)smartlist_get(args, 0));
  } else if (!zero_circ && !(circ = get_circ((const char*)smartlist_get(args, 1)))) {
    connection_printf_to_buf(conn, "552 Unknown circuit \"%s\"\r\n",
                              ...
                              const char *body)
{
  ...
  if (!(ap_conn = get_stream((const char *)smartlist_get(args, 0)))
    ...
    if (smartlist_len(args) > 2) { /* they included a port too */
      new_port = (uint16_t) tor_parse_ulong((const char *)smartlist_get(args, 2),
                                            10, 1, 65535, &ok, NULL);
    }
    if (!ok) {
      ...
      new_addr = tor_strdup((const char *)smartlist_get(args, 1));
    }
  }
                           ...
                           const char *body)
{
  ...
  else if (!(ap_conn = get_stream((const char *)smartlist_get(args, 0))))
    connection_printf_to_buf(conn, "552 Unknown stream \"%s\"\r\n",
                             (char*)smartlist_get(args, 0));
  else {
    reason = (uint8_t) tor_parse_ulong((const char *)smartlist_get(args,1), 10, 0, 255,
                                       &ok, NULL);
    if (!ok) {
                            ...
                            const char *body)
{
  ...
  if (!(circ=get_circ((const char*)smartlist_get(args, 0))))
    ...
    for (i=1; i < smartlist_len(args); ++i) {
      if (!strcasecmp((const char*)smartlist_get(args, i), "IfUnused"))
        safe = 1;
      else
                             ...
                             const char *body)
{
  ...
  if (*cp == '"') {
    ...
    client_nonce = (char *)tor_malloc_zero(client_nonce_len);
...

  {
    ...
    char *tmp = (char *)tor_malloc_zero(tmp_len);
    char *client_hash = (char *)tor_malloc_zero(DIGEST256_LEN);
...
connection_control_process_inbuf(control_connection_t *conn)
{
  ...
  if (!conn->incoming_cmd) {
    conn->incoming_cmd = (char *)tor_malloc(1024);
  ...
  while (1) {
    ...
    do {
      ...
      if (r == 0)
        ...
        conn->incoming_cmd = (char *)tor_realloc(conn->incoming_cmd,
...
control_event_circuit_cell_stats(void)
{
  ...
  if (!get_options()->TestingEnableCellStatsEvent ||
    ...
    return 0;
  cell_stats = (cell_stats_t *)tor_malloc(sizeof(cell_stats_t));;
  TOR_LIST_FOREACH(circ, circuit_get_global_list(), head) {
...
control_event_descriptors_changed(smartlist_t *routers)
{
...

  {
    ...
    SMARTLIST_FOREACH(routers, routerinfo_t *, ri, {
        char *b = (char *)tor_malloc(MAX_VERBOSE_NICKNAME_LEN+1);
...
control_event_conf_changed(const smartlist_t *elements)
{
  ...
  for (i = 0; i < smartlist_len(elements); i += 2) {
    char *k = (char *)smartlist_get(elements, i);
    char *v = (char *)smartlist_get(elements, i+1);
    if (v == NULL) {
                                            ...
                                            or_connection_t *or_conn))
{
  ...
  while (status>=0 && bootstrap_status_to_string((bootstrap_status_t)status, &tag, &summary) < 0)
