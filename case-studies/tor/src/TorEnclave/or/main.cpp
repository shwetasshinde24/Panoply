...
{
  printf("Call this connection_add_impl \n");
  tor_assert(conn);
...
connection_remove(connection_t *conn)
{
  ...
  smartlist_del(connection_array, current_index);
  tmp = (connection_t *)smartlist_get(connection_array, current_index);
...
close_closeable_connections(void)
{
  ...
  for (i = 0; i < smartlist_len(closeable_connection_lst); ) {
    connection_t *conn = (connection_t *)smartlist_get(closeable_connection_lst, i);
    if (conn->conn_array_index < 0) {
...
{
  connection_t *conn = (connection_t *)_conn;
  (void)fd;
  (void)event;
...
{
  cell_t cell;
  connection_t *conn = (connection_t *)smartlist_get(connection_array, i);
  const or_options_t *options = get_options();
...
tor_init(int argc, char *argv[])
{
 ...
 /* give it somewhere to log to initially */
  ...
  // Fix me: unknown function atexit
  fprintf(stderr, "Fix me: call unknown atexit function \n");
...
{
  if (!postfork) {
    // Fix me: Either check the libevent2 dns module, or port the ext eventdns
    fprintf(stderr, "Fix me: call unimplemented evdns_shutdown in tor_free_all in or/main.cpp \n");
    // evdns_shutdown(1);
  }
  geoip_free_all();
  dirvote_free_all();
...
{
  const or_options_t *options = get_options();
  if (options->command == or_options_t::CMD_RUN_TOR) {
    time_t now = time(NULL);
...
tor_main(int argc, char *argv[])
{
  ...
  PWCHAR p = NULL;
  SGX_WRAPPER_FILE redir = NULL;
  SGX_WRAPPER_FILE redirdbg = NULL;
  ...
  if (get_options()->Sandbox && get_options()->command == or_options_t::CMD_RUN_TOR) {
  ...
  case or_options_t::CMD_RUN_TOR:
#ifdef NT_SERVICE
    nt_service_set_state(SERVICE_RUNNING);
#endif
    result = do_main_loop();
    break;
  case or_options_t::CMD_LIST_FINGERPRINT:
    result = do_list_fingerprint();
    break;
  case or_options_t::CMD_HASH_PASSWORD:
    do_hash_password();
    result = 0;
    break;
  case or_options_t::CMD_VERIFY_CONFIG:
    printf("Configuration was valid\n");
    result = 0;
    break;
  case or_options_t::CMD_DUMP_CONFIG:
    result = do_dump_config();
    break;
  case or_options_t::CMD_RUN_UNITTESTS: /* only set by test.c */
  default:
