...
{
  transport_t *t = (transport_t *)tor_malloc_zero(sizeof(transport_t));
...
transport_copy(const transport_t *transport)
{
  ...
  new_transport = (transport_t *)tor_malloc_zero(sizeof(transport_t));
...
assert_unconfigured_count_ok(void)
{
  ...
  printf("\n assert_unconfigured_count_ok: %d -- %d \n", n_completed + unconfigured_proxies_n, 
    smartlist_len(managed_proxy_list));
  // Fix me: bug when test: unequal n_completed + unconfigured_proxies_n
...
launch_managed_proxy(managed_proxy_t *mp)
{
  ...
  retval = tor_spawn_background(NULL,
                                ...
                                &mp->process_handle);
#else
  // Fix me: Need to handle tor spawn background
  fprintf(stderr, "Fix me: launch_managed_proxy call in or/transports.cpp is not implemented \n");
  retval = -1;
...
parse_smethod_line(const char *line, managed_proxy_t *mp)
{
  ...
  tor_assert(!strcmp((const char*)smartlist_get(items,0),PROTO_SMETHOD));

  method_name = (char *)smartlist_get(items,1);
  ...
  if (smartlist_len(items) > 3) {
    ...
    char *options_string = (char *)smartlist_get(items, 3);
...
parse_cmethod_line(const char *line, managed_proxy_t *mp)
{
  ...
  tor_assert(!strcmp((const char *)smartlist_get(items,0),PROTO_CMETHOD));
  ...
  socks_ver_str = (char *)smartlist_get(items,2);
  ...
  addrport = (char *)smartlist_get(items, 3);
  if (tor_addr_port_split(LOG_WARN, addrport, &address, &port)<0) {
    log_warn(LD_CONFIG, "Error parsing transport "
             "address '%s'", addrport);
    goto err;
...
{
  managed_proxy_t *mp = (managed_proxy_t *)tor_malloc_zero(sizeof(managed_proxy_t));
  mp->conf_state = PT_PROTO_INFANT;
  mp->is_server = is_server;
  mp->argv = proxy_argv;
