  ...
  if (!circuit_build_times_disabled()) {
    ...
    if (num > 0) {
      if (num != cbt->liveness.num_recent_circs) {
         ...
         */
        recent_circs = (int8_t *)tor_malloc_zero(sizeof(int8_t)*num);
...
circuit_build_times_init(circuit_build_times_t *cbt)
{
  ...
  if (!circuit_build_times_disabled()) {
    ...
    cbt->liveness.timeouts_after_firsthop =
      (int8_t *)tor_malloc_zero(sizeof(int8_t)*cbt->liveness.num_recent_circs);
  } else {
    cbt->liveness.num_recent_circs = 0;
                                     ...
                                     build_time_t *nbins)
{
  ...
  *nbins = 1 + (max_build_time / CBT_BIN_WIDTH);
  histogram = (uint32_t *)tor_malloc_zero(*nbins * sizeof(build_time_t));
                                ...
                                or_state_t *state)
{
  ...
  loaded_times = (build_time_t *)tor_malloc_zero(sizeof(build_time_t)*state->TotalBuildTimes);
  ...
  for (line = state->BuildtimeHistogram; line; line = line->next) {
    ...
    if (smartlist_len(args) < 2) {
      ...
      const char *ms_str = (const char *)smartlist_get(args,0);
      const char *count_str = (const char *)smartlist_get(args,1);
      uint32_t count, k;
      build_time_t ms;
      int ok;
