...
#define CHANNEL_LISTENER_NOT_CLOSING channel_listener_s::CHANNEL_LISTENER_NOT_CLOSING
#define CHANNEL_LISTENER_CLOSE_REQUESTED channel_listener_s::CHANNEL_LISTENER_CLOSE_REQUESTED
#define CHANNEL_LISTENER_CLOSE_FROM_BELOW channel_listener_s::CHANNEL_LISTENER_CLOSE_FROM_BELOW
#define CHANNEL_LISTENER_CLOSE_FOR_ERROR channel_listener_s::CHANNEL_LISTENER_CLOSE_FOR_ERROR

#define CHANNEL_NOT_CLOSING channel_s::CHANNEL_NOT_CLOSING
#define CHANNEL_CLOSE_REQUESTED channel_s::CHANNEL_CLOSE_REQUESTED
#define CHANNEL_CLOSE_FROM_BELOW channel_s::CHANNEL_CLOSE_FROM_BELOW
...
channel_add_to_digest_map(channel_t *chan)
{
  ...
  if (! ent) {
    ent = (channel_idmap_entry_t *)tor_malloc(sizeof(channel_idmap_entry_t));
...
{
  tor_assert(chan);
  // Fix me: Bug in close channel, might be related to libevent
  // tor_assert(chan->state == CHANNEL_STATE_CLOSING ||
  //            chan->state == CHANNEL_STATE_CLOSED ||
  //            chan->state == CHANNEL_STATE_ERROR);
...
cell_queue_entry_dup(cell_queue_entry_t *q)
{
  ...
  rv = (cell_queue_entry_t *)tor_malloc(sizeof(*rv));
...
cell_queue_entry_free(cell_queue_entry_t *q, int handed_off)
{
  ...
  if (!handed_off) {
     ...
     */
    switch (q->type) {
      case cell_queue_entry_s::CELL_QUEUE_FIXED:
        if (q->u.fixed.cell) {
          /*
        ...
        break;
      case cell_queue_entry_s::CELL_QUEUE_PACKED:
        if (q->u.packed.packed_cell) {
          packed_cell_free(q->u.packed.packed_cell);
        }
        break;
      case cell_queue_entry_s::CELL_QUEUE_VAR:
        if (q->u.var.var_cell) {
          /*
...
cell_queue_entry_new_fixed(cell_t *cell)
{
  ...
  q = (cell_queue_entry_t *)tor_malloc(sizeof(*q));
  q->type = cell_queue_entry_s::CELL_QUEUE_FIXED;
  q->u.fixed.cell = cell;
...
cell_queue_entry_new_var(var_cell_t *var_cell)
{
  ...
  q = (cell_queue_entry_t *)tor_malloc(sizeof(*q));
  q->type = cell_queue_entry_s::CELL_QUEUE_VAR;
...
channel_write_packed_cell(channel_t *chan, packed_cell_t *packed_cell)
{
  ...
  q.type = cell_queue_entry_s::CELL_QUEUE_PACKED;
...
channel_process_cells(channel_t *chan)
{
  ...
  while (NULL != (q = TOR_SIMPLEQ_FIRST(&chan->incoming_queue))) {
    tor_assert(q);
    tor_assert(q->type == cell_queue_entry_s::CELL_QUEUE_FIXED ||
               q->type == cell_queue_entry_s::CELL_QUEUE_VAR);

    if (q->type == cell_queue_entry_s::CELL_QUEUE_FIXED &&
        chan->cell_handler) {
      ...
      tor_free(q);
    } else if (q->type == cell_queue_entry_s::CELL_QUEUE_VAR &&
               chan->var_cell_handler) {
