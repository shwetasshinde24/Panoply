...
addressmap_virtaddress_remove(const char *address, addressmap_entry_t *ent)
{
  if (ent && ent->new_address &&
    ...
    virtaddress_entry_t *ve =
      (virtaddress_entry_t *)strmap_get(virtaddress_reversemap, ent->new_address);
...
addressmap_match_superdomains(char *address)
{
  ...
  while ((cp = strchr(cp, '.'))) {
    ...
    val = (addressmap_entry_t *)strmap_get_lc(addressmap, cp+1);
                   ...
                   addressmap_entry_source_t *exit_source_out)
{
  ...
  for (rewrites = 0; rewrites < 16; rewrites++) {
    ...
    ent = (addressmap_entry_t *)strmap_get(addressmap, address);
                           ...
                           time_t *expires_out)
{
  ...
  ent = (addressmap_entry_t *)strmap_get(addressmap, s);
  if (ent) {
...
{
  addressmap_entry_t *ent;
  if (!(ent=(addressmap_entry_t *)strmap_get_lc(addressmap, address)))
    return 0;
                    ...
                    const int wildcard_new_addr)
{
  ...
  if (!ent) { /* make a new one and register it */
    ent = (addressmap_entry_t *)tor_malloc_zero(sizeof(addressmap_entry_t));
...
{
  addressmap_entry_t *ent = (addressmap_entry_t *)strmap_get(addressmap, address);
  if (!ent) {
    ent = (addressmap_entry_t *)tor_malloc_zero(sizeof(addressmap_entry_t));
...
addressmap_register_virtual_address(int type, char *new_address)
{
  ...
  vent = (virtaddress_entry_t *)strmap_get(virtaddress_reversemap, new_address);
  if (!vent) {
    vent = (virtaddress_entry_t *)tor_malloc_zero(sizeof(virtaddress_entry_t));
  ...
  if (*addrp) {
    addressmap_entry_t *ent = (addressmap_entry_t *)strmap_get(addressmap, *addrp);
    if (ent && ent->new_address &&
