                               ...
                               channel_t *chan)
{
  ...
  *circid_ptr = id;
  ...
  if (found) {
    ...
    found = (chan_circid_circuit_map_t *)tor_malloc_zero(sizeof(chan_circid_circuit_map_t));
    found->circ_id = id;
    found->chan = chan;
    found->circuit = circ;
  ...
  if (chan && id != 0 && (old_chan != chan || old_id != id) &&
    ...
    circuitmux_attach_circuit(chan->cmux, circ, (cell_direction_t)direction);
    attached = 1;
  }
  ...
  if (make_active && attached)
    update_circuit_on_cmux(circ, (cell_direction_t)direction);
...
channel_mark_circid_unusable(channel_t *chan, circid_t id)
{
  ...
  if (ent && ent->circuit) {
    ...
    ent = (chan_circid_circuit_map_t *)tor_malloc_zero(sizeof(chan_circid_circuit_map_t));
    ent->chan = chan;
    ent->circ_id = id;
...
origin_circuit_new(void)
{
  ...
  circ = (origin_circuit_t *)tor_malloc_zero(sizeof(origin_circuit_t));
...
or_circuit_new(circid_t p_circ_id, channel_t *p_chan)
{
  ...
  circ = (or_circuit_t *)tor_malloc_zero(sizeof(or_circuit_t));
...
/* #define DEBUG_CIRCUIT_UNLINK_ALL */
  ...
  SMARTLIST_FOREACH_BEGIN(detached, circuit_t *, circ) {
    ...
    if (circ->n_chan == chan) {
      ...
      if (chan->reason_for_closing != channel_s::CHANNEL_CLOSE_REQUESTED)
                                      ...
                                      const char *token)
{
  ...
  circ = (or_circuit_t *)digestmap_get(map, token);
  if (!circ ||
...
circuit_clear_rend_token(or_circuit_t *circ)
{
  ...
  found_circ = (or_circuit_t *)digestmap_get(map, circ->rendinfo->rend_token);
  if (found_circ == circ) {
                       ...
                       const uint8_t *token)
{
  ...
  found_circ = (or_circuit_t *)digestmap_get(map, (const char *)token);
  if (found_circ) {
  ...
  circ->rendinfo = (or_circuit_rendinfo_s *)tor_malloc(sizeof(*circ->rendinfo));
...
{
  const circuit_t *a = (const circuit_t *)*a_;
  const circuit_t *b = (const circuit_t *)*b_;
