...
{
  dir_connection_t *dir_conn = (dir_connection_t *)tor_malloc_zero(sizeof(dir_connection_t));
...
{
  or_connection_t *or_conn = (or_connection_t *)tor_malloc_zero(sizeof(or_connection_t));
...
{
  entry_connection_t *entry_conn = (entry_connection_t *)tor_malloc_zero(sizeof(entry_connection_t));
...
{
  edge_connection_t *edge_conn = (edge_connection_t *)tor_malloc_zero(sizeof(edge_connection_t));
...
control_connection_new(int socket_family)
{
  control_connection_t *control_conn =
    (control_connection_t *) tor_malloc_zero(sizeof(control_connection_t));
...
listener_connection_new(int type, int socket_family)
{
  listener_connection_t *listener_conn =
    (listener_connection_t *) tor_malloc_zero(sizeof(listener_connection_t));
...
connection_new(int type, int socket_family)
{
  switch (type) {
    ...
    default: {
      connection_t *conn = (connection_t *)tor_malloc_zero(sizeof(connection_t));
                     ...
                     socklen_t *len_out)
{
  ...
  sockaddr = (struct sockaddr_un *)tor_malloc_zero(sizeof(struct sockaddr_un));
                        ...
                        const port_cfg_t *port_cfg)
{
  ...
  if (listensockaddr->sa_family == AF_INET ||
    ...
    if (options->TransProxyType_parsed == or_options_t::TPT_TPROXY &&
...
check_sockaddr(const struct sockaddr *sa, int len, int level)
{
  ...
  if (sa->sa_family == AF_INET) {
    ...
    if (tor_mem_is_zero((const char*)sin6->sin6_addr.s6_addr, 16) ||
        sin6->sin6_port == 0) {
      log_fn(level, LD_NET,
             "Address for new connection has address/port equal to zero.");
      ok = 0;
...
{
  printf("connection_handle_listener_read \n");
                   ...
                   const tor_addr_t *addr, uint16_t port, int *socket_error)
{
  ...
  // Fix me: Need to open non-blocking socket, but non-blocking is buggy
...
connection_proxy_connect(connection_t *conn, int type)
{
  ...
  switch (type) {
    ...
    case PROXY_SOCKS4: {
      ...
      buf = (unsigned char *)tor_malloc_zero(buf_size);
                     ...
                     int control_listeners_only)
{
  ...
  SMARTLIST_FOREACH_BEGIN(launch, const port_cfg_t *, port) {
    ...
    if (port->is_unix_addr) {
      ...
      listensockaddr = (struct sockaddr *)tor_malloc(sizeof(struct sockaddr_storage));
...
alloc_http_authenticator(const char *authenticator)
{
  ...
  char *base64_authenticator = (char *)tor_malloc(base64_authenticator_length);
...
client_check_address_changed(tor_socket_t sock)
{
  ...
  if (! *last_interface_ip_ptr) {
    tor_addr_t *a = (tor_addr_t *)tor_malloc_zero(sizeof(tor_addr_t));
