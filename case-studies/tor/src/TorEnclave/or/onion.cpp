...
onion_pending_add(or_circuit_t *circ, create_cell_t *onionskin)
{
  ...
  tmp = (onion_queue_t *)tor_malloc_zero(sizeof(onion_queue_t));
  tmp->circ = circ;
...
setup_server_onion_keys(server_onion_keys_t *keys)
{
  ...
  keys->junk_keypair = (curve25519_keypair_t*)tor_malloc_zero(sizeof(curve25519_keypair_t));
                      ...
                      uint8_t *rend_nonce_out)
{
  ...
  case ONION_HANDSHAKE_TYPE_NTOR:
      ...
      return -1;
    {
      ...
      uint8_t *keys_tmp = (uint8_t *)tor_malloc(keys_out_len + DIGEST_LEN);
                      ...
                      uint8_t *rend_authenticator_out)
{
  ...
  case ONION_HANDSHAKE_TYPE_NTOR:
    ...
    {
      size_t keys_tmp_len = keys_out_len + DIGEST_LEN;
      uint8_t *keys_tmp = (uint8_t *)tor_malloc(keys_tmp_len);
