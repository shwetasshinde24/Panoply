...
ext_or_cmd_new(uint16_t len)
{
  ...
  ext_or_cmd_t *cmd = (ext_or_cmd_t *)tor_malloc(size);
  cmd->len = len;
  return cmd;
}
                         ...
                         char **reply_out, size_t *reply_len_out)
{
  ...
  { /* set up macs */
    ...
    char *hmac_s_msg = (char *)tor_malloc_zero(hmac_s_msg_len);
    char *hmac_c_msg = (char *)tor_malloc_zero(hmac_c_msg_len);
    char *correct_client_hash = (char *)tor_malloc_zero(EXT_OR_PORT_AUTH_HASH_LEN);
  ...
  { /* write reply: (server_hash, server_nonce) */
    ...
    reply = (char *)tor_malloc_zero(reply_len);
                                      ...
                                      const char *payload, uint16_t len)
{
  ...
  addr_str = (char *)tor_memdup_nulterm(payload, len);
                                       ...
                                       const char *payload, uint16_t len)
{
  ...
  transport_str = (char *)tor_memdup_nulterm(payload, len);
