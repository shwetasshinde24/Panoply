  ...
  *errcode_out = 0;
  ...
  while (remaining) {
    ...
    if (answer_type == RESOLVED_TYPE_IPV4) {
      ...
      addr = (address_ttl_t *)tor_malloc_zero(sizeof(*addr));
      ...
      addr = (address_ttl_t *)tor_malloc_zero(sizeof(*addr));
      addr->hostname = (char *)tor_memdup_nulterm(cp, answer_len);
      cp += answer_len;
      addr->ttl = ntohl(get_uint32(cp));
      cp += 4;
      smartlist_add(addrs, addr);
...
#define relay_alloc_cell() \
  (packed_cell_t*) tor_malloc_zero(sizeof(packed_cell_t))
#define relay_free_cell(cell) \
  tor_free(cell)
    ...
    if (get_options()->CellStatistics ||
      ...
      if (get_options()->TestingEnableCellStatsEvent) {
        ...
        testing_cell_stats_entry_t *ent =
          (testing_cell_stats_entry_t *)tor_malloc_zero(sizeof(testing_cell_stats_entry_t));
        ent->command = command;
