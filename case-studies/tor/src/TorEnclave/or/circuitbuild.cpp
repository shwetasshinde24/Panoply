...
circuit_list_path_impl(origin_circuit_t *circ, int verbose, int verbose_names)
{
  ...
  do {
    ...
    if (verbose_names) {
      elt = (char *)tor_malloc(MAX_VERBOSE_NICKNAME_LEN+1);
      ...
      if (node && node_is_named(node)) {
        ...
        elt = (char *)tor_malloc(HEX_DIGEST_LEN+2);
        elt[0] = '$';
        base16_encode(elt+1, HEX_DIGEST_LEN+1, id, DIGEST_LEN);
      }
    }
...
origin_circuit_init(uint8_t purpose, int flags)
{
  ...
  circ->build_state = (cpath_build_state_t *)tor_malloc_zero(sizeof(cpath_build_state_t));
...
circuit_extend(cell_t *cell, circuit_t *circ)
{
  ...
  if (!n_chan) {
    ...
    circ->n_chan_create_cell = (create_cell_t *)tor_memdup(&ec.create_cell,
                 ...
                 const uint8_t *rend_circ_nonce)
{
  ...
  tmp_cpath = (crypt_path_t *)tor_malloc_zero(sizeof(crypt_path_t));
                                    ...
                                    int *need_capacity)
{
  ...
  *need_capacity = 1;
  ...
  for (i = 0; i < smartlist_len(sl); ++i) {
    port = (uint16_t *)smartlist_get(sl, i);
...
//         n_pending_connections);
  ...
  n_supported = (int *)tor_malloc(sizeof(int)*smartlist_len(the_nodes));
                        ...
                        int need_uptime, int need_capacity, int is_internal)
{
  ...
  if (need_uptime)
    flags = (router_crn_flags_t)(flags | CRN_NEED_UPTIME);
  if (need_capacity)
    flags = (router_crn_flags_t)(flags | CRN_NEED_CAPACITY);
...
choose_good_entry_server(uint8_t purpose, cpath_build_state_t *state)
{
  ...
  smartlist_t *excluded;
  const or_options_t *options = get_options();
  router_crn_flags_t flags = (router_crn_flags_t)(CRN_NEED_GUARD|CRN_NEED_DESC);
  const node_t *node;
...
{
  crypt_path_t *hop = (crypt_path_t *)tor_malloc_zero(sizeof(crypt_path_t));
...
{
  extend_info_t *info = (extend_info_t *)tor_malloc_zero(sizeof(extend_info_t));
...
extend_info_dup(extend_info_t *info)
{
  ...
  tor_assert(info);
  newinfo = (extend_info_t *)tor_malloc(sizeof(extend_info_t));
