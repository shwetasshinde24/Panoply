...
{
  curve25519_keypair_t *k = (curve25519_keypair_t *)arg;
...
init_keys(void)
{
  ...
  type = (dirinfo_type_t)((options->V3AuthoritativeDir ?
...
router_rebuild_descriptor(int force)
{
  ...
  ri = (routerinfo_t *)tor_malloc_zero(sizeof(routerinfo_t));
  ...
  ri->onion_curve25519_pkey =
    (curve25519_public_key_t *)tor_memdup(&get_current_curve25519_keypair()->pubkey,
  ...
  if (options->MyFamily && ! options->BridgeRelay) {
    ...
    SMARTLIST_FOREACH_BEGIN(family, char *, name) {
       ...
       if (!member) {
         ...
         char *fp = (char *)tor_malloc(HEX_DIGEST_LEN+2);
         fp[0] = '$';
  ...
  ei = (extrainfo_t *)tor_malloc_zero(sizeof(extrainfo_t));
                                  ...
                                  int include_ipv6)
{
  ...
  SMARTLIST_FOREACH_BEGIN(router->exit_policy, addr_policy_t *, tmpe) {
    ...
    pbuf = (char *)tor_malloc(POLICY_BUF_LEN);
  ...
  switch (file_status(fname)) {
    case FN_FILE:
      ...
      if ((contents = read_file_to_str(fname, 0, NULL))) {
        tmp = (char *)strstr(contents, end_line);
        /* Find last block starting with end_line */
        while (tmp) {
          start = tmp;
          tmp = (char *)strstr(tmp + 1, end_line);
        }
        if (!start)
          goto notfound;
                         ...
                         crypto_pk_t *ident_key)
{
  ...
  while (strlen(s) > MAX_EXTRAINFO_UPLOAD_SIZE - DIROBJ_MAX_SIG_LEN) {
    ...
    if (smartlist_len(chunks) > 2) {
      ...
      char *e = (char *)smartlist_get(chunks, idx);
