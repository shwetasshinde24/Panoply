...
tds_connect(TDSSOCKET * tds, TDSLOGIN * login, int *p_oserr)
{
	...
	for (addrs = login->ip_addrs; addrs != NULL; addrs = (struct addrinfo*)addrs->ai_next) {
	...
	if (login->text_size || (!db_selected && !tds_dstr_isempty(&login->database))
		...
		str[0] = 0;
		if (login->text_size) {
			snprintf(str, len, "set textsize %d ", login->text_size);
		}
		if (tds->conn->spid == -1) {
...
// tds_connect(TDSSOCKET * tds, TDSLOGIN * login, int *p_oserr)	
// {
// 	sgx_wrapper_fprintf(SGX_STDERR, "Dummy login.c tds_connect call \n");
// 	return 1;
// }
...
tds_send_login(TDSSOCKET * tds, TDSLOGIN * login)
{
	...
	snprintf(blockstr, 16, "%d", (int) sgx_thread_self());
	...
	if (login->block_size < 65536u && login->block_size >= 512)
		snprintf(blockstr, 16, "%d", login->block_size);
	else
		strncpy(blockstr, "512", 3);
...
#define SET_FIELD_DSTR(field, dstr) do { \
	...
	tds_put_int(tds, sgx_thread_self());	/* process id of this process */
...
#define START_POS 26
	...
	/* pid */
	tds_put_int(tds, sgx_thread_self());
	/* MARS (1 enabled) */
	if (IS_TDS72_PLUS(tds->conn))
#if ENABLE_ODBC_MARS
