...
test0(const char *src, int len, int midtype, int dsttype, const char *result, int line)
{
	...
	if (res < 0)
		...
		case SYBINT1:
		case SYBUINT1:
			snprintf(buf, 256, "%d", cr.ti);
			break;
		case SYBINT2:
			snprintf(buf, 256, "%d", cr.si);
			break;
		case SYBUINT2:
			snprintf(buf, 256, "%u", cr.usi);
			break;
		case SYBINT4:
			snprintf(buf, 256, "%d", cr.i);
			break;
		case SYBUINT4:
			snprintf(buf, 256, "%u", cr.ui);
			break;
		case SYBINT8:
			snprintf(buf, 256, "0x%08x%08x", (unsigned int) ((cr.bi >> 32) & 0xfffffffflu), (unsigned int) (cr.bi & 0xfffffffflu));
			break;
		case SYB5BIGTIME:
		case SYB5BIGDATETIME:
		case SYBUINT8:
			snprintf(buf, 256, "0x%08x%08x", (unsigned int) ((cr.ubi >> 32) & 0xfffffffflu), (unsigned int) (cr.ubi & 0xfffffffflu));
			break;
		case SYBUNIQUE:
			snprintf(buf, 256, "%08X-%04X-%04X-%02X%02X%02X%02X"
				"%02X%02X%02X%02X",
				cr.u.Data1,
			...
			break;
		case SYBBINARY:
			snprintf(buf, 256, "len=%d", res);
			for (i = 0; i < res; ++i)
				snprintf(strchr(buf, 0), 256, " %02X", (TDS_UCHAR) cr.ib[i]);
			free(cr.ib);
			break;
		case SYBCHAR:
			snprintf(buf, 256, "len=%d %s", res, cr.c);
			free(cr.c);
			break;
		case SYBDATETIME:
			snprintf(buf, 256, "%ld %ld", (long int) cr.dt.dtdays, (long int) cr.dt.dttime);
			break;
		case SYBDATE:
			snprintf(buf, 256, "%ld", (long int) cr.date);
			break;
		case SYBTIME:
			snprintf(buf, 256, "%ld", (long int) cr.time);
			break;
		}
	}
	printf("%s\n", buf);
...
int
ecall_unittest_t0007_main(int argc, char **argv)
{
	int *type1, *type2;
	const char **value;
	int big_endian = 1;
	...
	for (type2 = int_types; *type2 >= 0; ++type2) {
		...
		if (*type2 == SYBMONEY4 || *type2 == SYBMONEY)
			snprintf(expected, 64, "%s.00", *value);
		else
