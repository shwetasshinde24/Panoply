...
static inline int isatty(int n)	{
	return 1;
}
...
get_coll_md5(const char *name, char *digest)
{
	...
	MD5Final(&ctx, dig);
	for (i = 0; i < 16; ++i)
		snprintf(digest + i * 2, 1000, "%02x", dig[i]);
}
...
get_encoding_coll(TDS71_COLLATION *coll, char *digest, char *cp, const char *name)
{
	...
	snprintf(sql, 512, "SELECT CAST(CAST('a' AS NVARCHAR(10)) COLLATE %s AS VARCHAR(10)) COLLATE %s", name, name);
...
{
	SGX_WRAPPER_FILE f = fopen("collations.txt", "r");
	char line[1024];
...
add_couple(unsigned n)
{
	...
	snprintf(buf + cnt * 4, CHUNK * 4+1, "%02x%02x", n & 0xff, (n >> 8) & 0xff);
...
int
ecall_unittest_collations_main(int argc, char **argv)
{
	TDSLOGIN *login;
	int ret;
	int verbose = 1;
