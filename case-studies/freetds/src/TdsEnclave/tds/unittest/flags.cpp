...
int
ecall_unittest_flags_main(int argc, char **argv)
{
	TDSRESULTINFO *info;
	char mymsg[256];
	...
	if (!IS_TDS42(tds->conn)) {
		...
		if (info->num_cols != 3) {
			snprintf(mymsg, 256,"wrong number of columns returned expected 3 got %d", info->num_cols);
			fatal_error(mymsg);
		}
