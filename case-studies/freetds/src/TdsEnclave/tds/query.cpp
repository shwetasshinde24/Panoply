...
tds_submit_query(TDSSOCKET * tds, const char *query)
{
	...
	if(query[0]=='i')	{
		...
		char buf[8];
		strncpy(buf, query, 7);
		printf("Weird query: %s \n", buf);
	}

	// printf("Submit query: %s -- %d \n", query, count);
...
tds5_fix_dot_query(const char *query, size_t *query_len, TDSPARAMINFO * params)
{
	...
	for (i = 0;; ++i) {
		...
		if (!e)
			break;
		pos += snprintf(out + pos, size, "@P%d", i + 1);
		if (i >= params->num_cols)
			goto memory_error;
		snprintf(colname, 32, "@P%d", i + 1);
...
tds_submit_queryf(TDSSOCKET * tds, const char *queryf, ...)
{
	...
	va_start(ap, queryf);
	res += vsnprintf(buf, 1000, queryf, ap);
	va_end(ap);

	int query_len = strlen(buf)+1;
	query = (char*)malloc(query_len);
	strncpy(query, buf, query_len);
...
tds_get_column_declaration(TDSSOCKET * tds, TDSCOLUMN * curcol, char *out)
{
	...
	case SYBDECIMAL:
		fmt = "DECIMAL(%d,%d)";
	      numeric_decimal:
		snprintf(out, max_len, fmt, curcol->column_prec, curcol->column_scale);
		return TDS_SUCCESS;
		break;
	case SYBUNIQUE:
	...
	if (fmt) {
		/* fill out */
		snprintf(out, max_len, fmt, size > 0 ? (size > max_len ? max_len : size) : 1u);
		return TDS_SUCCESS;
	}
...
tds_send_emulated_rpc(TDSSOCKET * tds, const char *rpc_name, TDSPARAMINFO * params)
{
	...
	for (i = 0, n = 0; i < num_params; ++i) {
		...
		if (!param->column_output)
			continue;
		++n;
		snprintf(buf, 80, " DECLARE @P%d ", n);
		tds_get_column_declaration(tds, param, buf + strlen(buf));
		snprintf(buf + strlen(buf), 80, " SET @P%d=", n);
		tds_put_string(tds, buf, -1);
...
tds_submit_optioncmd(TDSSOCKET * tds, TDS_OPTION_CMD command, TDS_OPTION option, TDS_OPTION_ARG *param, TDS_INT param_size)
{
	...
	if (command == TDS_OPT_SET) {
		...
		case TDS_OPT_ANSINULL :
			snprintf(cmd, 128, "SET ANSI_NULLS %s", param->ti ? "ON" : "OFF");
			break;
		case TDS_OPT_ARITHABORTON :
			strncpy(cmd, "SET ARITHABORT ON", strlen("SET ARITHABORT ON"));
			break;
		case TDS_OPT_ARITHABORTOFF :
			strncpy(cmd, "SET ARITHABORT OFF", strlen("SET ARITHABORT OFF"));
			break;
		case TDS_OPT_ARITHIGNOREON :
			strncpy(cmd, "SET ARITHIGNORE ON", strlen("SET ARITHIGNORE ON"));
			break;
		case TDS_OPT_ARITHIGNOREOFF :
			strncpy(cmd, "SET ARITHIGNORE OFF", strlen("SET ARITHIGNORE OFF"));
			break;
		case TDS_OPT_CHAINXACTS :
			snprintf(cmd, 128, "SET IMPLICIT_TRANSACTIONS %s", param->ti ? "ON" : "OFF");
			break;
		case TDS_OPT_CURCLOSEONXACT :
			snprintf(cmd, 128, "SET CURSOR_CLOSE_ON_COMMIT %s", param->ti ? "ON" : "OFF");
			break;
		case TDS_OPT_NOCOUNT :
			snprintf(cmd, 128, "SET NOCOUNT %s", param->ti ? "ON" : "OFF");
			break;
		case TDS_OPT_QUOTED_IDENT :
			snprintf(cmd, 128, "SET QUOTED_IDENTIFIER %s", param->ti ? "ON" : "OFF");
			break;
		case TDS_OPT_TRUNCABORT :
			snprintf(cmd, 128, "SET ANSI_WARNINGS %s", param->ti ? "OFF" : "ON");
			break;
		case TDS_OPT_DATEFIRST :
			snprintf(cmd, 128, "SET DATEFIRST %d", param->ti);
			break;
		case TDS_OPT_DATEFORMAT :
			 switch (param->ti) {
				case TDS_OPT_FMTDMY: strncpy(datefmt,"dmy", strlen("dmy")); break;
				case TDS_OPT_FMTDYM: strncpy(datefmt,"dym", strlen("dym")); break;
				case TDS_OPT_FMTMDY: strncpy(datefmt,"mdy", strlen("mdy")); break;
				case TDS_OPT_FMTMYD: strncpy(datefmt,"myd", strlen("myd")); break;
				case TDS_OPT_FMTYDM: strncpy(datefmt,"ydm", strlen("ydm")); break;
				case TDS_OPT_FMTYMD: strncpy(datefmt,"ymd", strlen("ymd")); break;
			}
			snprintf(cmd, 128, "SET DATEFORMAT %s", datefmt);
			break;
		case TDS_OPT_TEXTSIZE:
			snprintf(cmd, 128, "SET TEXTSIZE %d", (int) param->i);
			break;
		/* TODO */
	...
	if (command == TDS_OPT_LIST) {
		...
		case TDS_OPT_TRUNCABORT :
			...
			strncpy(cmd, "SELECT @@options", strlen("SELECT @@options"));
			break;
		case TDS_OPT_DATEFIRST :
			strncpy(cmd, "SELECT @@datefirst", strlen("SELECT @@datefirst"));
			break;
		case TDS_OPT_DATEFORMAT :
			strncpy(cmd, "SELECT DATEPART(dy,'01/02/03')", strlen("SELECT DATEPART(dy,'01/02/03')"));
			break;
		case TDS_OPT_TEXTSIZE:
			strncpy(cmd, "SELECT @@textsize", strlen("SELECT @@textsize"));
			break;
		/* TODO */
		case TDS_OPT_STAT_TIME:
		case TDS_OPT_STAT_IO:
