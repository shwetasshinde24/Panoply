...
tds_bcp_start_insert_stmt(TDSSOCKET * tds, TDSBCPINFO * bcpinfo)
{
	...
	if (IS_TDS7_PLUS(tds->conn)) {
		...
		if (bcpinfo->hint) {
			hint = (char *)malloc(BUFFSIZ);
			if (snprintf(hint, BUFFSIZ, " with (%s)", bcpinfo->hint) < 0)
				hint = NULL;
		} else {
			hint = strdup("");
		}
		...
		query = (char *)malloc(BUFFSIZ);
		erc = snprintf(query, BUFFSIZ, "insert bulk %s (%s)%s", tds_dstr_cstr(&bcpinfo->tablename), colclause.pb, hint);
		...
		query = (char *)malloc(BUFFSIZ);
		if (snprintf(query, BUFFSIZ, "insert bulk %s", tds_dstr_cstr(&bcpinfo->tablename)) < 0)
			return TDS_FAIL;
	}

	printf("Do query: %s", query);
 ...
 */
TDSRET
tds_bcp_fread(TDSSOCKET * tds, TDSICONV * char_conv, SGX_WRAPPER_FILE stream, const char *terminator, size_t term_len, char **outbuf, size_t * outbytes)
{
	TDSRET res;
	TDSFILESTREAM r;
	TDSDYNAMICSTREAM w;
	...
	ocall_fread(&readed, r.left, 1, term_len, stream);
	if (readed != term_len) {
		free(r.left);
