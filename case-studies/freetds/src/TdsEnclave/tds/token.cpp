...
tds7_get_data_info(TDSSOCKET * tds, TDSCOLUMN * curcol)
{
	...
	// printf("call function pointer get_info with pointer %p \n", curcol->funcs); 
	curcol->funcs->get_info(tds, curcol);
	...
	// printf("call adjust_character_column_size get_info \n"); 
	adjust_character_column_size(tds, curcol);
...
{
	// printf("Call tds7_process_result \n ");
	int col, num_cols;
	TDSRET result;
	TDSRESULTINFO *info;

	CHECK_TDS_EXTRA(tds);
	// printf("processing TDS7 result metadata \n ");
...
tds_process_info(TDSSOCKET * tds, int marker)
{
	...
	if ((!msg.server || !msg.server[0]) && tds->login) {
		...
		if (-1 == sgx_wrapper_asprintf(&msg.server, "[%s]", tds_dstr_cstr(&tds->login->server_name))) {
