...
get_default_instance_port(const char hostname[])
{
    ...
    printf("Test default get instance port for host: %s\n", hostname);
    printf("%p :\n", &addr);

    TdsEnclave_ecall_tds_lookup_host(global_eid, (void**)&addr, hostname);

    if ((addr == NULL))
        return 0;

    printf("Print out canonname: %s \n", addr->ai_canonname);
    printf("Port: %d \n", port);

    TdsEnclave_ecall_tds7_get_instance_port(global_eid, &port, addr, "MSSQLSERVER");

    ocall_freeaddrinfo(addr);
...
int SGX_CDECL main(int argc, char *argv[])
{
    ...
    int eret;
    sgx_status_t ret;
    ret = TdsEnclave_ecall_set_communication_enclave_ids(global_eid, global_eid, vrfcert_eid);
    ...
    ret = TdsEnclave_ecall_set_enclave_id(global_eid, global_eid);
    ...
    // ret = TdsEnclave_ecall_tsql_main(global_eid, &eret, argc, argv);
    // TdsEnclave_ecall_unittest_charconv_main(global_eid, &eret, argc, argv);
    // TdsEnclave_ecall_unittest_iconv_fread_main(global_eid, &eret, argc, argv);
    // TdsEnclave_ecall_unittest_challenge_main(global_eid, &eret);
    // TdsEnclave_ecall_unittest_convert_main(global_eid, &eret, argc, argv);

    // TdsEnclave_ecall_unittest_corrupt_main(global_eid, &eret, argc, argv);
    // TdsEnclave_ecall_unittest_dataread_main(global_eid, &eret, argc, argv);
    // TdsEnclave_ecall_unittest_dynamic1_main(global_eid, &eret, argc, argv);
    // TdsEnclave_ecall_unittest_flags_main(global_eid, &eret, argc, argv);
    // TdsEnclave_ecall_unittest_nulls_main(global_eid, &eret, argc, argv);
    // TdsEnclave_ecall_unittest_numeric_main(global_eid, &eret, argc, argv);
    // TdsEnclave_ecall_unittest_t0007_main(global_eid, &eret, argc, argv);
    // TdsEnclave_ecall_unittest_t0008_main(global_eid, &eret, argc, argv);

    // TdsEnclave_ecall_unittest_t0001_main(global_eid, &eret, argc, argv);
    // TdsEnclave_ecall_unittest_testweof_main(global_eid, &eret, argc, argv);
    // TdsEnclave_ecall_unittest_testchangelist_main(global_eid, &eret, argc, argv);
    // TdsEnclave_ecall_unittest_leproxy_main(global_eid, &eret, argc, argv);
    // TdsEnclave_ecall_unittest_regress_main(global_eid, &eret, argc, argv);
     // TdsEnclave_ecall_unittest_bench_main(global_eid, &eret, argc, argv);

    TdsEnclave_ecall_unittest_collations_main(global_eid, &eret, argc, argv);
