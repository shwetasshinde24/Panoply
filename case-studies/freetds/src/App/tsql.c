...
tsql_readline(char *prompt)
{
	...
	for (;;) {
		/* read a piece */
		if (sgx_wrapper_fgets(line + pos, (int)(sz - pos), SGX_STDIN) == NULL) {
			if (pos)
				return line;
			break;
		}
...
tds_version(TDSCONNECTION * conn, char *pversion_string)
{
	...
	sgx_wrapper_sprintf(pversion_string, "%d.%d", TDS_MAJOR(conn), TDS_MINOR(conn));
	...
	while ((rc = tds_process_tokens(tds, &resulttype, NULL, TDS_TOKEN_RESULTS)) == TDS_SUCCESS) {
		...
		case TDS_ROWFMT_RESULT:
			if ((!(opt_flags & OPT_NOHEADER)) && tds->current_results) {
				for (i = 0; i < tds->current_results->num_cols; i++) {
					if (i) sgx_wrapper_fputs(opt_col_term, SGX_STDOUT);
					sgx_wrapper_fputs(tds_dstr_cstr(&tds->current_results->columns[i]->column_name), SGX_STDOUT);
				}
				sgx_wrapper_fputs(opt_row_term, SGX_STDOUT);
			}
			break;
			...
			while ((rc = tds_process_tokens(tds, &resulttype, NULL, stop_mask)) == TDS_SUCCESS) {
				...
				for (i = 0; i < tds->current_results->num_cols; i++) {
					...
					if (col->column_cur_size < 0) {
						if (print_rows)  {
							if (i) sgx_wrapper_fputs(opt_col_term, SGX_STDOUT);
							sgx_wrapper_fputs("NULL", SGX_STDOUT);
						}
						continue;
					}
					...
					if (print_rows)  {
						if (i) sgx_wrapper_fputs(opt_col_term, SGX_STDOUT);
						sgx_wrapper_fputs(dres.c, SGX_STDOUT);
					}
					free(dres.c);
				}
				if (print_rows)
		...
		if (opt_flags & OPT_VERSION) {
			...
			if (line) {
				...
				msg.server = "tsql";
				sgx_wrapper_sprintf(message, "using TDS version %s", version);
				msg.message = message;
		...
		if (opt_flags & OPT_TIMER) {
			...
			sgx_wrapper_sprintf(message, "Total time for processing %d rows: %ld msecs\n",
...
populate_login(TDSLOGIN * login, int argc, char **argv)
{
	...
	if (!opt_col_term) {
		sgx_wrapper_fprintf(SGX_STDERR, "%s: missing delimiter for -t (check escaping)\n", argv[0]);
		exit(1);
	}
	if (!opt_row_term) {
		sgx_wrapper_fprintf(SGX_STDERR, "%s: missing delimiter for -r (check escaping)\n", argv[0]);
		exit(1);
	}
...
slurp_input_file(char *fname, char **mybuf, size_t *bufsz, size_t *buflen, int *line)
{
	...
	if ((fp = sgx_wrapper_fopen(fname, "r")) <=0 ) {
		sgx_wrapper_fprintf(SGX_STDERR, "Unable to open input file '%s': %s\n", fname, strerror(errno));
		return;
	}
	while ((s = sgx_wrapper_fgets(linebuf, sizeof(linebuf), fp)) != NULL) {
...
main(int argc, char **argv)
{
	...
	for (s=NULL, s2=NULL; ; free(s), free(s2), s2=NULL) {
		sgx_wrapper_sprintf(prompt, "%d> ", ++line);
