...
static sgx_status_t SGX_CDECL Enclave3_session_request_ocall(void* pms)
{
...
static sgx_status_t SGX_CDECL Enclave3_exchange_report_ocall(void* pms)
{
...
static sgx_status_t SGX_CDECL Enclave3_send_request_ocall(void* pms)
{
...
static sgx_status_t SGX_CDECL Enclave3_end_session_ocall(void* pms)
{
...
static sgx_status_t SGX_CDECL Enclave3_sgx_oc_cpuidex(void* pms)
{
...
static sgx_status_t SGX_CDECL Enclave3_sgx_thread_wait_untrusted_event_ocall(void* pms)
{
...
static sgx_status_t SGX_CDECL Enclave3_sgx_thread_set_untrusted_event_ocall(void* pms)
{
...
static sgx_status_t SGX_CDECL Enclave3_sgx_thread_setwait_untrusted_events_ocall(void* pms)
{
...
static sgx_status_t SGX_CDECL Enclave3_sgx_thread_set_multiple_untrusted_events_ocall(void* pms)
{
...
static const struct {
	size_t nr_ocall;
	void * table[9];
} ocall_table_Enclave3 = {
	9,
	{
		(void*)Enclave3_session_request_ocall,
		(void*)Enclave3_exchange_report_ocall,
		(void*)Enclave3_send_request_ocall,
		(void*)Enclave3_end_session_ocall,
		(void*)Enclave3_sgx_oc_cpuidex,
		(void*)Enclave3_sgx_thread_wait_untrusted_event_ocall,
		(void*)Enclave3_sgx_thread_set_untrusted_event_ocall,
		(void*)Enclave3_sgx_thread_setwait_untrusted_events_ocall,
		(void*)Enclave3_sgx_thread_set_multiple_untrusted_events_ocall,
	}
};
sgx_status_t Enclave3_test_create_session(sgx_enclave_id_t eid, uint32_t* retval, sgx_enclave_id_t src_enclave_id, sgx_enclave_id_t dest_enclave_id)
{
	...
	status = sgx_ecall(eid, 0, &ocall_table_Enclave3, &ms);
...
sgx_status_t Enclave3_test_enclave_to_enclave_call(sgx_enclave_id_t eid, uint32_t* retval, sgx_enclave_id_t src_enclave_id, sgx_enclave_id_t dest_enclave_id)
{
	...
	ms.ms_src_enclave_id = src_enclave_id;
	ms.ms_dest_enclave_id = dest_enclave_id;
	status = sgx_ecall(eid, 1, &ocall_table_Enclave3, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}
...
sgx_status_t Enclave3_test_message_exchange(sgx_enclave_id_t eid, uint32_t* retval, sgx_enclave_id_t src_enclave_id, sgx_enclave_id_t dest_enclave_id)
{
	...
	ms.ms_src_enclave_id = src_enclave_id;
	ms.ms_dest_enclave_id = dest_enclave_id;
	status = sgx_ecall(eid, 2, &ocall_table_Enclave3, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}
...
sgx_status_t Enclave3_test_close_session(sgx_enclave_id_t eid, uint32_t* retval, sgx_enclave_id_t src_enclave_id, sgx_enclave_id_t dest_enclave_id)
{
	...
	ms.ms_src_enclave_id = src_enclave_id;
	ms.ms_dest_enclave_id = dest_enclave_id;
	status = sgx_ecall(eid, 3, &ocall_table_Enclave3, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}
...
sgx_status_t Enclave3_session_request(sgx_enclave_id_t eid, uint32_t* retval, sgx_enclave_id_t src_enclave_id, sgx_dh_msg1_t* dh_msg1, uint32_t* session_id)
{
	...
	status = sgx_ecall(eid, 4, &ocall_table_Enclave3, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}
...
sgx_status_t Enclave3_exchange_report(sgx_enclave_id_t eid, uint32_t* retval, sgx_enclave_id_t src_enclave_id, sgx_dh_msg2_t* dh_msg2, sgx_dh_msg3_t* dh_msg3, uint32_t session_id)
{
	...
	ms.ms_session_id = session_id;
	status = sgx_ecall(eid, 5, &ocall_table_Enclave3, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}
...
sgx_status_t Enclave3_generate_response(sgx_enclave_id_t eid, uint32_t* retval, sgx_enclave_id_t src_enclave_id, secure_message_t* req_message, size_t req_message_size, size_t max_payload_size, secure_message_t* resp_message, size_t resp_message_size)
{
	...
	status = sgx_ecall(eid, 6, &ocall_table_Enclave3, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}
...
sgx_status_t Enclave3_end_session(sgx_enclave_id_t eid, uint32_t* retval, sgx_enclave_id_t src_enclave_id)
{
	...
	ms_end_session_t ms;
	ms.ms_src_enclave_id = src_enclave_id;
	status = sgx_ecall(eid, 7, &ocall_table_Enclave3, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}
