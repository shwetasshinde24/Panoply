...
static sgx_status_t SGX_CDECL Enclave_ocall_write0(void* pms)
{
	ms_ocall_write0_t* ms = SGX_CAST(ms_ocall_write0_t*, pms);
	ocall_write0(ms->ms_buffer, ms->ms_bufsiz);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_write1(void* pms)
{
	ms_ocall_write1_t* ms = SGX_CAST(ms_ocall_write1_t*, pms);
	ocall_write1(ms->ms_buffer, ms->ms_bufsiz);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_write2(void* pms)
{
	ms_ocall_write2_t* ms = SGX_CAST(ms_ocall_write2_t*, pms);
	ocall_write2(ms->ms_buffer, ms->ms_bufsiz);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_write3(void* pms)
{
	ms_ocall_write3_t* ms = SGX_CAST(ms_ocall_write3_t*, pms);
	ocall_write3(ms->ms_buffer, ms->ms_bufsiz);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_write4(void* pms)
{
	ms_ocall_write4_t* ms = SGX_CAST(ms_ocall_write4_t*, pms);
	ocall_write4(ms->ms_buffer, ms->ms_bufsiz);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_write5(void* pms)
{
	ms_ocall_write5_t* ms = SGX_CAST(ms_ocall_write5_t*, pms);
	ocall_write5(ms->ms_buffer, ms->ms_bufsiz);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_write6(void* pms)
{
	ms_ocall_write6_t* ms = SGX_CAST(ms_ocall_write6_t*, pms);
	ocall_write6(ms->ms_buffer, ms->ms_bufsiz);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_write7(void* pms)
{
	ms_ocall_write7_t* ms = SGX_CAST(ms_ocall_write7_t*, pms);
	ocall_write7(ms->ms_buffer, ms->ms_bufsiz);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_write8(void* pms)
{
	ms_ocall_write8_t* ms = SGX_CAST(ms_ocall_write8_t*, pms);
	ocall_write8(ms->ms_buffer, ms->ms_bufsiz);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_write9(void* pms)
{
	ms_ocall_write9_t* ms = SGX_CAST(ms_ocall_write9_t*, pms);
	ocall_write9(ms->ms_buffer, ms->ms_bufsiz);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_context_switch(void* pms)
{
	ms_ocall_context_switch_t* ms = SGX_CAST(ms_ocall_context_switch_t*, pms);
	ms->ms_retval = ocall_context_switch(ms->ms_dummy1, ms->ms_dummy2);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_dummy_clone_process(void* pms)
{
	ms_ocall_dummy_clone_process_t* ms = SGX_CAST(ms_ocall_dummy_clone_process_t*, pms);
	ms->ms_retval = ocall_dummy_clone_process();
...
static sgx_status_t SGX_CDECL Enclave_ocall_fork(void* pms)
{
	...
	ms->ms_retval = ocall_fork(ms->ms_buffer, ms->ms_bufsiz);
...
static const struct {
	...
	25,
	{
		...
		(void*)Enclave_ocall_write0,
		(void*)Enclave_ocall_write1,
		(void*)Enclave_ocall_write2,
		(void*)Enclave_ocall_write3,
		(void*)Enclave_ocall_write4,
		(void*)Enclave_ocall_write5,
		(void*)Enclave_ocall_write6,
		(void*)Enclave_ocall_write7,
		(void*)Enclave_ocall_write8,
		(void*)Enclave_ocall_write9,
		(void*)Enclave_ocall_context_switch,
		(void*)Enclave_ocall_dummy_clone_process,
...
sgx_status_t ecall_set_parsed_section_info(sgx_enclave_id_t eid, int* retval, const char* name, int type, uint64_t val)
{
	sgx_status_t status;
	ms_ecall_set_parsed_section_info_t ms;
...
sgx_status_t ecall_dump_data0(sgx_enclave_id_t eid, int n, size_t size)
{
	sgx_status_t status;
	ms_ecall_dump_data0_t ms;
	ms.ms_n = n;
	ms.ms_size = size;
...
sgx_status_t ecall_dump_data1(sgx_enclave_id_t eid, int n, size_t size)
{
	sgx_status_t status;
	ms_ecall_dump_data1_t ms;
	ms.ms_n = n;
	ms.ms_size = size;
...
sgx_status_t ecall_dump_data2(sgx_enclave_id_t eid, int n, size_t size)
{
	sgx_status_t status;
	ms_ecall_dump_data2_t ms;
	ms.ms_n = n;
	ms.ms_size = size;
...
sgx_status_t ecall_dump_data3(sgx_enclave_id_t eid, int n, size_t size)
{
	sgx_status_t status;
	ms_ecall_dump_data3_t ms;
	ms.ms_n = n;
	ms.ms_size = size;
...
sgx_status_t ecall_dump_data4(sgx_enclave_id_t eid, int n, size_t size)
{
	sgx_status_t status;
	ms_ecall_dump_data4_t ms;
	ms.ms_n = n;
	ms.ms_size = size;
...
sgx_status_t ecall_dump_data5(sgx_enclave_id_t eid, int n, size_t size)
{
	sgx_status_t status;
	ms_ecall_dump_data5_t ms;
	ms.ms_n = n;
	ms.ms_size = size;
...
sgx_status_t ecall_dump_data6(sgx_enclave_id_t eid, int n, size_t size)
{
	sgx_status_t status;
	ms_ecall_dump_data6_t ms;
	ms.ms_n = n;
	ms.ms_size = size;
...
sgx_status_t ecall_dump_data7(sgx_enclave_id_t eid, int n, size_t size)
{
	sgx_status_t status;
	ms_ecall_dump_data7_t ms;
	ms.ms_n = n;
	ms.ms_size = size;
...
sgx_status_t ecall_test_ctx_switch(sgx_enclave_id_t eid, void* buffer, int n)
{
	sgx_status_t status;
	ms_ecall_test_ctx_switch_t ms;
	ms.ms_buffer = buffer;
	ms.ms_n = n;
...
sgx_status_t ecall_simple_ctx_swich(sgx_enclave_id_t eid, int* retval, int n)
{
	sgx_status_t status;
	ms_ecall_simple_ctx_swich_t ms;
	ms.ms_n = n;
