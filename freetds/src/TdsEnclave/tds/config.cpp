...
static inline int getaddrinfo(const char *node, const char *service, const void *hints_cast, void **res_cast)
{
	int ret;
	ocall_getaddrinfo(&ret, node, service, hints_cast, res_cast);
	return ret;
}

static inline void freeaddrinfo(void * res_cast)
{
...
static int tds_read_conf_sections(SGX_WRAPPER_FILE in, const char *server, TDSLOGIN * login);
...
tds_get_home_file(const char *file)
{
	...
	// Fix me: Free outside enclave allocated string
	// free(home);
...
tds_read_conf_file(TDSLOGIN * login, const char *server)
{
	...
	if (!found) {
		...
		if (path) {
			...
			// Fix me: Free outside enclave memory
			// free(path);
		} else {
...
static int
tds_read_conf_sections(SGX_WRAPPER_FILE in, const char *server, TDSLOGIN * login)
{
	int ocall_ret;
 ...
 */
int
tds_read_conf_section(SGX_WRAPPER_FILE in, const char *section, TDSCONFPARSE tds_conf_parse, void *param)
{
	char line[256], *value;
#define option line
	char *s;
...
tds_lookup_host(const char *servername)	/* (I) name of the server                  */
{
	...
	if (getaddrinfo(servername, NULL, &hints, (void**)&addr))
		return NULL;
...
void* ecall_tds_lookup_host(const char* servername)
{
	return tds_lookup_host(servername);
}
		...
		if (!TDS_ISSPACE(line[0])) {
			...
			if (field != NULL && !strcmp(field, "query")) {
				...
				if (!strcmp(field, "tli")) {
					...
					field = strtok_r(NULL, "\n\t ", &lasts);	/* host/port */
					if (strlen(field) >= 18) {
						snprintf(tmp_port, sizeof(line), "%d", hex2num(&field[6]) * 256 + hex2num(&field[8]));
						snprintf(tmp_ip, sizeof(line), "%d.%d.%d.%d", hex2num(&field[10]),
