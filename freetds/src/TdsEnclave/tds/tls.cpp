...
#define OPENSSL_CIPHERS \
	...
	// sgx_wrapper_fprintf(SGX_STDERR, "initializing tls \n");
	...
	/*
		no more CA File certificates load, now CA File will be hardcode inside the others enclave
	*/
	// if (!tds_dstr_isempty(&tds->login->cafile)) {
		// tls_msg = "loading CA file";
		// if (strcasecmp(tds_dstr_cstr(&tds->login->cafile), "system") == 0)
...
// 	/* Perform the TLS handshake */
	tls_msg = "handshake";
	// sgx_wrapper_fprintf(SGX_STDERR, "handshake \n");
	...
	// 	sgx_wrapper_fprintf(SGX_STDERR, "checking hostname \n");
	...
	// }
	{
		...
		cert =  SSL_get_peer_certificate(con);
		tls_msg = "checking certificate";
		int buffer_len = i2d_X509(cert, &buffer);
		// sgx_wrapper_fprintf(SGX_STDERR, "checking certificate loaded in this addr: %p with buffer len: %d \n", cert, buffer_len);
	    // sgx_wrapper_fprintf(SGX_STDERR, "The tds_enclave_id : %d - vrf_enclave_id: %d \n", tds_enclave_id, vrfcert_enclave_id);
	    test_create_session(tds_enclave_id, vrfcert_enclave_id);
	    ATTESTATION_STATUS verifyResult = test_encalve_to_enclave_verify_cert(tds_enclave_id, vrfcert_enclave_id, (uint8_t*)buffer, buffer_len);
	    test_close_session(tds_enclave_id, vrfcert_enclave_id);	

	    if (verifyResult!=SUCCESS)	{
	    	// printf("Certificate check failed but still pass through for test !!!\n");
	    }
	}

	// sgx_wrapper_fprintf(SGX_STDERR, "handshake succeeded \n");
	tdsdump_log(TDS_DBG_INFO1, "handshake succeeded!!\n");
...
cleanup:
	// sgx_wrapper_fprintf(SGX_STDERR, "handshake failed \n");
	if (b2)
		BIO_free(b2);
	if (b)
		BIO_free(b);
