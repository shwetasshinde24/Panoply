...
test0(const char *type, ...)
{
	...
	snprintf(buf, 512, "CREATE TABLE #tmp(a %s)", type);
	exec_query(buf);
	...
	for (;;) {
		...
		snprintf(buf, 512, "INSERT INTO #tmp VALUES(CONVERT(%s,'%s'))", type, value);
		exec_query(buf);
		++num_data;
	}
        va_end(ap);
...
int
ecall_unittest_dataread_main(int argc, char **argv)
{
	...
	// Fix me: This test is not pass yet 
