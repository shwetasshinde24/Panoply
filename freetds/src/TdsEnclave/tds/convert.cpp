...
tds_convert_int8(const TDS_INT8 *src, int desttype, CONV_RESULT * cr)
{
	...
	case TDS_CONVERT_CHAR:
	case CASE_ALL_CHAR:
		snprintf(tmp_str, 24, "%" PRId64, buf);
		return string_to_result(tmp_str, cr);
		break;
	case SYBINT1:
	case SYBUINT1:
...
tds_convert_uint8(const TDS_UINT8 *src, int desttype, CONV_RESULT * cr)
{
	...
	case TDS_CONVERT_CHAR:
	case CASE_ALL_CHAR:
		snprintf(tmp_str, 24, "%" PRIu64, buf);
		return string_to_result(tmp_str, cr);
		break;
	case SYBINT1:
	case SYBUINT1:
...
tds_convert_money4(const TDS_MONEY4 * src, int desttype, CONV_RESULT * cr)
{
	...
	case CASE_ALL_CHAR:
		...
		snprintf(p, 33, "%ld.%02lu", dollars / 100, dollars % 100);
		return string_to_result(tmp_str, cr);
		break;
	case SYBINT1:
	case SYBUINT1:
...
/* FIXME how many big should be this buffer ?? */
	...
	case TDS_CONVERT_CHAR:
	case CASE_ALL_CHAR:
		snprintf(tmp_str, 128, "%.9g", the_value);
		return string_to_result(tmp_str, cr);
		break;
	case SYBINT1:
	case SYBUINT1:
	...
	case SYBNUMERIC:
	case SYBDECIMAL:
		snprintf(tmp_str, 128, "%.*f", cr->n.scale, the_value);
...
tds_convert_flt8(const TDS_FLOAT* src, int desttype, CONV_RESULT * cr)
{
	...
	case TDS_CONVERT_CHAR:
	case CASE_ALL_CHAR:
		snprintf(tmp_str, 25, "%.17g", the_value);
		return string_to_result(tmp_str, cr);
		break;
	case SYBINT1:
	case SYBUINT1:
	...
	case SYBNUMERIC:
	case SYBDECIMAL:
		snprintf(tmp_str, 25, "%.16g", the_value);
		return stringz_to_numeric(tmp_str, cr);
		break;
		/* not allowed */
	case SYBUNIQUE:
...
tds_strftime(char *buf, size_t maxsize, const char *format, const TDSDATEREC * dr, int prec)
{
	...
	for (pz = our_format; (pz = strdup(strstr(pz, "%z"))) != NULL; pz++) {
	...
	if (pz) {
		if (prec || pz <= our_format || pz[-1] != '.') {
			char buf[12];
			snprintf(buf, 12, "%07d", dr->decimicrosecond);
			memcpy(pz, buf, prec);
