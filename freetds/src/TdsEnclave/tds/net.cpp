...
static inline char* tds_addrinfo2str(void *addr, char *name, int namemax)
{
	char* ret;
	wrapper_tds_addrinfo2str(&ret, addr, name, namemax);
	return ret;
}
...
{
	assert(addr != NULL);
	// sgx_wrapper_printf("addrinfo set port : %d ", port);
...
tds_connect_socket(TDSSOCKET *tds, struct addrinfo *addr, unsigned int port, int timeout, int *p_oserr)
{
	...
	*p_oserr = 0;
	...
	fprintf(stderr, "Connecting to %s port %d (TDS version %d.%d)\n", 
			ipaddr, port,
	...
	// Fix me: tds_socket_set_nonblocking need to examine
	...
	if (retval == 0) {
		...
		sgx_wrapper_printf("tds_open_socket: connect(2) returned \"%s\"\n", errstr);
		...
		if (err != ECONNREFUSED && err != ENETUNREACH && err != TDSSOCK_EINPROGRESS) {
			sgx_wrapper_printf("Contents of sockaddr_in", addr->ai_addr, addr->ai_addrlen);
			tdsdump_dump_buf(TDS_DBG_ERROR, "Contents of sockaddr_in", addr->ai_addr, addr->ai_addrlen);
			sgx_wrapper_printf(" sockaddr_in:\t"
							      "%s = %x\n" 
							"\t\t\t%s = %x\n" 
							"\t\t\t%s = %s\n"
...
#else
		// Fix me: uncomment when query.cpp work
		// tds_disconnect(tds);
		int ocall_ret;
		ocall_close(&ocall_ret, tds_get_s(tds));
		if (ocall_ret == -1)
...
tds_select(TDSSOCKET * tds, unsigned tds_sel, int timeout_seconds)
{
	...
	for (seconds = timeout_seconds; timeout_seconds == 0 || seconds > 0; seconds -= poll_seconds) {
		...
		// Fix me: Blocking socket don't need poll 
...
{
	int ret;
	wrapper_tds_socket_set_nonblocking(&ret, sock);
	return ret;
}
...
tds_wakeup_close(TDSPOLLWAKEUP *wakeup)
{
	if (!TDS_IS_SOCKET_INVALID(wakeup->s_signal))	{
		int ocall_ret;
		ocall_close(&ocall_ret, wakeup->s_signal);
		// CLOSESOCKET(wakeup->s_signal);
	}
	if (!TDS_IS_SOCKET_INVALID(wakeup->s_signaled))	{
		int ocall_ret;
		ocall_close(&ocall_ret, wakeup->s_signaled);
 ...
 */
int
tds7_get_instance_ports(SGX_WRAPPER_FILE output, struct addrinfo *addr)
{
	int num_try;
	int ocall_ret;
	struct pollfd fd;
	...
	for (num_try = 0; num_try < 16 && msg_len == 0; ++num_try) {
		...
		if ((msg_len = recv(s, msg, sizeof(msg) - 1, 0)) > 3 && msg[0] == 5) {
			...
			if (0) {	/* To debug, print the whole string. */
				...
				ocall_fputs(&ocall_ret, msg + 3, output);
			}
...
{
	sgx_wrapper_printf("Call tds7_get_instance_port %d\n");
	int num_try;
	struct pollfd fd;
	int retval;
	int ocall_ret;
...
int ecall_tds7_get_instance_ports(SGX_WRAPPER_FILE output, void *addr_cast)
{
	struct addrinfo *addr = (struct addrinfo *)addr_cast;
	return tds7_get_instance_ports(output, addr);

}

int ecall_tds7_get_instance_port(void *addr_cast, const char *instance)
{
	struct addrinfo *addr = (struct addrinfo *)addr_cast;
	return tds7_get_instance_port(addr, instance);
}
