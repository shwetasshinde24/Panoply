...
void ecall_tds_alloc_login(int use_environment, TDSLOGIN* login)
{
	// assert(sizeof(TDSLOGIN)==size);
	// TDSLOGIN* return_result = tds_alloc_login(use_environment);
	const char *server_name = TDS_DEF_SERVER;
	...
	// memcpy(login, return_result, sizeof(TDSLOGIN)); 
	// return ;
}
