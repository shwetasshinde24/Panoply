...
void* increase_counter(void *)
{
    ...
    ret = TdsEnclave_ecall_increase_counter(global_eid, &cnr);
...
{
    sgx_status_t ret = SGX_ERROR_UNEXPECTED;
    ret = TdsEnclave_ecall_producer(global_eid);
    if (ret != SGX_SUCCESS)
        abort();
}
...
{
    sgx_status_t ret = SGX_ERROR_UNEXPECTED;
    ret = TdsEnclave_ecall_consumer(global_eid);
    if (ret != SGX_SUCCESS)
        abort();
}
...
void ecall_thread_functions(void)
{
    ...
    TdsEnclave_ecall_set_enclave_id(global_eid, global_eid);
    TdsEnclave_ecall_test_pthread_create_and_join(global_eid);
